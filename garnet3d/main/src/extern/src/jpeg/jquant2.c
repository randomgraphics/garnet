...
typedef struct {
  ...
  JPEG_BOOL needs_zeroed;		/* JPEG_TRUE if next pass must zero histogram */

  /* Variables for Floyd-Steinberg dithering */
  FSERRPTR fserrors;		/* accumulated errors */
  JPEG_BOOL on_odd_row;		/* flag to remember which row we are on */
...
/* This version performs Floyd-Steinberg dithering */
{
  ...
  for (row = 0; row < num_rows; row++) {
    ...
    if (cquantize->on_odd_row) {
      ...
      cquantize->on_odd_row = JPEG_FALSE; /* flip for next time */
    } else {
      /* work left to right in this row */
      dir = 1;
      dir3 = 3;
      errorptr = cquantize->fserrors; /* => entry before first real column */
      cquantize->on_odd_row = JPEG_TRUE; /* flip for next time */
    }
...
finish_pass1 (j_decompress_ptr cinfo)
{
  ...
  cquantize->needs_zeroed = JPEG_TRUE;
}
...
METHODDEF(void)
start_pass_2_quant (j_decompress_ptr cinfo, JPEG_BOOL is_pre_scan)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  hist3d histogram = cquantize->histogram;
  int i;
  ...
  if (is_pre_scan) {
    ...
    cquantize->needs_zeroed = JPEG_TRUE; /* Always zero histogram */
  } else {
    /* Set up method pointers */
	...
	init_error_limit(cinfo);
      cquantize->on_odd_row = JPEG_FALSE;
    }
  ...
  if (cquantize->needs_zeroed) {
    ...
    cquantize->needs_zeroed = JPEG_FALSE;
  }
}
...
jinit_2pass_quantizer (j_decompress_ptr cinfo)
{
  ...
  cquantize->needs_zeroed = JPEG_TRUE; /* histogram is garbage now */
