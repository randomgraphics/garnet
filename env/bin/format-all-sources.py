...
ap.add_argument(
    "-d",
    action="store_true",
    help="Only process files that are different than the master/main branch",
)
args = ap.parse_args()
...
if args.d:
    ...
    all_files = (
        subprocess.check_output(cmdline, cwd=root_dir).decode("utf-8").splitlines()
    )
else:
...
def format_one_file_with_black(x):
    ...
    if not args.q and len(out) > 0:
        with lock:
            print(out)
    # Forward stderr only for actual violations; suppress black's informational "All done!" summary.
    err = ret.stderr.decode("utf-8").strip()
    is_informational = (
        err.startswith("All done!") or "file would be left unchanged" in err
    )
    if len(err) > 0 and not is_informational:
        with lock:
            sys.stderr.write(err)
