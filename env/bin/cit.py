...
def binary_template(name):
    return str(pathlib.Path(args.build_dir) / "{variant}" / "bin" / name)


def run_all_tests(args):
    utils.run_the_latest_binary(
        binary_template("GNtest-unit-tests"),
        args.test_args,
        check=True,
        cwd=root_folder,
    )
    utils.run_the_latest_binary(
        binary_template("GNtest-internal"), args.test_args, check=True, cwd=root_folder
    )
    utils.run_the_latest_binary(
        binary_template("GNsample-rdg2-open-graph-simple-triangle"),
        ["t"],
        check=True,
        cwd=root_folder,
    )
    ...
    utils.run_the_latest_binary(
        binary_template("GNsample-gpu2-simple-copy"), [], check=True, cwd=root_folder
    )
    utils.run_the_latest_binary(
        binary_template("GNsample-gpu2-simple-compute"), [], check=True, cwd=root_folder
    )
...
def run_style_check():
    ...
    result = subprocess.run(
        [
            sys.executable,
            pathlib.Path(os.path.realpath(__file__)).parent / "format-all-sources.py",
            "-dqn",
        ],
...
ap.add_argument(
    "--build-dir",
    default=os.environ.get("GARNET_BUILD_DIR", "build"),
    help="Build output root, matching build.py -b.",
)
