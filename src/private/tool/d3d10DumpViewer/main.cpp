...
static bool sLoadBinary( const XmlElement & node, const StrA & attr, const StrA & basedir, DynaArray<T> & result )
{
    ...
    {
        GN_WARN(sLogger)("%s : binary file not found :  %s!", node.getLocation(), fullname.rawptr() );
    }
...
struct D3D10StateDump
{
    ...
    bool onCreate( ID3D10Device & dev )
    {
        ...
        if( !vs.binary.empty() )
        {
            GN_DX_CHECK_RETURN( dev.CreateVertexShader( vs.binary.rawptr(), vs.binary.size(), &vs.comptr ), false );
        }

        // ps
        if( !ps.binary.empty() )
        {
            GN_DX_CHECK_RETURN( dev.CreatePixelShader( ps.binary.rawptr(), ps.binary.size(), &ps.comptr ), false );
        }

        // gs
        if( !gs.binary.empty() )
        {
            GN_DX_CHECK_RETURN( dev.CreateGeometryShader( gs.binary.rawptr(), gs.binary.size(), &gs.comptr ), false );
        }
