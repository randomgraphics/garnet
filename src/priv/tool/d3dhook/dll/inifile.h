...
namespace GN
{
    ...
    public:
        ...
        StrA  dreads(const char * section, const char * key, const char * defaultValue) { StrA  v; if (!reads(section, key, v)) v = defaultValue; return v; }
        int   dreadi(const char * section, const char * key, int   defaultValue)        { int   v; if (!readi(section, key, v)) v = defaultValue; return v; }
        float dreadf(const char * section, const char * key, float defaultValue)        { float v; if (!readf(section, key, v)) v = defaultValue; return v; }
        bool  dreadb(const char * section, const char * key, bool  defaultValue)        { bool  v; if (!readb(section, key, v)) v = defaultValue; return v; }
