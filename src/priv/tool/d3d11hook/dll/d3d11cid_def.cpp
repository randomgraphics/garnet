// (Script generated header. DO NOT EDIT.)
#include "pch.h"
#include "d3d11cid_def.h"
const char * g_D3D11CallIDText[] =
{

    // CID for ID3D11DeviceChild
    "CID_ID3D11DeviceChild_GetDevice",
    "CID_ID3D11DeviceChild_GetPrivateData",
    "CID_ID3D11DeviceChild_SetPrivateData",
    "CID_ID3D11DeviceChild_SetPrivateDataInterface",

    // CID for ID3D11DepthStencilState
    "CID_ID3D11DepthStencilState_GetDesc",

    // CID for ID3D11BlendState
    "CID_ID3D11BlendState_GetDesc",

    // CID for ID3D11RasterizerState
    "CID_ID3D11RasterizerState_GetDesc",

    // CID for ID3D11Resource
    "CID_ID3D11Resource_GetType",
    "CID_ID3D11Resource_SetEvictionPriority",
    "CID_ID3D11Resource_GetEvictionPriority",

    // CID for ID3D11Buffer
    "CID_ID3D11Buffer_GetDesc",

    // CID for ID3D11Texture1D
    "CID_ID3D11Texture1D_GetDesc",

    // CID for ID3D11Texture2D
    "CID_ID3D11Texture2D_GetDesc",

    // CID for ID3D11Texture3D
    "CID_ID3D11Texture3D_GetDesc",

    // CID for ID3D11View
    "CID_ID3D11View_GetResource",

    // CID for ID3D11ShaderResourceView
    "CID_ID3D11ShaderResourceView_GetDesc",

    // CID for ID3D11RenderTargetView
    "CID_ID3D11RenderTargetView_GetDesc",

    // CID for ID3D11DepthStencilView
    "CID_ID3D11DepthStencilView_GetDesc",

    // CID for ID3D11UnorderedAccessView
    "CID_ID3D11UnorderedAccessView_GetDesc",

    // CID for ID3D11VertexShader

    // CID for ID3D11HullShader

    // CID for ID3D11DomainShader

    // CID for ID3D11GeometryShader

    // CID for ID3D11PixelShader

    // CID for ID3D11ComputeShader

    // CID for ID3D11InputLayout

    // CID for ID3D11SamplerState
    "CID_ID3D11SamplerState_GetDesc",

    // CID for ID3D11Asynchronous
    "CID_ID3D11Asynchronous_GetDataSize",

    // CID for ID3D11Query
    "CID_ID3D11Query_GetDesc",

    // CID for ID3D11Predicate

    // CID for ID3D11Counter
    "CID_ID3D11Counter_GetDesc",

    // CID for ID3D11ClassInstance
    "CID_ID3D11ClassInstance_GetClassLinkage",
    "CID_ID3D11ClassInstance_GetDesc",
    "CID_ID3D11ClassInstance_GetInstanceName",
    "CID_ID3D11ClassInstance_GetTypeName",

    // CID for ID3D11ClassLinkage
    "CID_ID3D11ClassLinkage_GetClassInstance",
    "CID_ID3D11ClassLinkage_CreateClassInstance",

    // CID for ID3D11CommandList
    "CID_ID3D11CommandList_GetContextFlags",

    // CID for ID3D11DeviceContext
    "CID_ID3D11DeviceContext_VSSetConstantBuffers",
    "CID_ID3D11DeviceContext_PSSetShaderResources",
    "CID_ID3D11DeviceContext_PSSetShader",
    "CID_ID3D11DeviceContext_PSSetSamplers",
    "CID_ID3D11DeviceContext_VSSetShader",
    "CID_ID3D11DeviceContext_DrawIndexed",
    "CID_ID3D11DeviceContext_Draw",
    "CID_ID3D11DeviceContext_Map",
    "CID_ID3D11DeviceContext_Unmap",
    "CID_ID3D11DeviceContext_PSSetConstantBuffers",
    "CID_ID3D11DeviceContext_IASetInputLayout",
    "CID_ID3D11DeviceContext_IASetVertexBuffers",
    "CID_ID3D11DeviceContext_IASetIndexBuffer",
    "CID_ID3D11DeviceContext_DrawIndexedInstanced",
    "CID_ID3D11DeviceContext_DrawInstanced",
    "CID_ID3D11DeviceContext_GSSetConstantBuffers",
    "CID_ID3D11DeviceContext_GSSetShader",
    "CID_ID3D11DeviceContext_IASetPrimitiveTopology",
    "CID_ID3D11DeviceContext_VSSetShaderResources",
    "CID_ID3D11DeviceContext_VSSetSamplers",
    "CID_ID3D11DeviceContext_Begin",
    "CID_ID3D11DeviceContext_End",
    "CID_ID3D11DeviceContext_GetData",
    "CID_ID3D11DeviceContext_SetPredication",
    "CID_ID3D11DeviceContext_GSSetShaderResources",
    "CID_ID3D11DeviceContext_GSSetSamplers",
    "CID_ID3D11DeviceContext_OMSetRenderTargets",
    "CID_ID3D11DeviceContext_OMSetRenderTargetsAndUnorderedAccessViews",
    "CID_ID3D11DeviceContext_OMSetBlendState",
    "CID_ID3D11DeviceContext_OMSetDepthStencilState",
    "CID_ID3D11DeviceContext_SOSetTargets",
    "CID_ID3D11DeviceContext_DrawAuto",
    "CID_ID3D11DeviceContext_DrawIndexedInstancedIndirect",
    "CID_ID3D11DeviceContext_DrawInstancedIndirect",
    "CID_ID3D11DeviceContext_Dispatch",
    "CID_ID3D11DeviceContext_DispatchIndirect",
    "CID_ID3D11DeviceContext_RSSetState",
    "CID_ID3D11DeviceContext_RSSetViewports",
    "CID_ID3D11DeviceContext_RSSetScissorRects",
    "CID_ID3D11DeviceContext_CopySubresourceRegion",
    "CID_ID3D11DeviceContext_CopyResource",
    "CID_ID3D11DeviceContext_UpdateSubresource",
    "CID_ID3D11DeviceContext_CopyStructureCount",
    "CID_ID3D11DeviceContext_ClearRenderTargetView",
    "CID_ID3D11DeviceContext_ClearUnorderedAccessViewUint",
    "CID_ID3D11DeviceContext_ClearUnorderedAccessViewFloat",
    "CID_ID3D11DeviceContext_ClearDepthStencilView",
    "CID_ID3D11DeviceContext_GenerateMips",
    "CID_ID3D11DeviceContext_SetResourceMinLOD",
    "CID_ID3D11DeviceContext_GetResourceMinLOD",
    "CID_ID3D11DeviceContext_ResolveSubresource",
    "CID_ID3D11DeviceContext_ExecuteCommandList",
    "CID_ID3D11DeviceContext_HSSetShaderResources",
    "CID_ID3D11DeviceContext_HSSetShader",
    "CID_ID3D11DeviceContext_HSSetSamplers",
    "CID_ID3D11DeviceContext_HSSetConstantBuffers",
    "CID_ID3D11DeviceContext_DSSetShaderResources",
    "CID_ID3D11DeviceContext_DSSetShader",
    "CID_ID3D11DeviceContext_DSSetSamplers",
    "CID_ID3D11DeviceContext_DSSetConstantBuffers",
    "CID_ID3D11DeviceContext_CSSetShaderResources",
    "CID_ID3D11DeviceContext_CSSetUnorderedAccessViews",
    "CID_ID3D11DeviceContext_CSSetShader",
    "CID_ID3D11DeviceContext_CSSetSamplers",
    "CID_ID3D11DeviceContext_CSSetConstantBuffers",
    "CID_ID3D11DeviceContext_VSGetConstantBuffers",
    "CID_ID3D11DeviceContext_PSGetShaderResources",
    "CID_ID3D11DeviceContext_PSGetShader",
    "CID_ID3D11DeviceContext_PSGetSamplers",
    "CID_ID3D11DeviceContext_VSGetShader",
    "CID_ID3D11DeviceContext_PSGetConstantBuffers",
    "CID_ID3D11DeviceContext_IAGetInputLayout",
    "CID_ID3D11DeviceContext_IAGetVertexBuffers",
    "CID_ID3D11DeviceContext_IAGetIndexBuffer",
    "CID_ID3D11DeviceContext_GSGetConstantBuffers",
    "CID_ID3D11DeviceContext_GSGetShader",
    "CID_ID3D11DeviceContext_IAGetPrimitiveTopology",
    "CID_ID3D11DeviceContext_VSGetShaderResources",
    "CID_ID3D11DeviceContext_VSGetSamplers",
    "CID_ID3D11DeviceContext_GetPredication",
    "CID_ID3D11DeviceContext_GSGetShaderResources",
    "CID_ID3D11DeviceContext_GSGetSamplers",
    "CID_ID3D11DeviceContext_OMGetRenderTargets",
    "CID_ID3D11DeviceContext_OMGetRenderTargetsAndUnorderedAccessViews",
    "CID_ID3D11DeviceContext_OMGetBlendState",
    "CID_ID3D11DeviceContext_OMGetDepthStencilState",
    "CID_ID3D11DeviceContext_SOGetTargets",
    "CID_ID3D11DeviceContext_RSGetState",
    "CID_ID3D11DeviceContext_RSGetViewports",
    "CID_ID3D11DeviceContext_RSGetScissorRects",
    "CID_ID3D11DeviceContext_HSGetShaderResources",
    "CID_ID3D11DeviceContext_HSGetShader",
    "CID_ID3D11DeviceContext_HSGetSamplers",
    "CID_ID3D11DeviceContext_HSGetConstantBuffers",
    "CID_ID3D11DeviceContext_DSGetShaderResources",
    "CID_ID3D11DeviceContext_DSGetShader",
    "CID_ID3D11DeviceContext_DSGetSamplers",
    "CID_ID3D11DeviceContext_DSGetConstantBuffers",
    "CID_ID3D11DeviceContext_CSGetShaderResources",
    "CID_ID3D11DeviceContext_CSGetUnorderedAccessViews",
    "CID_ID3D11DeviceContext_CSGetShader",
    "CID_ID3D11DeviceContext_CSGetSamplers",
    "CID_ID3D11DeviceContext_CSGetConstantBuffers",
    "CID_ID3D11DeviceContext_ClearState",
    "CID_ID3D11DeviceContext_Flush",
    "CID_ID3D11DeviceContext_GetType",
    "CID_ID3D11DeviceContext_GetContextFlags",
    "CID_ID3D11DeviceContext_FinishCommandList",

    // CID for ID3D11VideoDecoder
    "CID_ID3D11VideoDecoder_GetCreationParameters",
    "CID_ID3D11VideoDecoder_GetDriverHandle",

    // CID for ID3D11VideoProcessorEnumerator
    "CID_ID3D11VideoProcessorEnumerator_GetVideoProcessorContentDesc",
    "CID_ID3D11VideoProcessorEnumerator_CheckVideoProcessorFormat",
    "CID_ID3D11VideoProcessorEnumerator_GetVideoProcessorCaps",
    "CID_ID3D11VideoProcessorEnumerator_GetVideoProcessorRateConversionCaps",
    "CID_ID3D11VideoProcessorEnumerator_GetVideoProcessorCustomRate",
    "CID_ID3D11VideoProcessorEnumerator_GetVideoProcessorFilterRange",

    // CID for ID3D11VideoProcessor
    "CID_ID3D11VideoProcessor_GetContentDesc",
    "CID_ID3D11VideoProcessor_GetRateConversionCaps",

    // CID for ID3D11AuthenticatedChannel
    "CID_ID3D11AuthenticatedChannel_GetCertificateSize",
    "CID_ID3D11AuthenticatedChannel_GetCertificate",
    "CID_ID3D11AuthenticatedChannel_GetChannelHandle",

    // CID for ID3D11CryptoSession
    "CID_ID3D11CryptoSession_GetCryptoType",
    "CID_ID3D11CryptoSession_GetDecoderProfile",
    "CID_ID3D11CryptoSession_GetCertificateSize",
    "CID_ID3D11CryptoSession_GetCertificate",
    "CID_ID3D11CryptoSession_GetCryptoSessionHandle",

    // CID for ID3D11VideoDecoderOutputView
    "CID_ID3D11VideoDecoderOutputView_GetDesc",

    // CID for ID3D11VideoProcessorInputView
    "CID_ID3D11VideoProcessorInputView_GetDesc",

    // CID for ID3D11VideoProcessorOutputView
    "CID_ID3D11VideoProcessorOutputView_GetDesc",

    // CID for ID3D11VideoContext
    "CID_ID3D11VideoContext_GetDecoderBuffer",
    "CID_ID3D11VideoContext_ReleaseDecoderBuffer",
    "CID_ID3D11VideoContext_DecoderBeginFrame",
    "CID_ID3D11VideoContext_DecoderEndFrame",
    "CID_ID3D11VideoContext_SubmitDecoderBuffers",
    "CID_ID3D11VideoContext_DecoderExtension",
    "CID_ID3D11VideoContext_VideoProcessorSetOutputTargetRect",
    "CID_ID3D11VideoContext_VideoProcessorSetOutputBackgroundColor",
    "CID_ID3D11VideoContext_VideoProcessorSetOutputColorSpace",
    "CID_ID3D11VideoContext_VideoProcessorSetOutputAlphaFillMode",
    "CID_ID3D11VideoContext_VideoProcessorSetOutputConstriction",
    "CID_ID3D11VideoContext_VideoProcessorSetOutputStereoMode",
    "CID_ID3D11VideoContext_VideoProcessorSetOutputExtension",
    "CID_ID3D11VideoContext_VideoProcessorGetOutputTargetRect",
    "CID_ID3D11VideoContext_VideoProcessorGetOutputBackgroundColor",
    "CID_ID3D11VideoContext_VideoProcessorGetOutputColorSpace",
    "CID_ID3D11VideoContext_VideoProcessorGetOutputAlphaFillMode",
    "CID_ID3D11VideoContext_VideoProcessorGetOutputConstriction",
    "CID_ID3D11VideoContext_VideoProcessorGetOutputStereoMode",
    "CID_ID3D11VideoContext_VideoProcessorGetOutputExtension",
    "CID_ID3D11VideoContext_VideoProcessorSetStreamFrameFormat",
    "CID_ID3D11VideoContext_VideoProcessorSetStreamColorSpace",
    "CID_ID3D11VideoContext_VideoProcessorSetStreamOutputRate",
    "CID_ID3D11VideoContext_VideoProcessorSetStreamSourceRect",
    "CID_ID3D11VideoContext_VideoProcessorSetStreamDestRect",
    "CID_ID3D11VideoContext_VideoProcessorSetStreamAlpha",
    "CID_ID3D11VideoContext_VideoProcessorSetStreamPalette",
    "CID_ID3D11VideoContext_VideoProcessorSetStreamPixelAspectRatio",
    "CID_ID3D11VideoContext_VideoProcessorSetStreamLumaKey",
    "CID_ID3D11VideoContext_VideoProcessorSetStreamStereoFormat",
    "CID_ID3D11VideoContext_VideoProcessorSetStreamAutoProcessingMode",
    "CID_ID3D11VideoContext_VideoProcessorSetStreamFilter",
    "CID_ID3D11VideoContext_VideoProcessorSetStreamExtension",
    "CID_ID3D11VideoContext_VideoProcessorGetStreamFrameFormat",
    "CID_ID3D11VideoContext_VideoProcessorGetStreamColorSpace",
    "CID_ID3D11VideoContext_VideoProcessorGetStreamOutputRate",
    "CID_ID3D11VideoContext_VideoProcessorGetStreamSourceRect",
    "CID_ID3D11VideoContext_VideoProcessorGetStreamDestRect",
    "CID_ID3D11VideoContext_VideoProcessorGetStreamAlpha",
    "CID_ID3D11VideoContext_VideoProcessorGetStreamPalette",
    "CID_ID3D11VideoContext_VideoProcessorGetStreamPixelAspectRatio",
    "CID_ID3D11VideoContext_VideoProcessorGetStreamLumaKey",
    "CID_ID3D11VideoContext_VideoProcessorGetStreamStereoFormat",
    "CID_ID3D11VideoContext_VideoProcessorGetStreamAutoProcessingMode",
    "CID_ID3D11VideoContext_VideoProcessorGetStreamFilter",
    "CID_ID3D11VideoContext_VideoProcessorGetStreamExtension",
    "CID_ID3D11VideoContext_VideoProcessorBlt",
    "CID_ID3D11VideoContext_NegotiateCryptoSessionKeyExchange",
    "CID_ID3D11VideoContext_EncryptionBlt",
    "CID_ID3D11VideoContext_DecryptionBlt",
    "CID_ID3D11VideoContext_StartSessionKeyRefresh",
    "CID_ID3D11VideoContext_FinishSessionKeyRefresh",
    "CID_ID3D11VideoContext_GetEncryptionBltKey",
    "CID_ID3D11VideoContext_NegotiateAuthenticatedChannelKeyExchange",
    "CID_ID3D11VideoContext_QueryAuthenticatedChannel",
    "CID_ID3D11VideoContext_ConfigureAuthenticatedChannel",
    "CID_ID3D11VideoContext_VideoProcessorSetStreamRotation",
    "CID_ID3D11VideoContext_VideoProcessorGetStreamRotation",

    // CID for ID3D11VideoDevice
    "CID_ID3D11VideoDevice_CreateVideoDecoder",
    "CID_ID3D11VideoDevice_CreateVideoProcessor",
    "CID_ID3D11VideoDevice_CreateAuthenticatedChannel",
    "CID_ID3D11VideoDevice_CreateCryptoSession",
    "CID_ID3D11VideoDevice_CreateVideoDecoderOutputView",
    "CID_ID3D11VideoDevice_CreateVideoProcessorInputView",
    "CID_ID3D11VideoDevice_CreateVideoProcessorOutputView",
    "CID_ID3D11VideoDevice_CreateVideoProcessorEnumerator",
    "CID_ID3D11VideoDevice_GetVideoDecoderProfileCount",
    "CID_ID3D11VideoDevice_GetVideoDecoderProfile",
    "CID_ID3D11VideoDevice_CheckVideoDecoderFormat",
    "CID_ID3D11VideoDevice_GetVideoDecoderConfigCount",
    "CID_ID3D11VideoDevice_GetVideoDecoderConfig",
    "CID_ID3D11VideoDevice_GetContentProtectionCaps",
    "CID_ID3D11VideoDevice_CheckCryptoKeyExchange",
    "CID_ID3D11VideoDevice_SetPrivateData",
    "CID_ID3D11VideoDevice_SetPrivateDataInterface",

    // CID for ID3D11Device
    "CID_ID3D11Device_CreateBuffer",
    "CID_ID3D11Device_CreateTexture1D",
    "CID_ID3D11Device_CreateTexture2D",
    "CID_ID3D11Device_CreateTexture3D",
    "CID_ID3D11Device_CreateShaderResourceView",
    "CID_ID3D11Device_CreateUnorderedAccessView",
    "CID_ID3D11Device_CreateRenderTargetView",
    "CID_ID3D11Device_CreateDepthStencilView",
    "CID_ID3D11Device_CreateInputLayout",
    "CID_ID3D11Device_CreateVertexShader",
    "CID_ID3D11Device_CreateGeometryShader",
    "CID_ID3D11Device_CreateGeometryShaderWithStreamOutput",
    "CID_ID3D11Device_CreatePixelShader",
    "CID_ID3D11Device_CreateHullShader",
    "CID_ID3D11Device_CreateDomainShader",
    "CID_ID3D11Device_CreateComputeShader",
    "CID_ID3D11Device_CreateClassLinkage",
    "CID_ID3D11Device_CreateBlendState",
    "CID_ID3D11Device_CreateDepthStencilState",
    "CID_ID3D11Device_CreateRasterizerState",
    "CID_ID3D11Device_CreateSamplerState",
    "CID_ID3D11Device_CreateQuery",
    "CID_ID3D11Device_CreatePredicate",
    "CID_ID3D11Device_CreateCounter",
    "CID_ID3D11Device_CreateDeferredContext",
    "CID_ID3D11Device_OpenSharedResource",
    "CID_ID3D11Device_CheckFormatSupport",
    "CID_ID3D11Device_CheckMultisampleQualityLevels",
    "CID_ID3D11Device_CheckCounterInfo",
    "CID_ID3D11Device_CheckCounter",
    "CID_ID3D11Device_CheckFeatureSupport",
    "CID_ID3D11Device_GetPrivateData",
    "CID_ID3D11Device_SetPrivateData",
    "CID_ID3D11Device_SetPrivateDataInterface",
    "CID_ID3D11Device_GetFeatureLevel",
    "CID_ID3D11Device_GetCreationFlags",
    "CID_ID3D11Device_GetDeviceRemovedReason",
    "CID_ID3D11Device_GetImmediateContext",
    "CID_ID3D11Device_SetExceptionMode",
    "CID_ID3D11Device_GetExceptionMode",

    // CID for ID3D11Device1
    "CID_ID3D11Device1_GetImmediateContext1",
    "CID_ID3D11Device1_CreateDeferredContext1",
    "CID_ID3D11Device1_CreateBlendState1",
    "CID_ID3D11Device1_CreateRasterizerState1",
    "CID_ID3D11Device1_CreateDeviceContextState",
    "CID_ID3D11Device1_OpenSharedResource1",
    "CID_ID3D11Device1_OpenSharedResourceByName",

    // CID for ID3D11Debug
    "CID_ID3D11Debug_SetFeatureMask",
    "CID_ID3D11Debug_GetFeatureMask",
    "CID_ID3D11Debug_SetPresentPerRenderOpDelay",
    "CID_ID3D11Debug_GetPresentPerRenderOpDelay",
    "CID_ID3D11Debug_SetSwapChain",
    "CID_ID3D11Debug_GetSwapChain",
    "CID_ID3D11Debug_ValidateContext",
    "CID_ID3D11Debug_ReportLiveDeviceObjects",
    "CID_ID3D11Debug_ValidateContextForDispatch",

    // CID for ID3D11SwitchToRef
    "CID_ID3D11SwitchToRef_SetUseRef",
    "CID_ID3D11SwitchToRef_GetUseRef",

    // CID for ID3D11TracingDevice
    "CID_ID3D11TracingDevice_SetShaderTrackingOptionsByType",
    "CID_ID3D11TracingDevice_SetShaderTrackingOptions",

    // CID for ID3D11RefTrackingOptions
    "CID_ID3D11RefTrackingOptions_SetTrackingOptions",

    // CID for ID3D11RefDefaultTrackingOptions
    "CID_ID3D11RefDefaultTrackingOptions_SetTrackingOptions",

    // CID for ID3D11InfoQueue
    "CID_ID3D11InfoQueue_SetMessageCountLimit",
    "CID_ID3D11InfoQueue_ClearStoredMessages",
    "CID_ID3D11InfoQueue_GetMessage",
    "CID_ID3D11InfoQueue_GetNumMessagesAllowedByStorageFilter",
    "CID_ID3D11InfoQueue_GetNumMessagesDeniedByStorageFilter",
    "CID_ID3D11InfoQueue_GetNumStoredMessages",
    "CID_ID3D11InfoQueue_GetNumStoredMessagesAllowedByRetrievalFilter",
    "CID_ID3D11InfoQueue_GetNumMessagesDiscardedByMessageCountLimit",
    "CID_ID3D11InfoQueue_GetMessageCountLimit",
    "CID_ID3D11InfoQueue_AddStorageFilterEntries",
    "CID_ID3D11InfoQueue_GetStorageFilter",
    "CID_ID3D11InfoQueue_ClearStorageFilter",
    "CID_ID3D11InfoQueue_PushEmptyStorageFilter",
    "CID_ID3D11InfoQueue_PushCopyOfStorageFilter",
    "CID_ID3D11InfoQueue_PushStorageFilter",
    "CID_ID3D11InfoQueue_PopStorageFilter",
    "CID_ID3D11InfoQueue_GetStorageFilterStackSize",
    "CID_ID3D11InfoQueue_AddRetrievalFilterEntries",
    "CID_ID3D11InfoQueue_GetRetrievalFilter",
    "CID_ID3D11InfoQueue_ClearRetrievalFilter",
    "CID_ID3D11InfoQueue_PushEmptyRetrievalFilter",
    "CID_ID3D11InfoQueue_PushCopyOfRetrievalFilter",
    "CID_ID3D11InfoQueue_PushRetrievalFilter",
    "CID_ID3D11InfoQueue_PopRetrievalFilter",
    "CID_ID3D11InfoQueue_GetRetrievalFilterStackSize",
    "CID_ID3D11InfoQueue_AddMessage",
    "CID_ID3D11InfoQueue_AddApplicationMessage",
    "CID_ID3D11InfoQueue_SetBreakOnCategory",
    "CID_ID3D11InfoQueue_SetBreakOnSeverity",
    "CID_ID3D11InfoQueue_SetBreakOnID",
    "CID_ID3D11InfoQueue_GetBreakOnCategory",
    "CID_ID3D11InfoQueue_GetBreakOnSeverity",
    "CID_ID3D11InfoQueue_GetBreakOnID",
    "CID_ID3D11InfoQueue_SetMuteDebugOutput",
    "CID_ID3D11InfoQueue_GetMuteDebugOutput",

    // CID for IDXGIObject
    "CID_IDXGIObject_SetPrivateData",
    "CID_IDXGIObject_SetPrivateDataInterface",
    "CID_IDXGIObject_GetPrivateData",
    "CID_IDXGIObject_GetParent",

    // CID for IDXGIDeviceSubObject
    "CID_IDXGIDeviceSubObject_GetDevice",

    // CID for IDXGIResource
    "CID_IDXGIResource_GetSharedHandle",
    "CID_IDXGIResource_GetUsage",
    "CID_IDXGIResource_SetEvictionPriority",
    "CID_IDXGIResource_GetEvictionPriority",

    // CID for IDXGIKeyedMutex
    "CID_IDXGIKeyedMutex_AcquireSync",
    "CID_IDXGIKeyedMutex_ReleaseSync",

    // CID for IDXGISurface
    "CID_IDXGISurface_GetDesc",
    "CID_IDXGISurface_Map",
    "CID_IDXGISurface_Unmap",

    // CID for IDXGISurface1
    "CID_IDXGISurface1_GetDC",
    "CID_IDXGISurface1_ReleaseDC",

    // CID for IDXGIAdapter
    "CID_IDXGIAdapter_EnumOutputs",
    "CID_IDXGIAdapter_GetDesc",
    "CID_IDXGIAdapter_CheckInterfaceSupport",

    // CID for IDXGIOutput
    "CID_IDXGIOutput_GetDesc",
    "CID_IDXGIOutput_GetDisplayModeList",
    "CID_IDXGIOutput_FindClosestMatchingMode",
    "CID_IDXGIOutput_WaitForVBlank",
    "CID_IDXGIOutput_TakeOwnership",
    "CID_IDXGIOutput_ReleaseOwnership",
    "CID_IDXGIOutput_GetGammaControlCapabilities",
    "CID_IDXGIOutput_SetGammaControl",
    "CID_IDXGIOutput_GetGammaControl",
    "CID_IDXGIOutput_SetDisplaySurface",
    "CID_IDXGIOutput_GetDisplaySurfaceData",
    "CID_IDXGIOutput_GetFrameStatistics",

    // CID for IDXGISwapChain
    "CID_IDXGISwapChain_Present",
    "CID_IDXGISwapChain_GetBuffer",
    "CID_IDXGISwapChain_SetFullscreenState",
    "CID_IDXGISwapChain_GetFullscreenState",
    "CID_IDXGISwapChain_GetDesc",
    "CID_IDXGISwapChain_ResizeBuffers",
    "CID_IDXGISwapChain_ResizeTarget",
    "CID_IDXGISwapChain_GetContainingOutput",
    "CID_IDXGISwapChain_GetFrameStatistics",
    "CID_IDXGISwapChain_GetLastPresentCount",

    // CID for IDXGIFactory
    "CID_IDXGIFactory_EnumAdapters",
    "CID_IDXGIFactory_MakeWindowAssociation",
    "CID_IDXGIFactory_GetWindowAssociation",
    "CID_IDXGIFactory_CreateSwapChain",
    "CID_IDXGIFactory_CreateSoftwareAdapter",

    // CID for IDXGIDevice
    "CID_IDXGIDevice_GetAdapter",
    "CID_IDXGIDevice_CreateSurface",
    "CID_IDXGIDevice_QueryResourceResidency",
    "CID_IDXGIDevice_SetGPUThreadPriority",
    "CID_IDXGIDevice_GetGPUThreadPriority",

    // CID for IDXGIFactory1
    "CID_IDXGIFactory1_EnumAdapters1",
    "CID_IDXGIFactory1_IsCurrent",

    // CID for IDXGIAdapter1
    "CID_IDXGIAdapter1_GetDesc1",

    // CID for IDXGIDevice1
    "CID_IDXGIDevice1_SetMaximumFrameLatency",
    "CID_IDXGIDevice1_GetMaximumFrameLatency",

    // CID for IDXGIDisplayControl
    "CID_IDXGIDisplayControl_IsStereoEnabled",
    "CID_IDXGIDisplayControl_SetStereoEnabled",

    // CID for IDXGIOutputDuplication
    "CID_IDXGIOutputDuplication_GetDesc",
    "CID_IDXGIOutputDuplication_AcquireNextFrame",
    "CID_IDXGIOutputDuplication_GetFrameDirtyRects",
    "CID_IDXGIOutputDuplication_GetFrameMoveRects",
    "CID_IDXGIOutputDuplication_GetFramePointerShape",
    "CID_IDXGIOutputDuplication_MapDesktopSurface",
    "CID_IDXGIOutputDuplication_UnMapDesktopSurface",
    "CID_IDXGIOutputDuplication_ReleaseFrame",

    // CID for IDXGISurface2
    "CID_IDXGISurface2_GetResource",

    // CID for IDXGIResource1
    "CID_IDXGIResource1_CreateSubresourceSurface",
    "CID_IDXGIResource1_CreateSharedHandle",

    // CID for IDXGIDevice2
    "CID_IDXGIDevice2_OfferResources",
    "CID_IDXGIDevice2_ReclaimResources",
    "CID_IDXGIDevice2_EnqueueSetEvent",

    // CID for IDXGISwapChain1
    "CID_IDXGISwapChain1_GetDesc1",
    "CID_IDXGISwapChain1_GetFullscreenDesc",
    "CID_IDXGISwapChain1_GetHwnd",
    "CID_IDXGISwapChain1_GetCoreWindow",
    "CID_IDXGISwapChain1_Present1",
    "CID_IDXGISwapChain1_IsTemporaryMonoSupported",
    "CID_IDXGISwapChain1_GetRestrictToOutput",
    "CID_IDXGISwapChain1_SetBackgroundColor",
    "CID_IDXGISwapChain1_GetBackgroundColor",
    "CID_IDXGISwapChain1_SetRotation",
    "CID_IDXGISwapChain1_GetRotation",

    // CID for IDXGIFactory2
    "CID_IDXGIFactory2_IsWindowedStereoEnabled",
    "CID_IDXGIFactory2_CreateSwapChainForHwnd",
    "CID_IDXGIFactory2_CreateSwapChainForCoreWindow",
    "CID_IDXGIFactory2_GetSharedResourceAdapterLuid",
    "CID_IDXGIFactory2_RegisterStereoStatusWindow",
    "CID_IDXGIFactory2_RegisterStereoStatusEvent",
    "CID_IDXGIFactory2_UnregisterStereoStatus",
    "CID_IDXGIFactory2_RegisterOcclusionStatusWindow",
    "CID_IDXGIFactory2_RegisterOcclusionStatusEvent",
    "CID_IDXGIFactory2_UnregisterOcclusionStatus",
    "CID_IDXGIFactory2_CreateSwapChainForComposition",

    // CID for IDXGIAdapter2
    "CID_IDXGIAdapter2_GetDesc2",

    // CID for IDXGIOutput1
    "CID_IDXGIOutput1_GetDisplayModeList1",
    "CID_IDXGIOutput1_FindClosestMatchingMode1",
    "CID_IDXGIOutput1_GetDisplaySurfaceData1",
    "CID_IDXGIOutput1_DuplicateOutput",
};
