...
static bool sLoadVShader( LPDIRECT3DVERTEXSHADER9 & result, const GN::StrA & name )
{
    ...
    if( 0 == GN::strCmpI( ".asm", ext.cstr() ) ||
        ...
        0 == GN::strCmpI( ".xvs", ext.cstr() ) ||
        0 == GN::strCmpI( ".vsx", ext.cstr() ) )
    {
        result = GN::d3d::assembleVSFromFile( gD3D.getDevice(), name.cstr() );
    }
    else
    {
    	result = GN::d3d::compileVSFromFile( gD3D.getDevice(), name.cstr() );
    }
...
static bool sLoadPShader( LPDIRECT3DPIXELSHADER9 & result, const GN::StrA & name )
{
    ...
    if( 0 == GN::strCmpI( ".asm", ext.cstr() ) ||
        ...
        0 == GN::strCmpI( ".vsx", ext.cstr() ) )
    {
        result = GN::d3d::assemblePSFromFile( gD3D.getDevice(), name.cstr() );
    }
    else
    {
    	result = GN::d3d::compilePSFromFile( gD3D.getDevice(), name.cstr() );
    }
...
bool GN::d3d::D3D::present()
{
    ...
    win::processMswMessages( mWindow.getWindow() );
