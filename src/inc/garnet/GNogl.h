...
namespace GN { namespace ogl
{
    ...
    inline void clearScreen(
        const Vector4f & color = { 0.f, 0.f, 0.f, 1.f },
        float depth = 1.0f,
        uint32_t stencil = 0,
    ...
    public:
        ...
        void bind(size_t unit) const { GN_ASSERT(glIsSampler(_id)); glBindSampler((GLuint)unit, _id); }
    };
    ...
    public:
        ...
        uint32_t getFBO(size_t level) const
        {
            return _mips[level].fbo;
        }
    ...
    public:
        ...
        void apply() const {
            ...
            std::visit([&](auto && v) {
                ...
                else if constexpr (std::is_same_v<T, unsigned int>) glUniform1ui(_location, v);
                else if constexpr (std::is_same_v<T, float>)        glUniform1f (_location, v);
                else if constexpr (std::is_same_v<T, Vector2f>)     glUniform2f (_location, v.x, v.y);
                else if constexpr (std::is_same_v<T, Vector3f>)     glUniform3f (_location, v.x, v.y, v.z);
                else if constexpr (std::is_same_v<T, Vector4f>)     glUniform4f (_location, v.x, v.y, v.z, v.w);
                else if constexpr (std::is_same_v<T, Vector2i>)     glUniform2i (_location, v.x, v.y);
                else if constexpr (std::is_same_v<T, Vector3i>)     glUniform3i (_location, v.x, v.y, v.z);
                else if constexpr (std::is_same_v<T, Vector4i>)     glUniform4i (_location, v.x, v.y, v.z, v.w);
                else if constexpr (std::is_same_v<T, Vector2u>)     glUniform2ui(_location, v.x, v.y);
                else if constexpr (std::is_same_v<T, Vector3u>)     glUniform3ui(_location, v.x, v.y, v.z);
                else if constexpr (std::is_same_v<T, Vector4u>)     glUniform4ui(_location, v.x, v.y, v.z, v.w);
                else if constexpr (std::is_same_v<T, Matrix33f>)    glUniformMatrix3fv(_location, 1, false, v.data());
                else if constexpr (std::is_same_v<T, Matrix44f>)    glUniformMatrix4fv(_location, 1, false, v.data());
