...
#define GN_E2_DEFINE_PUBLIC_ABSTRACT_TYPE(baseType) \
protected:                                          \
...
// The very basic/root type of everything in this world is GN::RCRT64 — reference counting,
// runtime type info, and a 64-bit id. e2 always constructs objects through the
// (type, id, name) constructor with an id generated by the owning Universe.
...
template<typename T>
concept RcrtRefTarget = (!CompleteType<T>) || std::derived_from<T, RCRT64>;

} // namespace detail

/// Smart reference to an e2 object.
template<detail::RcrtRefTarget T>
using Ref = AutoRef<T>;

/// Abstraction of host operating system, providing cross platform abstraction to Window, keyboard, mouse support.
struct OperatingDomain : RCRT64 {
