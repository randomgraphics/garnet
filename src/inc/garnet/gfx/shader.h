...
struct GpuProgramLanguage {
    ...
    /// check for validity
    bool valid() const { return HLSL9 == *this || HLSL10 == *this || GLSL == *this; }
    ...
    const char * toString() const {
        ...
        case GLSL:
            return "GLSL";
        default:
            return "INVALID_GPU_PROGRAM_LANGUAGE";
        };
    }

    /// convert string to from string
    static GpuProgramLanguage sFromString(const char * s) {
        if (0 == str::compareI(s, "HLSL9"))
            return HLSL9;
        else if (0 == str::compareI(s, "HLSL10"))
            return HLSL10;
        else if (0 == str::compareI(s, "GLSL"))
            return GLSL;
        else
            return INVALID;
    }
...
struct ShaderModel {
    ...
    static uint32 sFromString(const char * str) {
        ...
        while (*str) {
            ...
            if (0 == str::compareI("SM_2_0", s, len))
                flags |= SM_2_0;
            else if (0 == str::compareI("SM_3_0", s, len))
                flags |= SM_3_0;
            else if (0 == str::compareI("SM_3_X", s, len))
                flags |= SM_3_X;
            else if (0 == str::compareI("SM_4_0", s, len))
                flags |= SM_4_0;
            else if (0 == str::compareI("SM_5_0", s, len))
                flags |= SM_5_0;
            else if (0 == str::compareI("GLSL_1_10", s, len))
                flags |= GLSL_1_10;
            else if (0 == str::compareI("GLSL_3_30", s, len))
                flags |= GLSL_3_30;
    ...
    static StrA sToString(uint32 flags) {
        ...
        if (flags & SM_2_0) {
            flags &= ~SM_2_0;
            str += str.empty() ? "SM_2_0" : "|SM_2_0";
        }
        if (flags & SM_3_0) {
            flags &= ~SM_3_0;
            str += str.empty() ? "SM_3_0" : "|SM_3_0";
        }
        if (flags & SM_3_X) {
            flags &= ~SM_3_X;
            str += str.empty() ? "SM_3_X" : "|SM_3_X";
        }
        if (flags & SM_4_0) {
            flags &= ~SM_4_0;
            str += str.empty() ? "SM_4_0" : "|SM_4_0";
        }
        if (flags & SM_5_0) {
            flags &= ~SM_5_0;
            str += str.empty() ? "SM_5_0" : "|SM_5_0";
        }
        if (flags & GLSL_1_10) {
            flags &= ~GLSL_1_10;
            str += str.empty() ? "GLSL_1_10" : "|GLSL_1_10";
        }
        if (flags & GLSL_3_30) {
            flags &= ~GLSL_3_30;
            str += str.empty() ? "GLSL_3_30" : "|GLSL_3_30";
        }
...
struct GpuProgramDesc {
    ...
    /// default constructor
    ///
    GpuProgramDesc(const char * name_ = nullptr): lang(GpuProgramLanguage::INVALID), shaderModels(0), name(name_), optimize(true), debug(true) {
...
public:
    ...
    GpuProgramParameterDesc()
        : uniforms(mUniformArray, mUniformCount, mUniformArrayStride), textures(mTextureArray, mTextureCount, mTextureArrayStride),
          attributes(mAttributeArray, mAttributeCount, mAttributeArrayStride), mUniformCount(0), mTextureCount(0), mAttributeCount(0) {}
