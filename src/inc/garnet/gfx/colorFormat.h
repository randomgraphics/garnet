...
#define GN_MAKE_COLOR_FORMAT(layout, sign, sw0123) \
    GN_MAKE_COLOR_FORMAT_EX(layout, sign, sign, (sw0123 & 7), ((sw0123 >> 3) & 7), ((sw0123 >> 6) & 7), ((sw0123 >> 9) & 7))

///
/// compose color format from simplified swizzles
///
#define GN_MAKE_COLOR_FORMAT2(layout, sign012, sign3, sw0123) \
    GN_MAKE_COLOR_FORMAT_EX(layout, sign012, sign3, (sw0123 & 7), ((sw0123 >> 3) & 7), ((sw0123 >> 6) & 7), ((sw0123 >> 9) & 7))
...
union GN_API ColorFormat {
    ...
    ///
    bool valid() const {
        return layout < NUM_COLOR_LAYOUTS && sign012 <= SIGN_FLOAT && sign3 <= SIGN_FLOAT && swizzle0 <= SWIZZLE_1 && swizzle1 <= SWIZZLE_1 &&
               swizzle2 <= SWIZZLE_1 && swizzle3 <= SWIZZLE_1 && 0 == reserved;
    }
...
inline Rgba8 convertColorFormat(const Rgba32f & c) {
    return Rgba8((uint8_t) math::clamp(c.x * 255.0f, 0.0f, 255.0f), (uint8_t) math::clamp(c.y * 255.0f, 0.0f, 255.0f),
                 (uint8_t) math::clamp(c.z * 255.0f, 0.0f, 255.0f), (uint8_t) math::clamp(c.w * 255.0f, 0.0f, 255.0f));
}
