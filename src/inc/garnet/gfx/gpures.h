...
public:
  ...
  /// loaded.
  static AutoRef<TextureResource> loadFromFile(GpuResourceDatabase &db,
                                               const char *filename);
...
struct MeshVertexElement {
  ...
  char semantic[16] =
      {}; ///< Semantic name (null terminated string, 15 characters at most).
  ...
  bool operator==(const MeshVertexElement &rhs) const {
    return format == rhs.format && stream == rhs.stream &&
           offset == rhs.offset &&
...
struct MeshVertexFormat {
  ...
  MeshVertexElement elements[MAX_VERTEX_ELEMENTS] =
  ...
  uint16 inline calcStreamStride(uint32 stream) const {
    ...
    for (uint32 i = 0; i < numElements; ++i) {
      ...
      if (stream == e.stream && stride < elementEnd)
        stride = elementEnd;
    }
    return stride;
  }
...
struct MeshResourceDesc : public MeshResourceDescBase {
  void *vertices[GpuContext::MAX_VERTEX_BUFFERS] =
      {};                  ///< NULL pointer means vertex
                           ///< data are undefined
...
public:
  ...
  //@{
  static const Guid &guid();
  static AutoRef<MeshResource> loadFromFile(GpuResourceDatabase &db,
                                            const char *filename);
  //@}
  ...
  void
  calculateBoundingBox(Box<float> &) const; // AABB: axis aligned bounding box
  void calculateBoundingSphere(Sphere<float> &) const;
  //@}
...
struct GN_API EffectResourceDesc {
  ...
  struct GN_API EffectGpuProgramDesc {
    ...
    DynaArray<char> shaderSourceBuffer; ///< optional buffer used to store store
                                        ///< shader source.
    StringMap<char, StrA>
        textures; ///< textures. Key is shader parameter name, value is name of
                  ///< one texture in EffectResourceDesc.textures.
    StringMap<char, StrA>
        uniforms; ///< uniforms. Key is shader parameter name, value is name of
                  ///< one uniform in EffectResourceDesc.textures.
    StringMap<char, StrA>
        attributes; ///< attributes. Key is shader parameter name, value is name
  ...
  struct EffectRenderStateDesc {
    ...
    /// object
    template <typename T> struct OverridableVariable {
      T value;
      ...
      /// set value
      template <typename T2> OverridableVariable &operator=(const T2 &rhs) {
        value = (T)rhs;
        overridden = true;
        return *this;
      }
  ...
  EffectRenderStateDesc
      renderstates; ///< Root render state descriptor for the effect.
...
public:
  ...
  /// loaded.
  static AutoRef<EffectResource> loadFromFile(GpuResourceDatabase &db,
                                              const char *filename);
  ...
  bool hasTexture(const char *name) const {
    return PARAMETER_NOT_FOUND != findTexture(name);
  }
  ...
  bool hasUniform(const char *name) const {
    return PARAMETER_NOT_FOUND != findUniform(name);
  }
  ...
  bool hasAttribute(const char *name) const {
    return PARAMETER_NOT_FOUND != findAttribute(name);
  }
...
struct MeshResourceSubset {
  ...
  bool isWholeMesh() const {
    return 0 == basevtx && 0 == numvtx && 0 == startidx && 0 == numidx;
  }
...
struct GN_API ModelResourceDesc {
  ...
  /// effect parameter
  ///
  bool hasTexture(const char *effectParameterName) const {
    return NULL != textures.find(effectParameterName);
  }
  ...
  /// effect parameter
  ///
  bool hasUniform(const char *effectParameterName) const {
    return NULL != uniforms.find(effectParameterName);
  }
...
public:
  //@{
  static const Guid &guid();
  static AutoRef<ModelResource> loadFromFile(GpuResourceDatabase &db,
                                             const char *filename);
  //@}
  ...
  textureResource(const char *effectParameterName) const;

  void setUniformResource(const char *effectParameterName, GpuResource *);
  AutoRef<UniformResource>
  uniformResource(const char *effectParameterName) const;

  void setMeshResource(GpuResource *mesh,
                       const MeshResourceSubset *subset = NULL);
...
public:
  ...
  //@{
  bool registerResourceFactory(const Guid &type, const char *descriptiveName,
  ...
  AutoRef<GpuResource> findOrCreateResource(const Guid &type, const char *name,
                                            bool *isExistingResource = NULL);
  bool validResource(const Guid &type, const GpuResource *resource)
      const; // valid resource pointer of specific type
  bool validResource(const GpuResource *resource)
      const; // valid resource pointer of whatever type.
  ...
  template <class T>
  inline AutoRef<T> findOrCreateResource(const char *name,
                                         bool *isExistingResource = NULL);
  //@}
  ...
  template <typename T> void setStandardUniform(int index, const T &value) {
    setStandardUniform(index, &value, sizeof(T));
  }

  void setTransform(const Matrix44f &proj, const Matrix44f &view);

  void setLight0(const Vector4f &diffuse, const Vector4f &ambient,
                 const Vector4f &specular, const Vector3f &position,
