...
#define M33_CALCULATE_ONE_CELL(r, c) t[r][c] = rows[r][0] * m[0][c] + rows[r][1] * m[1][c] + rows[r][2] * m[2][c];
...
Matrix33<T> & Matrix33<T>::inverse() {
    ...
    T fDet = src[0][0] * result[0][0] + src[0][1] * result[1][0] + src[0][2] * result[2][0];
    ...
    for (int iRow = 0; iRow < 3; iRow++) {
        for (int iCol = 0; iCol < 3; iCol++) result[iRow][iCol] *= fInvDet;
    }
...
// -------------------------------------------------------------------------
template<typename T>
Matrix33<T> & Matrix33<T>::lookAtLh(const Vector3<T> & forward, const Vector3<T> & up) {
    Vector3<T> F = forward;
    F.normalize();
...
// -------------------------------------------------------------------------
template<typename T>
Matrix33<T> & Matrix33<T>::lookAtRh(const Vector3<T> & forward, const Vector3<T> & up) {
    Vector3<T> F = forward;
    F.normalize();
...
#define M44_CALCULATE_ONE_CELL(r, c) t[r][c] = rows[r][0] * m[0][c] + rows[r][1] * m[1][c] + rows[r][2] * m[2][c] + rows[r][3] * m[3][c];
...
Matrix44<T> & Matrix44<T>::inverse() {
    ...
    for (c = 0; c < 4; c++) {
        ...
        rowMax = c;
        for (r = c + 1; r < 4; r++) {
            if (::fabs(a[r][c]) > ::fabs(a[rowMax][c])) { rowMax = r; }
        }
...
// -------------------------------------------------------------------------
template<typename T>
Matrix44<T> & Matrix44<T>::rotate(const Vector3<T> & v, T angle) {
    Vector3<T> nrm = v;
    nrm.normalize();
...
// -------------------------------------------------------------------------
template<typename T>
Matrix44<T> & Matrix44<T>::lookAtLh(const Vector3<T> & eye, const Vector3<T> & to, const Vector3<T> & up) {
    Vector3<T> F = to - eye;
    F.normalize();
...
// -------------------------------------------------------------------------
template<typename T>
Matrix44<T> & Matrix44<T>::lookAtRh(const Vector3<T> & eye, const Vector3<T> & to, const Vector3<T> & up) {
    Vector3<T> F = to - eye;
    F.normalize();
...
// -------------------------------------------------------------------------
template<typename T>
Matrix44<T> & Matrix44<T>::orthoOGLLh(T left, T right, T bottom, T top, T znear, T zfar) {
    GN_ASSERT(left != right && bottom != top && znear != zfar);

    T w = right - left, h = top - bottom, d = zfar - znear;
...
// -------------------------------------------------------------------------
template<typename T>
Matrix44<T> & Matrix44<T>::orthoOGLRh(T left, T right, T bottom, T top, T znear, T zfar) {
    GN_ASSERT(left != right && bottom != top && znear != zfar);
...
// -------------------------------------------------------------------------
template<typename T>
Matrix44<T> & Matrix44<T>::orthoD3DLh(T left, T right, T bottom, T top, T znear, T zfar) {
    GN_ASSERT(left != right && bottom != top && znear != zfar);
...
// -------------------------------------------------------------------------
template<typename T>
Matrix44<T> & Matrix44<T>::orthoD3DRh(T left, T right, T bottom, T top, T znear, T zfar) {
    GN_ASSERT(left != right && bottom != top && znear != zfar);
...
// -------------------------------------------------------------------------
template<typename T>
Matrix44<T> & Matrix44<T>::perspectiveOGLLh(T fovy, T ratio, T znear, T zfar) {
    GN_ASSERT(0.0f != fovy && 0.0f != ratio && znear != zfar);

    T h = tan(fovy / 2.0f) * znear * 2.0f, w = h * ratio, d = zfar - znear;
...
// -------------------------------------------------------------------------
template<typename T>
Matrix44<T> & Matrix44<T>::perspectiveOGLRh(T fovy, T ratio, T znear, T zfar) {
    GN_ASSERT(0.0f != fovy && 0.0f != ratio && znear != zfar);
...
// -------------------------------------------------------------------------
template<typename T>
Matrix44<T> & Matrix44<T>::perspectiveD3DLh(T fovy, T ratio, T znear, T zfar) {
    GN_ASSERT(0.0f != fovy && 0.0f != ratio && znear != zfar);
...
// -------------------------------------------------------------------------
template<typename T>
Matrix44<T> & Matrix44<T>::perspectiveD3DRh(T fovy, T ratio, T znear, T zfar) {
    GN_ASSERT(0.0f != fovy && 0.0f != ratio && znear != zfar);
...
StrA Matrix44<T>::print() const {
    return StrA::format("%f,\t%f,\t%f,\t%f,\n"
                        ...
                        "%f,\t%f,\t%f,\t%f,\n"
                        "%f,\t%f,\t%f,\t%f,\n",
                        rows[0][0], rows[0][1], rows[0][2], rows[0][3], rows[1][0], rows[1][1], rows[1][2], rows[1][3], rows[2][0], rows[2][1], rows[2][2],
                        rows[2][3], rows[3][0], rows[3][1], rows[3][2], rows[3][3]);
}
} // namespace GN
