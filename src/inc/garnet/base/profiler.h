...
//@{
    ...
    #define GN_DEFINE_STATIC_PROFILER(name, desc) \
        static GN::ProfileTimer & GN_JOIN(__GN_profiler_, name) = GN::ProfilerManager::sGetGlobalInstance().getTimer(desc)
    ...
    #define GN_START_PROFILER(name) \
        if (0) {                    \
            ...
            GN_JOIN(__GN_profiler_, name).start()
    ...
    #define GN_STOP_PROFILER(name) \
        if (0) {                   \
            ...
            GN_JOIN(__GN_profiler_, name).stop()

    ///
    /// define an automatic profiler that evaluate the time of its life-scope.
    ///
    #define GN_SCOPE_PROFILER(name, desc)      \
        GN_DEFINE_STATIC_PROFILER(name, desc); \
        GN::ScopeTimer GN_JOIN(__GN_scope_profiler, name)(&GN_JOIN(__GN_profiler_, name))
...
public:
    ...
    /// start the timer
    ///
    ScopeTimer(const char * name): mTimer(&ProfilerManager::sGetGlobalInstance().getTimer(name)) { mTimer->start(); }

    ///
    /// start the user-defined timer
    ///
    ScopeTimer(ProfileTimer * timer): mTimer(timer) { mTimer->start(); }
