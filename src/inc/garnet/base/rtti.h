/// \file rtti.h
/// Run-time type information: unique 64-bit IDs, string names, optional base types.
/// Safe across DLL boundaries: getNextUniqueTypeId() is defined in one TU and exported (GN_API)
/// so all modules share the same counter.
...
#define GN_REGISTER_RUNTIME_TYPE(...)                                                                                          \
    ...
    static const GN::RuntimeType::TypeInfo & TYPE_INFO() {                                                                     \
        static const auto bases = GN::RttiBaseTypeIds<__VA_ARGS__>::BASE();                                                    \
        static const auto ti    = GN::RuntimeType::TypeInfo {.id    = TYPE_ID,                                                 \
                                                             .name  = GN_FUNCTION,                                             \
                                                             .level = GN::RttiBaseTypeIds<__VA_ARGS__>::LEVEL() + 1,           \
                                                             .bases = bases.data(),                                            \
                                                             .count = bases.size()};                                           \
        return ti;                                                                                                             \
    }                                                                                                                          \
    const GN::RuntimeType::TypeInfo & typeInfo() const { return GN::RttiBaseTypeIds<__VA_ARGS__>::FirstBaseType::typeInfo(); } \
