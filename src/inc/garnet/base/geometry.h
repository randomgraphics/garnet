...
public:
    ...
    ///
    friend Vector2 operator*(const Vector2 & v, T f) { return Vector2(f * v.x, f * v.y); }
    ///
    /// Multiply Vector2 by a scalar
    ///
    friend Vector2 operator*(T f, const Vector2 & v) { return Vector2(f * v.x, f * v.y); }
    ///
    /// Divide Vector2 by a scalar
    ///
    friend Vector2 operator/(const Vector2 & v, T f) { return Vector2(v.x / f, v.y / f); }
    ///
...
public:
    ...
    /// dot production
    ///
    static T sDot(const Vector2 & v1, const Vector2 & v2) { return v1.x * v2.x + v1.y * v2.y; }
};
...
public:
    ...
    friend Vector3 operator*(const Vector3 & v, T f) { return Vector3(f * v.x, f * v.y, f * v.z); }
    friend Vector3 operator*(T f, const Vector3 & v) { return Vector3(f * v.x, f * v.y, f * v.z); }
    friend Vector3 operator/(const Vector3 & v, T f) { return Vector3(v.x / f, v.y / f, v.z / f); }
    friend Vector3 operator/(const Vector3 & a, const Vector3 & b) { return Vector3(a.x / b.x, a.y / b.y, a.z / b.z); }
...
public:
    ...
    static T       sDot(const Vector3 & v1, const Vector3 & v2) { return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z; }
    ...
    static void sCross(Vector3 & o, const Vector3 & v1, const Vector3 & v2) {
        o.set(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z, v1.x * v2.y - v1.y * v2.x);
    }
...
public:
    ...
    friend bool    operator==(const Vector4 & a, const Vector4 & b) { return ((a.x == b.x) && (a.y == b.y) && (a.z == b.z) && (a.w == b.w)); }
    friend bool    operator!=(const Vector4 & a, const Vector4 & b) { return ((a.x != b.x) || (a.y != b.y) || (a.z != b.z) || (a.w != b.w)); }
    friend Vector4 operator-(const Vector4 & a) { return Vector4(-a.x, -a.y, -a.z, -a.w); }
    ...
    friend Vector4 operator*(const Vector4 & a, const Vector4 & b) { return Vector4(a.x * b.x, a.y * b.y, a.z * b.z, a.w * b.w); }
    friend Vector4 operator*(const Vector4 & v, T f) { return Vector4(f * v.x, f * v.y, f * v.z, f * v.w); }
    friend Vector4 operator*(T f, const Vector4 & v) { return Vector4(f * v.x, f * v.y, f * v.z, f * v.w); }
    friend Vector4 operator/(const Vector4 & v, T f) { return Vector4(v.x / f, v.y / f, v.z / f, v.w / f); }
    friend Vector4 operator/(const Vector4 & a, const Vector4 & b) { return Vector4(a.x / b.x, a.y / b.y, a.z / b.z, a.w / b.w); }
...
public:
    ...
    static T sDot(const Vector4 & v1, const Vector4 & v2) { return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z + v1.w * v2.w; }
...
public:
    ...
    Matrix33() {}
    //
    Matrix33(T _00, T _01, T _02, T _10, T _11, T _12, T _20, T _21, T _22) {
...
public:
    ...
    //
    explicit Matrix44(const Matrix33<T> & m33, const Vector3<T> & col3 = Vector3<T>(0, 0, 0), const Vector3<T> & row3 = Vector3<T>(0, 0, 0),
                      T _33 = ((T) 1.0)) {
...
public:
    ...
    //    return ret;
    //}
    friend Vector4<T> operator*(const Matrix44 & m, const Vector4<T> & v) {
        Vector4<T> ret;
...
public:
    ...
    Matrix44 & set(T _00, T _01, T _02, T _03, T _10, T _11, T _12, T _13, T _20, T _21, T _22, T _23, T _30, T _31, T _32, T _33) {
        rows[0].set(_00, _01, _02, _03);
        rows[1].set(_10, _11, _12, _13);
        rows[2].set(_20, _21, _22, _23);
        rows[3].set(_30, _31, _32, _33);
    ...
    Matrix44 & set(const Matrix33<T> & m33, const Vector3<T> & col3 = Vector3<T>(0, 0, 0), const Vector3<T> & row3 = Vector3<T>(0, 0, 0), T _33 = ((T) 1.0)) {
        rows[0].set(m33[0], col3.x);
        rows[1].set(m33[1], col3.y);
        rows[2].set(m33[2], col3.z);
        rows[3].set(row3, _33);
    ...
    static Matrix44 sIdentity() {
        return Matrix44((T) 1, (T) 0, (T) 0, (T) 0, (T) 0, (T) 1, (T) 0, (T) 0, (T) 0, (T) 0, (T) 1, (T) 0, (T) 0, (T) 0, (T) 0, (T) 1);
    }
    Matrix44 & transpose() {
        detail::swap(rows[0][1], rows[1][0]);
        detail::swap(rows[0][2], rows[2][0]);
    ...
    Matrix44 &      translate(const Vector3<T> & v) { return translate(v.x, v.y, v.z); }
    Matrix44 &      translate(const Vector2<T> & v) { return translate(v.x, v.y, (T) 0); }
    static Matrix44 sTranslate(const Vector3<T> & v) {
        Matrix44 m;
        m.translate(v);
        return m;
    }
    Matrix44 & lookAtLh(const Vector3<T> & eye, const Vector3<T> & to, const Vector3<T> & up);
    Matrix44 & lookAtRh(const Vector3<T> & eye, const Vector3<T> & to, const Vector3<T> & up);
    Matrix44 & lookAt(const Vector3<T> & eye, const Vector3<T> & to, const Vector3<T> & up) {
#if GN_BUILD_LEFT_HAND
    ...
    ///     投影矩阵的函数。
    ///
    Matrix44 & orthoOGLLh(T left, T right, T bottom, T top, T znear, T zfar);

    Matrix44 & orthoOGLRh(T left, T right, T bottom, T top, T znear, T zfar);

    Matrix44 & orthoOGL(T left, T right, T bottom, T top, T znear, T zfar) {
#if GN_BUILD_LEFT_HAND
    ...
    Matrix44 & orthoD3DLh(T left, T right, T bottom, T top, T znear, T zfar);
    Matrix44 & orthoD3DRh(T left, T right, T bottom, T top, T znear, T zfar);
    Matrix44 & orthoD3D(T left, T right, T bottom, T top, T znear, T zfar) {
#if GN_BUILD_LEFT_HAND
    ...
    ///
    Matrix44 & perspectiveOGLLh(T fovy, T ratio, T znear, T zfar);
    Matrix44 & perspectiveOGLRh(T fovy, T ratio, T znear, T zfar);
    Matrix44 & perspectiveOGL(T fovy, T ratio, T znear, T zfar) {
#if GN_BUILD_LEFT_HAND
    ...
    Matrix44 & perspectiveD3DLh(T fovy, T ratio, T znear, T zfar);
    Matrix44 & perspectiveD3DRh(T fovy, T ratio, T znear, T zfar);
    Matrix44 & perspectiveD3D(T fovy, T ratio, T znear, T zfar) {
#if GN_BUILD_LEFT_HAND
...
public:
    ...
    //
    Quaternion(T x_, T y_, T z_, T w_): v(x_, y_, z_), w(w_) {}
    //@}
...
public:
    ...
    /// equality
    ///
    friend bool operator==(const Quaternion & a, const Quaternion & b) { return a.w == b.w && a.v == b.v; }
...
public:
    ...
    /// identity quaternion
    static Quaternion sIdentity() { return Quaternion((T) 0, (T) 0, (T) 0, (T) 1); }

    ///
    /// get norm
    ///
    T getNormal() const { return w * w + Vector3<T>::sDot(v, v); }
    ///
    ...
    ///
    Quaternion & fromRotation(const Vector3<T> & axis, const T angle) {
        T r = angle / (T) 2.0;
        v   = axis * sin(r);
        w   = cos(r);
    ...
    ///
    Quaternion & fromEuler(T pitch, T yaw, T roll) {
        ...
        v.set(cr * sp * cy + sr * cp * sy, cr * cp * sy - sr * sp * cy, sr * cp * cy - cr * sp * sy);
        w = cr * cp * cy + sr * sp * sy;
        return *this;
    }
    ///
    /// construct from euler angles
    ///
    Quaternion & fromEuler(const Vector3<T> & euler) { return fromEuler(euler.x, euler.y, euler.z); }
    ///
...
public:
    ...
    //@{
    Plane3() {}
    Plane3(T a_, T b_, T c_, T d_): n(a_, b_, c_), d(d_) {}
    Plane3(const Plane3 & p): n(p.n), d(p.d) {}
    Plane3(const Vector3<T> & point, const Vector3<T> & normal) { fromPointNormal(point, normal); }
    Plane3(const Vector3<T> & v1, const Vector3<T> & v2, const Vector3<T> & v3) { fromPoints(v1, v2, v3); }
    //@}
...
public:
    ...
    /// type conversion
    ///
    operator const T *() const { return n; }
    ///
    /// type conversion
    ///
    operator T *() { return n; }
    ...
    /// equality
    ///
    bool operator==(const Plane3 & p) const { return n == p.n && d == p.d; }
    ///
    /// inequality
    ///
    bool operator!=(const Plane3 & p) const { return n != p.n || d != p.d; }
    ...
    /// Negate a plane
    ///
    friend Plane3 operator-(const Plane3 & p) { return Plane3(-p.n.x, -p.n.y, -p.n.z, -p.d); }
    ///
    /// dot production with vector3 ( ax + by + cz + d )
    ///
    friend T operator*(const Plane3 & p, const Vector3<T> & v) { return Vector3<T>::dot(p.n, v) + p.d; }
    ///
    /// dot production with 3D vector
    ///
    friend T operator*(const Vector3<T> & v, const Plane3 & p) { return p * v; }
    ///
    /// dot production with vector4 ( ax + by + cz + dw )
    ///
    friend T operator*(const Plane3 & p, const Vector4<T> & v) { return p.n.x * v.x + p.n.y * v.y + p.n.z * v.z + p.d * v.w; }
    ///
    /// dot production with 4D vector
    ///
    friend T operator*(const Vector4<T> & v, const Plane3 & p) { return p * v; }
    //@}
...
public:
    ...
    ///
    Plane3 & fromPointNormal(const Vector3<T> & point, const Vector3<T> & normal) {
        n = normal;
    ...
    ///
    Plane3 & fromPoints(const Vector3<T> & v1, const Vector3<T> & v2, const Vector3<T> & v3) {
    ...
    ///
    Vector4<T> & toVec4() { return *reinterpret_cast<Vector4<T> *>(this); }
    ///
    /// treate plane as an 4-D vector
    ///
    const Vector4<T> & toVec4() const { return *reinterpret_cast<const Vector4<T> *>(this); }
    //@}
};
...
public:
    ...
    ///
    const PointType & origin() const { return *reinterpret_cast<const PointType *>(this); }
    ///
    /// get rectangle origin
    ///
    PointType & origin() { return *reinterpret_cast<PointType *>(this); }
    ///
    /// get rectangle size
    ///
    const PointType & size() const { return *reinterpret_cast<const PointType *>(&w); }
    ///
    /// get rectangle size
    ///
    PointType & size() { return *reinterpret_cast<PointType *>(&w); }
    ///
    /// Check if the point is inside the rectangle
    ///
    bool contain(const PointType & p) const {
        return (((w >= 0) && x <= p.x && p.x < (x + w)) || ((w < 0) && (x + w) <= p.x && p.x < x)) &&
               (((y >= 0) && y <= p.y && p.y < (y + h)) || ((y < 0) && y <= p.y && p.y < (y + h)));
    }
    ///
...
public:
    ...
    ///
    Box(T x_, T y_, T z_, T w_, T h_, T d_): x(x_), y(y_), z(z_), w(w_), h(h_), d(d_) {}
    ///
    /// construct from 2 points
    ///
    Box(const PointType & v1, const PointType & v2): x(v1.x), y(v1.y), z(v1.z), w(v2.x - v1.x), h(v2.y - v1.y), d(v2.z - v1.z) {}
    ///
    /// copy constructor
    ///
...
public:
    ...
    /// equal operator
    ///
    bool operator==(const Box & b) const { return x == b.x && y == b.y && z == b.z && w == b.w && h == b.h && d == b.d; }
    ///
    /// non-equal operator
    ///
    bool operator!=(const Box & b) const { return x != b.x || y != b.y || z != b.z || w != b.w || h != b.h || d != b.d; }
...
public:
    ...
    PointType corner(int i) const {
        ...
        case 1:
            return PointType(x + w, y, z);
        case 2:
            return PointType(x + w, y + h, z);
        case 3:
            return PointType(x, y + h, z);
        case 4:
            return PointType(x, y, z + d);
        case 5:
            return PointType(x + w, y, z + d);
        case 6:
            return PointType(x + w, y + h, z + d);
        case 7:
            return PointType(x, y + h, z + d);
        default:
    ...
    bool checkPoint(const PointType & v) {
        ...
        return b.x <= v.x && v.x < (b.x + b.w) && b.y <= v.y && v.y < (b.y + b.h) && b.z <= v.z && v.z < (b.z + b.d);
    }
    ///
...
public:
    ...
    ///
    explicit Sphere(const Box<T> & b): center(b.center()), radius((b.vmax - b.vmin).length() / (T) 2.0) {}
    //@}
...
///
GN_API void calculateBoundingSphere(Spheref & result, const float * x, size_t strideX, const float * y, size_t strideY, const float * z, size_t strideZ,
                                    size_t count);
...
///
GN_API void calculateBoundingBox(Boxf & result, const float * x, size_t strideX, const float * y, size_t strideY, const float * z, size_t strideZ,
                                 size_t count);
