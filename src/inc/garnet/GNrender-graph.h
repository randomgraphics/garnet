...
struct Artifact : public RefCounter {
    struct Identification {
        ...
        Identification(const Guid & type, const StrA & name)
        : type(type)
        , name(name)
        {}
...
struct Action : public Artifact {
    ...
    virtual DynaArray<const Parameter *> parameters() const = 0;
...
struct Task {
    struct Shard {
        ...
        DynaArray<uint64_t> dependencies; // dependencies of the shard.
    };
    ...
    /// global dependencies of the task.
    DynaArray<uint64_t> dependencies;
};

struct RenderGraph {
    static RenderGraph * create();

    /// Schedule a new task. Returns a unique task id. 0 is always invalid.
    /// multi-thread safe.
    virtual uint64_t schedule(const Task &) = 0;
