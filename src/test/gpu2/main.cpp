...
class DX12Triangle : public StdClass {
    ...
    void initPSO() {
        ...
        // create pso
        Gpu2::InputElement             ie[] = {{"POSITION", 0, img::PixelFormat::RGB_32_32_32_FLOAT(), 0, 0, false, 0}};
...
public:
    DX12Triangle(gpu2ex & g): _g(g) { clear(); }
    ...
    void render(gpu2ex & g) {
        ...
        g.cl->draw({_pso, GN_ARRAY_COUNT(vb), vb,
                    nullptr, // no ib
                    PrimitiveType::TRIANGLE_LIST, 3});
    }
};
...
int main() {
    ...
    while (mainWindow->runUntilNoNewEvents()) {
        ...
        if (gInput.getKeyStatus(KeyCode::ESCAPE).down || gInput.getKeyStatus(KeyCode::XB360_A).down) { break; }
