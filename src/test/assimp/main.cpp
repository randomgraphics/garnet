...
void get_bounding_box_for_node(const struct aiNode * nd, struct aiVector3D * min, struct aiVector3D * max, struct aiMatrix4x4 * trafo) {
    ...
    for (n = 0; n < nd->mNumChildren; ++n) { get_bounding_box_for_node(nd->mChildren[n], min, max, trafo); }
    *trafo = prev;
}
    ...
    for (; n < nd->mNumMeshes; ++n) {
        ...
        for (t = 0; t < mesh->mNumFaces; ++t) {
            ...
            for (i = 0; i < (int) face->mNumIndices; i++) {
                ...
                if (mesh->mColors[0] != NULL) Color4f(&mesh->mColors[0][index]);
...
int main(int argc, char ** argv) {
    ...
    if (0 != loadasset(argc >= 2 ? argv[1] : (const char *) GN::fs::toNativeDiskFilePath("media::model/R.F.R01/a01.ase"))) { return -1; }
