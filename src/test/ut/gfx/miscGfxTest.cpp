...
public:
    /// constructor
    GpuProgramResourceBinding(): mBindBy(NOTHING) {}

    /// clear current binding
    void clear() { mBindBy = NOTHING; }
    ...
    const char * getBindingName() const {
        GN_ASSERT(isBindByName());
        return mBindingName;
    }
    size_t getBindingIndex() const {
        GN_ASSERT(isBindByIndex());
        return mBindingIndex;
    }
    void bindTo(const char * name) { doBindByName(name); }
    void bindTo(size_t index) {
        mBindingIndex = index;
        mBindBy       = INDEX;
    }
    //@}
