...
public:
    ...
    void testReallocExpandsMemoryAndPreservesContentAndAlignment() {
        ...
        if (newPtr) {
            ...
            for (size_t i = 0; i < origSize; ++i) {
                TS_ASSERT_EQUALS(newPtr[i], (char)(42 + i));
            }
    ...
    void testReallocToZeroIsSameAsReallocTo1Byte() {
        // We realloc to 0 bytes first, which should be the same as realloc to 1 byte.
        void *ptr0 = GN::HeapMemory::realloc(nullptr, 0);

        // Then we realloc to 1 byte, which should be an no-op
        void *ptr1 = GN::HeapMemory::realloc(ptr0, 1);

        TS_ASSERT_EQUALS(ptr0, ptr1);
        
        GN::HeapMemory::dealloc(ptr0);
    }
};
