...
public:
    void testHashMapCrash() {
        ...
        wchar_t keys[] = {70,  80, 83, 58, 32, 48, 46, 51, 57, 40, 114, 101, 115, 49, 102,   111,   104, 108,
                          112, 41, 55, 50, 54, 53, 56, 69, 67, 47, 88,  66,  84,  78, 36864, 20986, 52};
    ...
    void testEmptyMapIteration() {
        ...
        bool isempty = true;
        for (StrHashMap::KeyValuePair * i = m.first(); i != NULL; i = m.next(i)) { isempty = false; }
        TS_ASSERT(isempty);
    }
    ...
    void testNonEmptyMapIteration() {
        ...
        int count = 0;
        for (const StrHashMap::KeyValuePair * i = m.first(); i != NULL; i = m.next(i)) { ++count; }
        TS_ASSERT(1 == count);
