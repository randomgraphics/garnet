...
public:
    void testCtor() {
        {
            GN::Variant var(true);
            bool        v = false;
            TS_ASSERT(var.getb(v));
            TS_ASSERT_EQUALS(true, v);
        }
        {
            GN::Variant var(1);
            int         v = 0;
            TS_ASSERT(var.geti(v));
            TS_ASSERT_EQUALS(1, v);
        }
        {
            GN::Variant var(1.0f);
            float       v = 0.0f;
            TS_ASSERT(var.getf(v));
            TS_ASSERT_EQUALS(1.0f, v);
        }
        {
            GN::Variant var((void *) NULL);
            void *      v = &var; // initialize to any non-NULL value.
            TS_ASSERT(var.getp(v));
    ...
    void testFromBool() {
        ...
        s = var.gets();
        TS_ASSERT_EQUALS("1", s);
        TS_ASSERT(var.getb(b));
        TS_ASSERT_EQUALS(true, b);
        TS_ASSERT(var.geti(i));
        TS_ASSERT_EQUALS(1, i);
        TS_ASSERT(var.getf(f));
        TS_ASSERT_EQUALS(1.0f, f);
        TS_ASSERT(var.getp(p));
        TS_ASSERT_EQUALS((void *) 1, p);
        TS_ASSERT(!var.getv(v));
        TS_ASSERT(!var.getm(m));
        ...
        TS_ASSERT(var.getf(f));
        TS_ASSERT_EQUALS(0.0f, f);
        TS_ASSERT(var.getp(p));
        TS_ASSERT_EQUALS((void *) 0, p);
        TS_ASSERT(!var.getv(v));
        TS_ASSERT(!var.getm(m));
    }
