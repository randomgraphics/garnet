...
struct SType : public ID3DX11EffectType {
    ...
    SType(): VarType(EVT_Invalid), Elements(0), pTypeName(NULL), TotalSize(0), Stride(0), PackedSize(0) {
    ...
    BOOL IsObjectType(EObjectType ObjType) const { return IsObjectTypeHelper(VarType, ObjectType, ObjType); }
    BOOL IsShader() const { return IsShaderHelper(VarType, ObjectType); }
    BOOL BelongsInConstantBuffer() const { return (VarType == EVT_Numeric) || (VarType == EVT_Struct); }
    BOOL IsStateBlockObject() const { return IsStateBlockObjectHelper(VarType, ObjectType); }
    BOOL IsClassInstance() const { return (VarType == EVT_Struct) && StructType.ImplementsInterface; }
    BOOL IsInterface() const { return IsInterfaceHelper(VarType, ObjectType); }
    BOOL IsShaderResource() const { return IsShaderResourceHelper(VarType, ObjectType); }
    BOOL IsUnorderedAccessView() const { return IsUnorderedAccessViewHelper(VarType, ObjectType); }
    BOOL IsSampler() const { return IsSamplerHelper(VarType, ObjectType); }
    BOOL IsRenderTargetView() const { return IsRenderTargetViewHelper(VarType, ObjectType); }
    BOOL IsDepthStencilView() const { return IsDepthStencilViewHelper(VarType, ObjectType); }
...
struct SInterface {
    ...
    SInterface() { pClassInstance = NULL; }
};
...
template<class T, class D3DTYPE>
struct SShaderDependency {
    UINT StartIndex;
    UINT Count;
...
struct SPointerMapping {
    ...
    static BOOL AreMappingsEqual(const SPointerMapping & pMap1, const SPointerMapping & pMap2) { return (pMap1.pOld == pMap2.pOld); }
...
public:
    ...
    BOOL IsInHeap(void * pData) const { return (pData >= m_pData && pData < (m_pData + m_dwBufferSize)); }
...
public:
    ...
    ID3DX11EffectVariable * CreatePooledVariableMemberInterface(TTopLevelVariable<ID3DX11EffectVariable> * pTopLevelEntity, SVariable * pMember,
