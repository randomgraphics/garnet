...
public:
    bool onCreateDevice() {
        ...
        // compile effect
        AutoComPtr<ID3DBlob> binary = AutoComPtr<ID3DBlob>::sAttach(compileShader("fx_5_0", fxcode, str::length(fxcode), D3DCOMPILE_PACK_MATRIX_ROW_MAJOR,
                                                                                  NULL)); // entry
        if (nullptr == binary) return false;

        // create effect
        GN_RETURN_FALSE_ON_HR_FAILED(
            D3DX11CreateEffectFromMemory(binary->GetBufferPointer(), binary->GetBufferSize(), D3DCOMPILE_PACK_MATRIX_ROW_MAJOR, &dev, &effect));
