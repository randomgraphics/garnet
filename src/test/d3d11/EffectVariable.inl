...
enum ETemplateVarType { ETVT_Bool, ETVT_Int, ETVT_Float };
...
#define VERIFYPARAMETER(x)                                       \
    {                                                            \
        if (!(x)) {                                              \
            DPF(0, "%s: Parameter " #x " was NULL.", pFuncName); \
            __BREAK_ON_FAIL;                                     \
            hr = E_INVALIDARG;                                   \
            goto lExit;                                          \
        }                                                        \
...
BOOL GetVariableByIndexHelper(UINT Index, UINT VariableCount, SVarType * pVariables, BYTE * pBaseAddress, SVarType ** ppMember, void ** ppDataPtr) {
...
template<typename SVarType>
BOOL GetVariableByNameHelper(LPCSTR Name, UINT VariableCount, SVarType * pVariables, BYTE * pBaseAddress, SVarType ** ppMember, void ** ppDataPtr,
                             UINT * pIndex) {
    ...
    for (i = 0; i < VariableCount; ++i) {
        *ppMember = pVariables + i;
        ...
        if (strcmp((*ppMember)->pName, Name) == 0) {
            ...
            *pIndex    = i;
            return TRUE;
        } else if (i == 0 && (*ppMember)->pName[0] == '$' && strcmp((*ppMember)->pName, "$super") == 0) {
            bHasSuper = true;
        }
    }
    ...
    if (bHasSuper) {
        ...
        return GetVariableByNameHelper<SVarType>(Name, pSuper->pType->StructType.Members, (SVarType *) pSuper->pType->StructType.pMembers,
                                                 pBaseAddress + pSuper->Data.Offset, ppMember, ppDataPtr, pIndex);
    }
...
template<typename SVarType>
BOOL GetVariableBySemanticHelper(LPCSTR Semantic, UINT VariableCount, SVarType * pVariables, BYTE * pBaseAddress, SVarType ** ppMember, void ** ppDataPtr,
                                 UINT * pIndex) {
    ...
    for (i = 0; i < VariableCount; ++i) {
        *ppMember = pVariables + i;
        if (NULL != (*ppMember)->pSemantic && _stricmp((*ppMember)->pSemantic, Semantic) == 0) {
            *ppDataPtr = pBaseAddress + (*ppMember)->Data.Offset;
            *pIndex    = i;
            return TRUE;
        }
...
D3DX11INLINE BOOL AreBoundsValid(UINT Offset, UINT Count, CONST void * pData, CONST SType * pType, UINT TotalUnpackedSize) {
    ...
    return ((Offset + Count >= Offset) && ((Offset + Count) < ((UINT) -1) / pType->Stride) && (Count * pType->Stride + (BYTE *) pData >= (BYTE *) pData) &&
...
__forceinline HRESULT CopyScalarValue(SRC_TYPE SrcValue, void * pDest, const char * pFuncName) {
    HRESULT hr = S_OK;
#ifdef _DEBUG
    if (ValidatePtr) VERIFYPARAMETER(pDest);
#endif
...
D3DX11INLINE HRESULT SetScalarArray(CONST SRC_TYPE * pSrcValues, DEST_TYPE * pDestValues, UINT Offset, UINT Count, SType * pType, UINT TotalUnpackedSize,
...
template<ETemplateVarType SourceType, ETemplateVarType DestType, typename SRC_TYPE, typename DEST_TYPE>
D3DX11INLINE HRESULT GetScalarArray(SRC_TYPE * pSrcValues, DEST_TYPE * pDestValues, UINT Offset, UINT Count, SType * pType, UINT TotalUnpackedSize,
                                    const char * pFuncName) {
    HRESULT hr = S_OK;
...
struct TVariable : public IBaseInterface {
    ...
    STDMETHOD_(ID3DX11EffectVariable *, GetMemberByIndex)(UINT Index) {
        ...
        if (!GetVariableByIndexHelper<SVariable>(Index, pType->StructType.Members, pType->StructType.pMembers, Data.pNumeric, &pMember, &dataPtr.pGeneric)) {
            return &g_InvalidScalarVariable;
        }
    ...
    STDMETHOD_(ID3DX11EffectVariable *, GetMemberByName)(LPCSTR Name) {
        ...
        if (!GetVariableByNameHelper<SVariable>(Name, pType->StructType.Members, pType->StructType.pMembers, Data.pNumeric, &pMember, &dataPtr.pGeneric,
                                                &index)) {
            return &g_InvalidScalarVariable;
        }
    ...
    STDMETHOD_(ID3DX11EffectVariable *, GetMemberBySemantic)(LPCSTR Semantic) {
        ...
        if (!GetVariableBySemanticHelper<SVariable>(Semantic, pType->StructType.Members, pType->StructType.pMembers, Data.pNumeric, &pMember, &dataPtr.pGeneric,
                                                    &index)) {
            return &g_InvalidScalarVariable;
        }
    ...
    STDMETHOD_(ID3DX11EffectScalarVariable *, AsScalar)() {
        ...
        if (pType->VarType != EVT_Numeric || pType->NumericType.NumericLayout != ENL_Scalar) {
    ...
    STDMETHOD_(ID3DX11EffectVectorVariable *, AsVector)() {
        ...
        if (pType->VarType != EVT_Numeric || pType->NumericType.NumericLayout != ENL_Vector) {
            DPF(0, "%s: Invalid typecast", pFuncName);
    ...
    STDMETHOD_(ID3DX11EffectMatrixVariable *, AsMatrix)() {
        ...
        if (pType->VarType != EVT_Numeric || pType->NumericType.NumericLayout != ENL_Matrix) {
            DPF(0, "%s: Invalid typecast", pFuncName);
    ...
    STDMETHOD_(ID3DX11EffectClassInstanceVariable *, AsClassInstance)() {
        ...
        if (!pType->IsClassInstance()) {
            ...
            DPF(0,
                "%s: Non-global class instance variables (members of structs or classes) and class instances "
                "inside tbuffers are not supported.",
                pFuncName);
            return &g_InvalidClassInstanceVariable;
        }
...
struct TTopLevelVariable : public SVariable, public IBaseInterface {
    ...
    TTopLevelVariable() { pEffect = NULL; }

    UINT GetTotalUnpackedSize() { return ((SType *) pType)->GetTotalUnpackedSize(FALSE); }

    STDMETHOD_(ID3DX11EffectType *, GetType)() { return (ID3DX11EffectType *) (SType *) pType; }

    TTopLevelVariable<ID3DX11EffectVariable> * GetTopLevelEntity() { return (TTopLevelVariable<ID3DX11EffectVariable> *) this; }

    BOOL IsArray() { return (pType->Elements > 0); }
};
...
struct TMember : public SVariable, public IBaseInterface {
    ...
    CEffect * GetEffect() { return pTopLevelEntity->pEffect; }

    UINT GetTotalUnpackedSize() { return pType->GetTotalUnpackedSize(IsSingleElement); }
    ...
    TTopLevelVariable<ID3DX11EffectVariable> * GetTopLevelEntity() { return pTopLevelEntity; }

    BOOL IsArray() { return (pType->Elements > 0 && !IsSingleElement); }

    STDMETHOD_(ID3DX11EffectVariable *, GetAnnotationByIndex)(UINT Index) { return pTopLevelEntity->GetAnnotationByIndex(Index); }
    STDMETHOD_(ID3DX11EffectVariable *, GetAnnotationByName)(LPCSTR Name) { return pTopLevelEntity->GetAnnotationByName(Name); }

    STDMETHOD_(ID3DX11EffectConstantBuffer *, GetParentConstantBuffer)() { return pTopLevelEntity->GetParentConstantBuffer(); }
...
struct TAnnotation : public TVariable<TTopLevelVariable<IBaseInterface>> {
    ...
    STDMETHOD_(ID3DX11EffectConstantBuffer *, GetParentConstantBuffer)() { return NoParentCB(); }

    void DirtyVariable() { D3DXASSERT(0); }
};
...
struct TNumericVariable : public IBaseInterface {
    STDMETHOD(SetRawValue)(CONST void * pData, UINT ByteOffset, UINT ByteCount) {
        if (IsAnnotation) {
            ...
            if ((ByteOffset + ByteCount < ByteOffset) || (ByteCount + (BYTE *) pData < (BYTE *) pData) || ((ByteOffset + ByteCount) > GetTotalUnpackedSize())) {
...
HRESULT TFloatScalarVariable<IBaseInterface, IsAnnotation>::SetFloatArray(CONST float * pData, UINT Offset, UINT Count) {
    ...
    if (IsAnnotation) return AnnotationInvalidSetCall(pFuncName);
    DirtyVariable();
    return SetScalarArray<ETVT_Float, ETVT_Float, float, float>(pData, Data.pNumericFloat, Offset, Count, pType, GetTotalUnpackedSize(), pFuncName);
}

template<typename IBaseInterface, BOOL IsAnnotation>
HRESULT TFloatScalarVariable<IBaseInterface, IsAnnotation>::GetFloatArray(float * pData, UINT Offset, UINT Count) {
    return GetScalarArray<ETVT_Float, ETVT_Float, float, float>(Data.pNumericFloat, pData, Offset, Count, pType, GetTotalUnpackedSize(),
...
HRESULT TFloatScalarVariable<IBaseInterface, IsAnnotation>::SetIntArray(CONST int * pData, UINT Offset, UINT Count) {
    ...
    if (IsAnnotation) return AnnotationInvalidSetCall(pFuncName);
    DirtyVariable();
    return SetScalarArray<ETVT_Int, ETVT_Float, int, float>(pData, Data.pNumericFloat, Offset, Count, pType, GetTotalUnpackedSize(), pFuncName);
}

template<typename IBaseInterface, BOOL IsAnnotation>
HRESULT TFloatScalarVariable<IBaseInterface, IsAnnotation>::GetIntArray(int * pData, UINT Offset, UINT Count) {
    return GetScalarArray<ETVT_Float, ETVT_Int, float, int>(Data.pNumericFloat, pData, Offset, Count, pType, GetTotalUnpackedSize(),
...
HRESULT TFloatScalarVariable<IBaseInterface, IsAnnotation>::SetBoolArray(CONST BOOL * pData, UINT Offset, UINT Count) {
    ...
    if (IsAnnotation) return AnnotationInvalidSetCall(pFuncName);
    DirtyVariable();
    return SetScalarArray<ETVT_Bool, ETVT_Float, BOOL, float>(pData, Data.pNumericFloat, Offset, Count, pType, GetTotalUnpackedSize(), pFuncName);
}

template<typename IBaseInterface, BOOL IsAnnotation>
HRESULT TFloatScalarVariable<IBaseInterface, IsAnnotation>::GetBoolArray(BOOL * pData, UINT Offset, UINT Count) {
    return GetScalarArray<ETVT_Float, ETVT_Bool, float, BOOL>(Data.pNumericFloat, pData, Offset, Count, pType, GetTotalUnpackedSize(),
...
HRESULT TIntScalarVariable<IBaseInterface, IsAnnotation>::SetFloatArray(CONST float * pData, UINT Offset, UINT Count) {
    ...
    if (IsAnnotation) return AnnotationInvalidSetCall(pFuncName);
    DirtyVariable();
    return SetScalarArray<ETVT_Float, ETVT_Int, float, int>(pData, Data.pNumericInt, Offset, Count, pType, GetTotalUnpackedSize(), pFuncName);
}

template<typename IBaseInterface, BOOL IsAnnotation>
HRESULT TIntScalarVariable<IBaseInterface, IsAnnotation>::GetFloatArray(float * pData, UINT Offset, UINT Count) {
    return GetScalarArray<ETVT_Int, ETVT_Float, int, float>(Data.pNumericInt, pData, Offset, Count, pType, GetTotalUnpackedSize(),
                                                            "ID3DX11EffectScalarVariable::GetFloatArray");
}
...
HRESULT TIntScalarVariable<IBaseInterface, IsAnnotation>::SetIntArray(CONST int * pData, UINT Offset, UINT Count) {
    ...
    if (IsAnnotation) return AnnotationInvalidSetCall(pFuncName);
    DirtyVariable();
    return SetScalarArray<ETVT_Int, ETVT_Int, int, int>(pData, Data.pNumericInt, Offset, Count, pType, GetTotalUnpackedSize(), pFuncName);
}

template<typename IBaseInterface, BOOL IsAnnotation>
HRESULT TIntScalarVariable<IBaseInterface, IsAnnotation>::GetIntArray(int * pData, UINT Offset, UINT Count) {
    return GetScalarArray<ETVT_Int, ETVT_Int, int, int>(Data.pNumericInt, pData, Offset, Count, pType, GetTotalUnpackedSize(),
                                                        "ID3DX11EffectScalarVariable::GetIntArray");
}
...
HRESULT TIntScalarVariable<IBaseInterface, IsAnnotation>::SetBoolArray(CONST BOOL * pData, UINT Offset, UINT Count) {
    ...
    if (IsAnnotation) return AnnotationInvalidSetCall(pFuncName);
    DirtyVariable();
    return SetScalarArray<ETVT_Bool, ETVT_Int, BOOL, int>(pData, Data.pNumericInt, Offset, Count, pType, GetTotalUnpackedSize(), pFuncName);
}

template<typename IBaseInterface, BOOL IsAnnotation>
HRESULT TIntScalarVariable<IBaseInterface, IsAnnotation>::GetBoolArray(BOOL * pData, UINT Offset, UINT Count) {
    return GetScalarArray<ETVT_Int, ETVT_Bool, int, BOOL>(Data.pNumericInt, pData, Offset, Count, pType, GetTotalUnpackedSize(),
                                                          "ID3DX11EffectScalarVariable::GetBoolArray");
}
...
HRESULT TBoolScalarVariable<IBaseInterface, IsAnnotation>::SetFloatArray(CONST float * pData, UINT Offset, UINT Count) {
    ...
    if (IsAnnotation) return AnnotationInvalidSetCall(pFuncName);
    DirtyVariable();
    return SetScalarArray<ETVT_Float, ETVT_Bool, float, BOOL>(pData, Data.pNumericBool, Offset, Count, pType, GetTotalUnpackedSize(), pFuncName);
}

template<typename IBaseInterface, BOOL IsAnnotation>
HRESULT TBoolScalarVariable<IBaseInterface, IsAnnotation>::GetFloatArray(float * pData, UINT Offset, UINT Count) {
    return GetScalarArray<ETVT_Bool, ETVT_Float, BOOL, float>(Data.pNumericBool, pData, Offset, Count, pType, GetTotalUnpackedSize(),
                                                              "ID3DX11EffectScalarVariable::GetFloatArray");
}
...
HRESULT TBoolScalarVariable<IBaseInterface, IsAnnotation>::SetIntArray(CONST int * pData, UINT Offset, UINT Count) {
    ...
    if (IsAnnotation) return AnnotationInvalidSetCall(pFuncName);
    DirtyVariable();
    return SetScalarArray<ETVT_Int, ETVT_Bool, int, BOOL>(pData, Data.pNumericBool, Offset, Count, pType, GetTotalUnpackedSize(), pFuncName);
}

template<typename IBaseInterface, BOOL IsAnnotation>
HRESULT TBoolScalarVariable<IBaseInterface, IsAnnotation>::GetIntArray(int * pData, UINT Offset, UINT Count) {
    return GetScalarArray<ETVT_Bool, ETVT_Int, BOOL, int>(Data.pNumericBool, pData, Offset, Count, pType, GetTotalUnpackedSize(),
                                                          "ID3DX11EffectScalarVariable::GetIntArray");
}
...
HRESULT TBoolScalarVariable<IBaseInterface, IsAnnotation>::SetBoolArray(CONST BOOL * pData, UINT Offset, UINT Count) {
    ...
    if (IsAnnotation) return AnnotationInvalidSetCall(pFuncName);
    DirtyVariable();
    return SetScalarArray<ETVT_Bool, ETVT_Bool, BOOL, BOOL>(pData, Data.pNumericBool, Offset, Count, pType, GetTotalUnpackedSize(), pFuncName);
}

template<typename IBaseInterface, BOOL IsAnnotation>
HRESULT TBoolScalarVariable<IBaseInterface, IsAnnotation>::GetBoolArray(BOOL * pData, UINT Offset, UINT Count) {
    return GetScalarArray<ETVT_Bool, ETVT_Bool, BOOL, BOOL>(Data.pNumericBool, pData, Offset, Count, pType, GetTotalUnpackedSize(),
                                                            "ID3DX11EffectScalarVariable::GetBoolArray");
}
...
void __forceinline CopyDataWithTypeConversion(__out_bcount(vecCount * dstVecSize * sizeof(UINT)) void * pDest, CONST void * pSource, UINT dstVecSize,
                                              UINT srcVecSize, UINT elementCount, UINT vecCount) {
    UINT i, j;
    ...
    case ETVT_Bool:
        ...
        case ETVT_Int:
            for (j = 0; j < vecCount; j++) {
                for (i = 0; i < elementCount; i++) ((int *) pDest)[i] = ((BOOL *) pSource)[i] ? -1 : 0;
        ...
        case ETVT_Float:
            for (j = 0; j < vecCount; j++) {
                for (i = 0; i < elementCount; i++) ((float *) pDest)[i] = ((BOOL *) pSource)[i] ? -1.0f : 0.0f;
    ...
    case ETVT_Int:
        ...
        case ETVT_Bool:
            for (j = 0; j < vecCount; j++) {
                for (i = 0; i < elementCount; i++) ((int *) pDest)[i] = (((int *) pSource)[i] != 0) ? -1 : 0;
        ...
        case ETVT_Float:
            for (j = 0; j < vecCount; j++) {
                for (i = 0; i < elementCount; i++) ((float *) pDest)[i] = (float) (((int *) pSource)[i]);
    ...
    case ETVT_Float:
        ...
        case ETVT_Bool:
            for (j = 0; j < vecCount; j++) {
                for (i = 0; i < elementCount; i++) ((int *) pDest)[i] = (((float *) pSource)[i] != 0.0f) ? -1 : 0;
        ...
        case ETVT_Int:
            for (j = 0; j < vecCount; j++) {
                for (i = 0; i < elementCount; i++) ((int *) pDest)[i] = (int) ((float *) pSource)[i];
...
HRESULT TVectorVariable<IBaseInterface, IsAnnotation, BaseType>::SetFloatVectorArray(CONST float * pData, UINT Offset, UINT Count) {
    ...
    CopyDataWithTypeConversion<BaseType, ETVT_Float>(Data.pVector + Offset, pData, 4, pType->NumericType.Columns, pType->NumericType.Columns,
                                                     std::max(std::min((int) Count, (int) pType->Elements - (int) Offset), 0));
...
HRESULT TVectorVariable<IBaseInterface, IsAnnotation, BaseType>::GetFloatVectorArray(float * pData, UINT Offset, UINT Count) {
    ...
    CopyDataWithTypeConversion<ETVT_Float, BaseType>(pData, Data.pVector + Offset, pType->NumericType.Columns, 4, pType->NumericType.Columns,
                                                     std::max(std::min((int) Count, (int) pType->Elements - (int) Offset), 0));
...
HRESULT TVectorVariable<IBaseInterface, IsAnnotation, BaseType>::SetIntVectorArray(CONST int * pData, UINT Offset, UINT Count) {
    ...
    DirtyVariable();
    // ensure we don't write over the padding at the end of the vector array
    CopyDataWithTypeConversion<BaseType, ETVT_Int>(Data.pVector + Offset, pData, 4, pType->NumericType.Columns, pType->NumericType.Columns,
                                                   std::max(std::min((int) Count, (int) pType->Elements - (int) Offset), 0));
...
HRESULT TVectorVariable<IBaseInterface, IsAnnotation, BaseType>::GetIntVectorArray(int * pData, UINT Offset, UINT Count) {
    ...
    // ensure we don't read past the end of the vector array
    CopyDataWithTypeConversion<ETVT_Int, BaseType>(pData, Data.pVector + Offset, pType->NumericType.Columns, 4, pType->NumericType.Columns,
                                                   std::max(std::min((int) Count, (int) pType->Elements - (int) Offset), 0));
...
HRESULT TVectorVariable<IBaseInterface, IsAnnotation, BaseType>::SetBoolVectorArray(CONST BOOL * pData, UINT Offset, UINT Count) {
    ...
    DirtyVariable();
    // ensure we don't write over the padding at the end of the vector array
    CopyDataWithTypeConversion<BaseType, ETVT_Bool>(Data.pVector + Offset, pData, 4, pType->NumericType.Columns, pType->NumericType.Columns,
                                                    std::max(std::min((int) Count, (int) pType->Elements - (int) Offset), 0));
...
HRESULT TVectorVariable<IBaseInterface, IsAnnotation, BaseType>::GetBoolVectorArray(BOOL * pData, UINT Offset, UINT Count) {
    ...
    // ensure we don't read past the end of the vector array
    CopyDataWithTypeConversion<ETVT_Bool, BaseType>(pData, Data.pVector + Offset, pType->NumericType.Columns, 4, pType->NumericType.Columns,
                                                    std::max(std::min((int) Count, (int) pType->Elements - (int) Offset), 0));
...
static void GetMatrixTransposeHelper(SType * pType, __in_bcount(64) BYTE * pSrcData, __out_ecount(16) float * pMatrix) {
    ...
    for (i = 0; i < registers; ++i) {
        for (j = 0; j < entries; ++j) { ((float *) pMatrix)[j * 4 + i] = ((float *) pSrcData)[j]; }
...
HRESULT TMatrixVariable<IBaseInterface, IsAnnotation>::SetMatrix(CONST float * pData) {
    ...
    if (IsAnnotation) return AnnotationInvalidSetCall(pFuncName);
    DirtyVariable();
    return DoMatrixArrayInternal<FALSE, TRUE, FALSE>(pType, GetTotalUnpackedSize(), Data.pNumeric, const_cast<float *>(pData), 0, 1, pFuncName);
}

template<typename IBaseInterface, BOOL IsAnnotation>
HRESULT TMatrixVariable<IBaseInterface, IsAnnotation>::GetMatrix(float * pData) {
    return DoMatrixArrayInternal<FALSE, FALSE, FALSE>(pType, GetTotalUnpackedSize(), Data.pNumeric, pData, 0, 1, "ID3DX11EffectMatrixVariable::GetMatrix");
}
...
HRESULT TMatrixVariable<IBaseInterface, IsAnnotation>::SetMatrixArray(CONST float * pData, UINT Offset, UINT Count) {
    ...
    if (IsAnnotation) return AnnotationInvalidSetCall(pFuncName);
    DirtyVariable();
    return DoMatrixArrayInternal<FALSE, TRUE, FALSE>(pType, GetTotalUnpackedSize(), Data.pNumeric, const_cast<float *>(pData), Offset, Count,
...
HRESULT TMatrixVariable<IBaseInterface, IsAnnotation>::GetMatrixArray(float * pData, UINT Offset, UINT Count) {
    return DoMatrixArrayInternal<FALSE, FALSE, FALSE>(pType, GetTotalUnpackedSize(), Data.pNumeric, pData, Offset, Count,
...
HRESULT TMatrixVariable<IBaseInterface, IsAnnotation>::SetMatrixPointerArray(CONST float ** ppData, UINT Offset, UINT Count) {
    ...
    if (IsAnnotation) return AnnotationInvalidSetCall(pFuncName);
    DirtyVariable();
    return DoMatrixArrayInternal<FALSE, TRUE, TRUE>(pType, GetTotalUnpackedSize(), Data.pNumeric, const_cast<float **>(ppData), Offset, Count,
                                                    "ID3DX11EffectMatrixVariable::SetMatrixPointerArray");
}

template<typename IBaseInterface, BOOL IsAnnotation>
HRESULT TMatrixVariable<IBaseInterface, IsAnnotation>::GetMatrixPointerArray(float ** ppData, UINT Offset, UINT Count) {
    return DoMatrixArrayInternal<FALSE, FALSE, TRUE>(pType, GetTotalUnpackedSize(), Data.pNumeric, ppData, Offset, Count,
                                                     "ID3DX11EffectMatrixVariable::GetMatrixPointerArray");
}
...
HRESULT TMatrixVariable<IBaseInterface, IsAnnotation>::SetMatrixTranspose(CONST float * pData) {
    ...
    if (IsAnnotation) return AnnotationInvalidSetCall(pFuncName);
    DirtyVariable();
    return DoMatrixArrayInternal<TRUE, TRUE, FALSE>(pType, GetTotalUnpackedSize(), Data.pNumeric, const_cast<float *>(pData), 0, 1,
...
HRESULT TMatrixVariable<IBaseInterface, IsAnnotation>::GetMatrixTranspose(float * pData) {
    return DoMatrixArrayInternal<TRUE, FALSE, FALSE>(pType, GetTotalUnpackedSize(), Data.pNumeric, pData, 0, 1,
...
HRESULT TMatrixVariable<IBaseInterface, IsAnnotation>::SetMatrixTransposeArray(CONST float * pData, UINT Offset, UINT Count) {
    ...
    if (IsAnnotation) return AnnotationInvalidSetCall(pFuncName);
    DirtyVariable();
    return DoMatrixArrayInternal<TRUE, TRUE, FALSE>(pType, GetTotalUnpackedSize(), Data.pNumeric, const_cast<float *>(pData), Offset, Count,
...
HRESULT TMatrixVariable<IBaseInterface, IsAnnotation>::GetMatrixTransposeArray(float * pData, UINT Offset, UINT Count) {
    return DoMatrixArrayInternal<TRUE, FALSE, FALSE>(pType, GetTotalUnpackedSize(), Data.pNumeric, pData, Offset, Count,
...
HRESULT TMatrixVariable<IBaseInterface, IsAnnotation>::SetMatrixTransposePointerArray(CONST float ** ppData, UINT Offset, UINT Count) {
    ...
    if (IsAnnotation) return AnnotationInvalidSetCall(pFuncName);
    DirtyVariable();
    return DoMatrixArrayInternal<TRUE, TRUE, TRUE>(pType, GetTotalUnpackedSize(), Data.pNumeric, const_cast<float **>(ppData), Offset, Count,
                                                   "ID3DX11EffectMatrixVariable::SetMatrixTransposePointerArray");
}

template<typename IBaseInterface, BOOL IsAnnotation>
HRESULT TMatrixVariable<IBaseInterface, IsAnnotation>::GetMatrixTransposePointerArray(float ** ppData, UINT Offset, UINT Count) {
    return DoMatrixArrayInternal<TRUE, FALSE, TRUE>(pType, GetTotalUnpackedSize(), Data.pNumeric, ppData, Offset, Count,
                                                    "ID3DX11EffectMatrixVariable::GetMatrixTransposePointerArray");
}
...
HRESULT TMatrix4x4Variable<IBaseInterface, IsColumnMajor>::GetMatrixTransposeArray(float * pData, UINT Offset, UINT Count) {
    ...
    #define CHECK_OBJECT_ARRAY_BOUNDS(Offset, Count, Pointer) HRESULT hr = S_OK;

    #define CHECK_OBJECT_SCALAR_BOUNDS(Index, Pointer) HRESULT hr = S_OK;

    #define CHECK_SCALAR_BOUNDS(Index) HRESULT hr = S_OK;
...
HRESULT TStringVariable<IBaseInterface, IsAnnotation>::GetStringArray(__out_ecount(Count) LPCSTR * ppStrings, UINT Offset, UINT Count) {
    ...
    UINT i;
    for (i = 0; i < Count; ++i) { ppStrings[i] = (Data.pString + Offset + i)->pString; }
...
static HRESULT ValidateTextureType(ID3D11ShaderResourceView * pView, EObjectType ObjectType, LPCSTR pFuncName) {
    if (NULL != pView) {
        ...
        switch (ObjectType) {
        case EOT_Texture:
            if (desc.ViewDimension != D3D11_SRV_DIMENSION_BUFFER && desc.ViewDimension != D3D11_SRV_DIMENSION_BUFFEREX) return S_OK;
            break;
        case EOT_Buffer:
            if (desc.ViewDimension != D3D11_SRV_DIMENSION_BUFFER && desc.ViewDimension != D3D11_SRV_DIMENSION_BUFFEREX) break;
        ...
        case EOT_Texture1D:
        case EOT_Texture1DArray:
            if (desc.ViewDimension == D3D11_SRV_DIMENSION_TEXTURE1D || desc.ViewDimension == D3D11_SRV_DIMENSION_TEXTURE1DARRAY) return S_OK;
            break;
        case EOT_Texture2D:
        case EOT_Texture2DArray:
            if (desc.ViewDimension == D3D11_SRV_DIMENSION_TEXTURE2D || desc.ViewDimension == D3D11_SRV_DIMENSION_TEXTURE2DARRAY) return S_OK;
            break;
        case EOT_Texture2DMS:
        case EOT_Texture2DMSArray:
            if (desc.ViewDimension == D3D11_SRV_DIMENSION_TEXTURE2DMS || desc.ViewDimension == D3D11_SRV_DIMENSION_TEXTURE2DMSARRAY) return S_OK;
            break;
        case EOT_Texture3D:
            if (desc.ViewDimension == D3D11_SRV_DIMENSION_TEXTURE3D) return S_OK;
            break;
        case EOT_TextureCube:
        case EOT_TextureCubeArray:
            if (desc.ViewDimension == D3D11_SRV_DIMENSION_TEXTURECUBE || desc.ViewDimension == D3D11_SRV_DIMENSION_TEXTURECUBEARRAY) return S_OK;
            break;
        case EOT_ByteAddressBuffer:
            if (desc.ViewDimension == D3D11_SRV_DIMENSION_BUFFEREX && (desc.BufferEx.Flags & D3D11_BUFFEREX_SRV_FLAG_RAW)) return S_OK;
            break;
        case EOT_StructuredBuffer:
        ...
        DPF(0, "%s: Resource type mismatch; %s expected, %s provided.", pFuncName, GetTextureTypeNameFromEnum(ObjectType),
            GetSRVDimensionNameFromEnum(desc.ViewDimension));
        return E_INVALIDARG;
    }
    return S_OK;
}
...
#ifdef _DEBUG
    for (i = 0; i < Count; ++i) { VH(ValidateTextureType(ppResources[i], pType->ObjectType, pFuncName)); }
#endif
...
static HRESULT ValidateTextureType(ID3D11UnorderedAccessView * pView, EObjectType ObjectType, LPCSTR pFuncName) {
    if (NULL != pView) {
        ...
        switch (ObjectType) {
        case EOT_RWBuffer:
            if (desc.ViewDimension != D3D11_UAV_DIMENSION_BUFFER) break;
        ...
        case EOT_RWTexture1D:
        case EOT_RWTexture1DArray:
            if (desc.ViewDimension == D3D11_UAV_DIMENSION_TEXTURE1D || desc.ViewDimension == D3D11_UAV_DIMENSION_TEXTURE1DARRAY) return S_OK;
            break;
        case EOT_RWTexture2D:
        case EOT_RWTexture2DArray:
            if (desc.ViewDimension == D3D11_UAV_DIMENSION_TEXTURE2D || desc.ViewDimension == D3D11_UAV_DIMENSION_TEXTURE2DARRAY) return S_OK;
            break;
        case EOT_RWTexture3D:
            if (desc.ViewDimension == D3D11_UAV_DIMENSION_TEXTURE3D) return S_OK;
            break;
        case EOT_RWByteAddressBuffer:
            if (desc.ViewDimension == D3D11_UAV_DIMENSION_BUFFER && (desc.Buffer.Flags & D3D11_BUFFER_UAV_FLAG_RAW)) return S_OK;
            break;
        case EOT_RWStructuredBuffer:
        ...
        DPF(0, "%s: Resource type mismatch; %s expected, %s provided.", pFuncName, GetTextureTypeNameFromEnum(ObjectType),
            GetUAVDimensionNameFromEnum(desc.ViewDimension));
        return E_INVALIDARG;
    }
    return S_OK;
}
...
BOOL TShaderVariable<IBaseInterface>::IsValid() {
    ...
    BOOL valid       = TRUE;
    while (numElements > 0 && (valid = Data.pShader[numElements - 1].IsValid)) numElements--;
    return valid;
}
...
HRESULT TBlendVariable<IBaseInterface>::UndoSetBlendState(UINT Index) {
    ...
    if (!Data.pBlend[Index].IsUserManaged) { return S_FALSE; }
...
BOOL TBlendVariable<IBaseInterface>::IsValid() {
    UINT numElements = IsArray() ? pType->Elements : 1;
    BOOL valid       = TRUE;
    while (numElements > 0 && (valid = Data.pBlend[numElements - 1].IsValid)) numElements--;
    return valid;
}
...
HRESULT TDepthStencilVariable<IBaseInterface>::UndoSetDepthStencilState(UINT Index) {
    ...
    if (!Data.pDepthStencil[Index].IsUserManaged) { return S_FALSE; }
...
BOOL TDepthStencilVariable<IBaseInterface>::IsValid() {
    UINT numElements = IsArray() ? pType->Elements : 1;
    BOOL valid       = TRUE;
    while (numElements > 0 && (valid = Data.pDepthStencil[numElements - 1].IsValid)) numElements--;
    return valid;
}
...
HRESULT TRasterizerVariable<IBaseInterface>::UndoSetRasterizerState(UINT Index) {
    ...
    if (!Data.pRasterizer[Index].IsUserManaged) { return S_FALSE; }
...
BOOL TRasterizerVariable<IBaseInterface>::IsValid() {
    UINT numElements = IsArray() ? pType->Elements : 1;
    BOOL valid       = TRUE;
    while (numElements > 0 && (valid = Data.pRasterizer[numElements - 1].IsValid)) numElements--;
    return valid;
}
...
HRESULT TSamplerVariable<IBaseInterface>::UndoSetSampler(UINT Index) {
    ...
    if (!Data.pSampler[Index].IsUserManaged) { return S_FALSE; }
