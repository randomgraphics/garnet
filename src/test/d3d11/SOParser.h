...
public:
    ...
    ~CSOParser() {
        for (UINT Stream = 0; Stream < D3D11_SO_STREAM_COUNT; Stream++) { SAFE_DELETE_ARRAY(m_SemanticString[Stream]); }
    }
    ...
    D3D11_SO_DECLARATION_ENTRY * GetDeclArray() { return &m_vDecls[0]; }

    char * GetErrorString() { return m_pError; }

    UINT GetDeclCount() const { return m_vDecls.GetSize(); }
    ...
    void GetStrides(UINT strides[4]) {
        ...
        for (UINT i = 0; i < len; i++) { strides[m_vDecls[i].OutputSlot] += m_vDecls[i].ComponentCount * sizeof(float); }
    }
...
protected:
    ...
    HRESULT Parse(UINT Stream, __in_z LPCSTR pString) {
        ...
        while (TRUE) {
            ...
            while (isspace((unsigned char) *pSemantic)) pSemantic++;
    ...
    HRESULT AddSemantic(__inout_z LPSTR pSemantic) {
        ...
        if (strcmp("$SKIP", pSemantic) != 0) { m_newEntry.SemanticName = pSemantic; }
    ...
    HRESULT ConsumeOutputSlot(__deref_inout_z LPSTR * ppSemantic) {
        ...
        if (pColon == NULL) return S_OK;

        if (pColon == *ppSemantic) {
            StringCchCopyA(m_pError, MAX_ERROR_SIZE, "ID3D11Effect::ParseSODecl - Invalid output slot");
            VH(E_FAIL);
        }
        ...
        *pColon        = '\0';
        ...
        while (isspace((unsigned char) **ppSemantic)) (*ppSemantic)++;
    ...
    HRESULT ConsumeSemanticIndex(__inout_z LPSTR pSemantic) {
        ...
        // Grab semantic index
        while (uLen > 0 && isdigit((unsigned char) pSemantic[uLen - 1])) uLen--;
