...
D3DX11INLINE void dwordMemcpy(__out_bcount(uByteCount) void * __restrict pDest, __in_bcount(uByteCount) CONST void * __restrict pSource, UINT uByteCount) {
    ...
    if (uByteCount & 0x4) { *((UINT *) dst64) = *((UINT *) src64); }
#else
...
protected:
    ...
    HRESULT Grow() { return Reserve(m_CurSize + 1); }

    HRESULT Reserve(UINT DesiredSize) {
        if (DesiredSize > m_MaxSize) {
            BYTE * pNewData;
            UINT   newSize = std::max<UINT>(m_MaxSize * 2, DesiredSize);
...
public:
    ...
    HRESULT CopyFrom(CEffectVector<T> & vOther) {
        ...
        UINT i;
        for (i = 0; i < m_CurSize; ++i) { ((T *) m_pData)[i] = ((T *) vOther.m_pData)[i]; }
    ...
    void Empty() {
        ...
        for (i = 0; i < m_CurSize; ++i) { ((T *) m_pData + i)->~T(); }
        m_CurSize    = 0;
        m_hLastError = S_OK;
    }

    T * Add() {
        if (FAILED(Grow())) return NULL;
    ...
    T * AddRange(UINT count) {
        ...
        if (FAILED(Reserve(m_CurSize + count))) return NULL;

        T *  pData = (T *) m_pData + m_CurSize;
        UINT i;
        for (i = 0; i < count; ++i) { new (pData + i) T; }
        m_CurSize += count;
        return pData;
    }

    HRESULT Add(const T & var) {
        if (FAILED(Grow())) return m_hLastError;
    ...
    HRESULT AddRange(const T * pVar, UINT count) {
        ...
        if (FAILED(Reserve(m_CurSize + count))) return m_hLastError;
    ...
    inline UINT GetSize() const { return m_CurSize; }

    inline T * GetData() const { return (T *) m_pData; }

    UINT FindIndexOf(void * pEntry) const {
        UINT i;

        for (i = 0; i < m_CurSize; ++i) {
            if (((T *) m_pData + i) == pEntry) return i;
        }

        return -1;
    }

    void Sort(int(__cdecl * pfnCompare)(const void * pElem1, const void * pElem2)) { qsort(m_pData, m_CurSize, sizeof(T), pfnCompare); }
};
...
public:
    ~CEffectVectorOwner<T>() {
        ...
        for (i = 0; i < m_CurSize; i++) SAFE_DELETE(((T **) m_pData)[i]);
...
public:
    ...
    CheckedNumber<T, MaxValue> & operator+=(const CheckedNumber<T, MaxValue> & other) {
        if (!other.m_bValid) {
            ...
            if (m_Value < other.m_Value) m_bValid = FALSE;
        }
    ...
    CheckedNumber<T, MaxValue> & operator*=(const CheckedNumber<T, MaxValue> & other) {
        if (!other.m_bValid) {
            ...
            if (other.m_Value != 0) {
                if (m_Value > MaxValue / other.m_Value) { m_bValid = FALSE; }
            }
            m_Value *= other.m_Value;
        }
...
static UINT ComputeHashLower(BYTE * pb, UINT cbToHash) {
    ...
    while (cbLeft >= 12) {
        BYTE pbT[12];
        for (UINT i = 0; i < 12; i++) pbT[i] = (BYTE) tolower(pb[i]);
    ...
    BYTE pbT[12];
    for (UINT i = 0; i < cbLeft; i++) pbT[i] = (BYTE) tolower(pb[i]);
...
static UINT ComputeHash(LPCSTR pString) { return ComputeHash((BYTE *) pString, (UINT) strlen(pString)); }
...
public:
    ...
    static UINT GetNextHashTableSize(__in UINT DesiredSize) {
        ...
        for (UINT i = 0; i < c_NumPrimes; ++i) {
            if (c_PrimeSizes[i] >= DesiredSize) { return c_PrimeSizes[i]; }
        }
    ...
    HRESULT Grow(__in UINT DesiredSize, __in UINT ProvidedArraySize = 0, __in_ecount_opt(ProvidedArraySize) void ** ProvidedArray = NULL,
        ...
        if (ProvidedArray && ProvidedArraySize >= actualSize) {
    ...
    void GetNextEntry(CIterator * pIterator) {
        ...
        if (NULL != pIterator->pHashEntry) { return; }
...
public:
    CEffectHashTableWithPrivateHeap() { m_pPrivateHeap = NULL; }
    ...
    ~CEffectHashTableWithPrivateHeap() { Cleanup(); }
