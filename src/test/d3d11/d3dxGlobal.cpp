...
HRESULT CMemoryStream::ReadAtOffset(SIZE_T offset, SIZE_T size, void ** ppData) {
    if (offset >= m_cbData) return E_FAIL;
...
HRESULT CMemoryStream::Read(void ** ppData, SIZE_T size) {
    ...
    if (temp < m_readPtr || temp > m_cbData) return E_FAIL;
...
HRESULT CMemoryStream::Read(LPCSTR * ppString) {
    ...
    for (iChar = m_readPtr; m_pData[iChar]; iChar++) {
        if (iChar > m_cbData) return E_FAIL;
    }
...
SIZE_T CMemoryStream::GetPosition() { return m_readPtr; }
...
void CDataBlock::EnableAlignment() { m_IsAligned = TRUE; }
...
HRESULT CDataBlock::AddData(const void * pvNewData, UINT bufferSize, CDataBlock ** ppBlock) {
    ...
    bytesToCopy = std::min<UINT>(m_maxSize - m_size, bufferSize);
    ...
    *ppBlock = this;
    ...
    if (bufferSize != 0) {
        ...
        if (m_IsAligned) { m_pNext->EnableAlignment(); }
...
void CDataBlockStore::EnableAlignment() { m_IsAligned = TRUE; }
...
HRESULT CDataBlockStore::AddData(const void * pNewData, UINT bufferSize, UINT * pCurOffset) {
    ...
    if (bufferSize == 0) {
        if (pCurOffset) { *pCurOffset = 0; }
        goto lExit;
    }

    if (!m_pFirst) {
        VN(m_pFirst = NEW CDataBlock());
        if (m_IsAligned) { m_pFirst->EnableAlignment(); }
        m_pLast = m_pFirst;
    }

    if (pCurOffset) *pCurOffset = m_Size + m_Offset;
...
void * CDataBlockStore::Allocate(UINT bufferSize) {
    ...
    if (FAILED(UIntAdd(m_Size, bufferSize, &m_Size))) return NULL;

    pRetValue = m_pLast->Allocate(bufferSize, &m_pLast);
    if (!pRetValue) return NULL;

    return pRetValue;
}

UINT CDataBlockStore::GetSize() { return m_Size; }
