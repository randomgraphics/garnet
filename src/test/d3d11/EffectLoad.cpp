...
inline HRESULT VerifyPointer(UINT oBase, UINT dwSize, UINT dwMaxSize) {
    ...
    if (dwAdd < oBase || dwAdd > dwMaxSize) return E_FAIL;
    return S_OK;
}
...
CEffectHeap::~CEffectHeap() { SAFE_DELETE_ARRAY(m_pData); }

UINT CEffectHeap::GetSize() { return m_dwSize; }
...
HRESULT CEffectHeap::AddDataInternal(const void * pData, UINT dwSize, void ** ppPointer) {
    ...
    *ppPointer = m_pData + m_dwSize;
    ...
    if (bCopyData) { memcpy(*ppPointer, pData, dwSize); }
    m_dwSize = finalSize;

lExit:
    if (FAILED(hr)) *ppPointer = NULL;

    return hr;
}

HRESULT CEffectHeap::AddData(const void * pData, UINT dwSize, void ** ppPointer) { return AddDataInternal<true>(pData, dwSize, ppPointer); }
...
HRESULT CEffectHeap::MoveInterfaceParameters(UINT InterfaceCount, __in_ecount(1) SShaderBlock::SInterfaceParameter ** ppInterfaces) {
    ...
    if (*ppInterfaces == NULL) return S_OK;
    ...
    for (UINT i = 0; i < InterfaceCount; i++) { VH(MoveString(&pNewPointer[i].pName)); }
...
HRESULT CEffectLoader::GetInterfaceParametersAndAddToReflection(UINT InterfaceCount, UINT offset,
                                                                __out_ecount_full(1) SShaderBlock::SInterfaceParameter ** ppInterfaces) {
    HRESULT                       hr = S_OK;
...
HRESULT CEffectLoader::FixupShaderPointer(SShaderBlock ** ppShaderBlock) {
    HRESULT hr = S_OK;
    if (*ppShaderBlock != &g_NullVS && *ppShaderBlock != &g_NullGS && *ppShaderBlock != &g_NullPS && *ppShaderBlock != &g_NullHS &&
        *ppShaderBlock != &g_NullDS && *ppShaderBlock != &g_NullCS && *ppShaderBlock != NULL) {
    ...
    if (index < m_pEffect->m_VariableCount) {
        ...
        *ppVar = m_pEffect->m_pVariables + index;
        ...
        for (index = 0; index < Members; index++) {
            if ((ID3DX11EffectVariable *) (*m_pvOldMemberInterfaces)[(UINT) index] == (ID3DX11EffectVariable *) *ppVar) { break; }
        }
...
HRESULT CEffectLoader::LoadEffect(CEffect * pEffect, CONST void * pEffectBuffer, UINT cbEffectBuffer) {
    ...
    for (i = 0; i < m_pEffect->m_ShaderBlockCount; i++) { VH(BuildShaderBlock(&m_pEffect->m_pShaderBlocks[i])); }
    ...
    VBD(m_pEffect->m_VariableCount == (m_pHeader->Effect.cObjectVariables + m_pHeader->Effect.cNumericVariables + m_pHeader->cInterfaceVariables),
        "Internal loading error: mismatched variable count.");
    ...
    VBD(m_pEffect->m_InterfaceCount == m_pHeader->cInterfaceVariableElements + m_BackgroundInterfaces.GetSize(),
        "Internal loading error: mismatched interface count.");
...
HRESULT CEffectLoader::LoadTypeAndAddToPool(SType ** ppType, UINT dwOffset) {
    ...
    case EVT_Object:
        ...
        VN(pHashBuffer = m_HashBuffer.AddRange(sizeof(temporaryType.VarType) + sizeof(temporaryType.Elements) + sizeof(temporaryType.pTypeName) +
                                               sizeof(temporaryType.ObjectType)));
    ...
    case EVT_Numeric:
        ...
        VBD(temporaryType.NumericType.Rows >= 1 && temporaryType.NumericType.Rows <= 4 && temporaryType.NumericType.Columns >= 1 &&
                temporaryType.NumericType.Columns <= 4 && temporaryType.NumericType.NumericLayout != ENL_Invalid &&
                temporaryType.NumericType.NumericLayout < ENL_Count && temporaryType.NumericType.ScalarType > EST_Invalid &&
                temporaryType.NumericType.ScalarType < EST_Count,
        ...
        VN(pHashBuffer = m_HashBuffer.AddRange(sizeof(temporaryType.VarType) + sizeof(temporaryType.Elements) + sizeof(temporaryType.pTypeName) +
                                               sizeof(temporaryType.NumericType)));
        memcpy(pHashBuffer, &temporaryType.VarType, sizeof(temporaryType.VarType));
        pHashBuffer += sizeof(temporaryType.VarType);
        memcpy(pHashBuffer, &temporaryType.Elements, sizeof(temporaryType.Elements));
        pHashBuffer += sizeof(temporaryType.Elements);
    ...
    case EVT_Struct:
        ...
        for (iMember = 0; iMember < cMembers; iMember++) {
            ...
            VBD(psMember[iMember].Offset == totalSize || psMember[iMember].Offset == AlignToPowerOf2(totalSize, SType::c_RegisterSize),
        ...
        VN(pHashBuffer = m_HashBuffer.AddRange(sizeof(temporaryType.VarType) + sizeof(temporaryType.Elements) + sizeof(temporaryType.pTypeName) +
                                               sizeof(temporaryType.StructType.Members) + cMembers * sizeof(SVariable)));
...
UINT CEffectLoader::UnpackData(BYTE * pDestData, BYTE * pSrcData, UINT PackedDataSize, SType * pType, UINT * pBytesRead) {
    ...
    HRESULT hr             = S_OK;
    UINT    elementsToCopy = std::max<UINT>(pType->Elements, 1);
    ...
    case EVT_Struct:
        for (i = 0; i < elementsToCopy; ++i) {
            for (j = 0; j < pType->StructType.Members; ++j) {
                ...
                VH(UnpackData(pDestData + pType->StructType.pMembers[j].Data.Offset, pSrcData + bytesRead, PackedDataSize - bytesRead,
...
HRESULT CEffectLoader::LoadNumericVariable(SConstantBuffer * pParentCB) {
    ...
    if (pType->VarType == EVT_Struct && pType->StructType.ImplementsInterface && !pParentCB->IsTBuffer) {
        ...
        m_pEffect->m_MemberDataCount += std::max<UINT>(pType->Elements, 1);
    }
    ...
    VBD(pType->TotalSize + psVar->Offset <= pParentCB->Size && pType->TotalSize + psVar->Offset >= pType->TotalSize,
        "Invalid pEffectBuffer: variable does not fit in CB.");
...
HRESULT CEffectLoader::LoadCBs() {
    ...
    for (iCB = 0; iCB < m_pHeader->Effect.cCBs; iCB++) {
        ...
        for (iVar = 0; iVar < psCB->cVariables; iVar++) { VH(LoadNumericVariable(pCB)); }
    }
...
HRESULT CEffectLoader::LoadAssignments(UINT Assignments, SAssignment ** ppAssignments, BYTE * pBackingStore, UINT * pRTVAssignments, UINT * pFinalAssignments) {
    ...
    *pFinalAssignments = 0;
    if (pRTVAssignments) *pRTVAssignments = 0;
    ...
    for (i = 0; i < Assignments; ++i) {
        ...
        if (pAssignment->LhsType == ELHS_RenderTargetView) renderTargetViewAssns++;
        ...
        case ECAT_Constant: // e.g. LHS = 1; or LHS = NULL;
            ...
            VHD(m_msUnstructured.ReadAtOffset(psAssignments[i].oInitializer, sizeof(UINT), (void **) &pNumConstants),
                "Invalid pEffectBuffer: cannot read NumConstants.");
        ...
        case ECAT_VariableIndex: // e.g. LHS = myVar[numLights];
            ...
            VBD(pVarIndex->pType->VarType == EVT_Numeric &&
                    (pVarIndex->pType->NumericType.ScalarType == EST_Int || pVarIndex->pType->NumericType.ScalarType == EST_UInt),
        ...
        case ECAT_InlineShader5:
            ...
            *((void **) pLHS) = pShaderBlock;
            ...
            VHD(GetUnstructuredDataBlock(psInlineShader->oShader, &cbShaderBin, (void **) &pShaderBin),
                "Invalid pEffectBuffer: cannot read inline shader block.");
            ...
            if (cbShaderBin > 0) {
                ...
                pShaderBlock->pReflectionData->pStreamOutDecls[0]     = pShaderBlock->pReflectionData->pStreamOutDecls[1] =
                    pShaderBlock->pReflectionData->pStreamOutDecls[2] = pShaderBlock->pReflectionData->pStreamOutDecls[3] = NULL;
            ...
            if (psAssignments[i].AssignmentType == ECAT_InlineShader5) {
                ...
                VH(GetInterfaceParametersAndAddToReflection(psInlineShader5->cInterfaceBindings, psInlineShader5->oInterfaceBindings,
                                                            &pShaderBlock->pReflectionData->pInterfaceParameters));
            }
    ...
    *pFinalAssignments = finalAssignments;
    if (pRTVAssignments) *pRTVAssignments = renderTargetViewAssns;
    ...
    for (iBlock = 0; iBlock < cBlocks; iBlock++) {
        ...
        if (0 != psBlock->oDefaultValue) {
            ...
            for (UINT i = 0; i < elementsToRead; i++) {
                ...
                VHD(m_msUnstructured.ReadAtOffset(pInterfaceInit->oInstanceName, &pClassInstanceName),
                    "Invalid pEffectBuffer: cannot read interface initializer.");
...
HRESULT CEffectLoader::LoadGroups() {
    ...
    for (iGroup = 0; iGroup < m_pHeader->cGroups; iGroup++) {
        ...
        UINT iTechnique;
        for (iTechnique = 0; iTechnique < psGroup->cTechniques; iTechnique++) { VH(LoadTechnique(&pGroup->pTechniques[iTechnique])); }
...
HRESULT CEffectLoader::GrabShaderData(SShaderBlock * pShaderBlock) {
    ...
    for (i = 0; i < ShaderDesc.BoundResources; i++) {
                ...
                for (j = 0; j < size; ++j) {
                    if (NULL == pSampler[j].BackingStore.pTexture) {
                        ...
                        if (0 == pVariable->pType->Elements) {
                            DPF(0,
                                "%s: Sampler %s does not have a texture bound to it, even though the sampler is used in a DX9-style texture load instruction",
                                g_szEffectLoadArea, pName);
                        } else {
                            DPF(0,
                                "%s: Sampler %s[%d] does not have a texture bound to it, even though the sampler array is used in a DX9-style texture load instruction",
                                g_szEffectLoadArea, pName, j);
                        }
        ...
        case D3D11_SIT_BYTEADDRESS:
            if (isFX9TextureLoad) {
                ...
                // add the whole array
                for (j = 0; j < size; ++j) { VHD(pRange->vResources.Add(pShaderResource + j), "Internal loading error: cannot add SRV to range."); }
            }
            break;
        case D3D11_SIT_UAV_RWTYPED:
        case D3D11_SIT_UAV_RWSTRUCTURED:
        ...
        case D3D11_SIT_UAV_RWSTRUCTURED_WITH_COUNTER:
            // add the whole array
            for (j = 0; j < size; ++j) { VHD(pRange->vResources.Add(pUnorderedAccessView + j), "Internal loading error: cannot add UAV to range."); }
            break;
        case D3D10_SIT_SAMPLER:
            // add the whole array
            for (j = 0; j < size; ++j) { VHD(pRange->vResources.Add(pSampler + j), "Internal loading error: cannot add sampler to range."); }
            break;
        default:
            VHD(E_FAIL, "Internal loading error: unexpected shader dependency type.");
        }
    ...
    if (NumInterfaces > 0) {
        ...
        for (i = 0; i < ShaderDesc.ConstantBuffers; i++) {
            ...
            if (CBDesc.Type != D3D11_CT_INTERFACE_POINTERS) { continue; }
            ...
            for (UINT iVar = 0; iVar < CBDesc.Variables; iVar++) {
                ...
                if (bindPoint < pRange->last) {
                    ...
                    for (j = 0; j < size; ++j) { pRange->vResources[j + bindPoint] = pInterface + j; }
                } else {
                    ...
                    for (j = 0; j < size; ++j) { VHD(pRange->vResources.Add(pInterface + j), "Internal loading error: cannot at interface to range."); }
                    pRange->last = bindPoint + size;
                }
            }
    ...
    if (pShaderBlock->TBufferDepCount > 0) { memcpy(pShaderBlock->ppTbufDeps, &vTBuffers[0], pShaderBlock->TBufferDepCount * sizeof(SConstantBuffer *)); }
...
HRESULT CEffectLoader::BuildShaderBlock(SShaderBlock * pShaderBlock) {
    ...
    VHD(D3DReflect(pShaderBlock->pReflectionData->pBytecode, pShaderBlock->pReflectionData->BytecodeLength, IID_ID3D11ShaderReflection,
                   (void **) &pShaderBlock->pReflectionData->pReflection),
    ...
    if (EOT_VertexShader == pShaderBlock->GetShaderType()) {
        ...
        VHD(D3DGetInputSignatureBlob(pShaderBlock->pReflectionData->pBytecode, pShaderBlock->pReflectionData->BytecodeLength,
...
HRESULT CEffectLoader::ReallocateAnnotationData(UINT cAnnotations, SAnnotation ** ppAnnotations) {
    ...
    VHD(m_pReflection->m_Heap.MoveData((void **) ppAnnotations, cAnnotations * sizeof(SAnnotation)), "Internal loading error: cannot move annotation data.");
    ...
    for (i = 0; i < cAnnotations; i++) {
        ...
        if (pAn->pType->BelongsInConstantBuffer()) {
            ...
            VHD(m_pReflection->m_Heap.MoveData((void **) &pAn->Data.pString, cElements * sizeof(SString)),
                "Internal loading error: cannot move annotation string.");
            for (j = 0; j < cElements; ++j) {
...
HRESULT CEffectLoader::InitializeReflectionDataAndMoveStrings(UINT KnownSize) {
    ...
    if (KnownSize) {
        ...
        for (i = 0; i < m_pEffect->m_CBCount; i++) { VH(CalculateAnnotationSize(m_pEffect->m_pCBs[i].AnnotationCount, m_pEffect->m_pCBs[i].pAnnotations)); }
        ...
        for (i = 0; i < m_pEffect->m_GroupCount; i++) {
            ...
            for (j = 0; j < m_pEffect->m_pGroups[i].TechniqueCount; j++) {
                ...
                for (k = 0; k < m_pEffect->m_pGroups[i].pTechniques[j].PassCount; k++) {
                    VH(CalculateAnnotationSize(m_pEffect->m_pGroups[i].pTechniques[j].pPasses[k].AnnotationCount,
                                               m_pEffect->m_pGroups[i].pTechniques[j].pPasses[k].pAnnotations));
                }
            }
        }
...
HRESULT CEffectLoader::ReallocateReflectionData(bool Cloning) {
    ...
    for (i = 0; i < m_pEffect->m_GroupCount; i++) {
        ...
        for (j = 0; j < m_pEffect->m_pGroups[i].TechniqueCount; j++) {
            ...
            for (k = 0; k < m_pEffect->m_pGroups[i].pTechniques[j].PassCount; k++) {
                ...
                VH(ReallocateAnnotationData(m_pEffect->m_pGroups[i].pTechniques[j].pPasses[k].AnnotationCount,
                                            &m_pEffect->m_pGroups[i].pTechniques[j].pPasses[k].pAnnotations));
            }
        }
    }
    ...
    for (i = 0; i < m_pEffect->m_ShaderBlockCount; i++) {
        if (NULL != m_pEffect->m_pShaderBlocks[i].pReflectionData) {
            ...
            VHD(pHeap->MoveData((void **) &m_pEffect->m_pShaderBlocks[i].pReflectionData->pBytecode,
                                m_pEffect->m_pShaderBlocks[i].pReflectionData->BytecodeLength),
            ...
            VH(pHeap->MoveInterfaceParameters(m_pEffect->m_pShaderBlocks[i].pReflectionData->InterfaceParameterCount,
                                              &m_pEffect->m_pShaderBlocks[i].pReflectionData->pInterfaceParameters));
        }
    }
...
template<class T>
HRESULT CEffectLoader::ReallocateBlockAssignments(T *& pBlocks, UINT cBlocks, T * pOldBlocks) {
    HRESULT       hr    = S_OK;
    ...
    for (i = 0; i < cBlocks; i++) {
        T * pBlock = &pBlocks[i];
        VHD(pHeap->MoveData((void **) &pBlock->pAssignments, sizeof(SAssignment) * pBlock->AssignmentCount),
            "Internal loading error: cannot move assignment count.");
...
template<class T>
UINT CEffectLoader::CalculateBlockAssignmentSize(T *& pBlocks, UINT cBlocks) {
    UINT dwSize = 0;
    UINT i, j;
...
HRESULT CEffectLoader::ReallocateShaderBlocks() {
    ...
    for (i = 0; i < m_pEffect->m_ShaderBlockCount; i++) {
        ...
        for (j = 0; j < pShader->CBDepCount; j++) {
            ...
            for (k = 0; k < pCBDeps->Count; k++) { VH(FixupCBPointer(&pCBDeps->ppFXPointers[k])); }
        }
        ...
        for (j = 0; j < pShader->SampDepCount; j++) {
            ...
            for (k = 0; k < pSampDeps->Count; k++) { VH(FixupSamplerPointer(&pSampDeps->ppFXPointers[k])); }
        }
        ...
        for (j = 0; j < pShader->InterfaceDepCount; j++) {
            ...
            for (k = 0; k < pInterfaceDeps->Count; k++) { VH(FixupInterfacePointer(&pInterfaceDeps->ppFXPointers[k], true)); }
        }
        ...
        for (j = 0; j < pShader->ResourceDepCount; j++) {
            ...
            for (k = 0; k < pResourceDeps->Count; k++) { VH(FixupShaderResourcePointer(&pResourceDeps->ppFXPointers[k])); }
        }
        ...
        for (j = 0; j < pShader->UAVDepCount; j++) {
            ...
            for (k = 0; k < pUAVDeps->Count; k++) { VH(FixupUnorderedAccessViewPointer(&pUAVDeps->ppFXPointers[k])); }
        }

        for (j = 0; j < pShader->TBufferDepCount; j++) { VH(FixupCBPointer(&pShader->ppTbufDeps[j])); }
    }
...
HRESULT CEffectLoader::ReallocateEffectData(bool Cloning) {
    ...
    for (i = 0; i < m_pEffect->m_VariableCount; i++) {
        ...
        if (pVar->pCB) { VH(FixupCBPointer(&pVar->pCB)); }
    ...
    for (i = 0; i < m_pEffect->m_CBCount; i++) {
        ...
        if (pCB->pVariables != NULL) { VH(FixupVariablePointer(&pCB->pVariables)); }
    }
    ...
    for (i = 0; i < m_pEffect->m_SamplerBlockCount; ++i) { VH(FixupShaderResourcePointer(&m_pEffect->m_pSamplerBlocks[i].BackingStore.pTexture)); }
    ...
    for (i = 0; i < m_pEffect->m_InterfaceCount; i++) {
        ...
        if (pInterface->pClassInstance != NULL) { VH(FixupVariablePointer((SGlobalVariable **) &pInterface->pClassInstance)); }
    }
    ...
    for (i = 0; i < m_pEffect->m_VariableCount; i++) {
        ...
        if (pVar->pType->IsShader()) {
            ...
            if (!m_pEffect->IsOptimized()) { VH(FixupStringPointer(&pVar->Data.pString)); }
