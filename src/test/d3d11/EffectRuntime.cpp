...
BOOL SBaseBlock::ApplyAssignments(CEffect * pEffect) {
    ...
    for (; pAssignment < pLastAssn; pAssignment++) { bRecreate |= pEffect->EvaluateAssignment(pAssignment); }
...
BOOL SPassBlock::CheckDependencies() {
    ...
    for (UINT i = 0; i < AssignmentCount; i++) {
        if (pAssignments[i].DependencyCount > 0) return HasDependencies = TRUE;
    }
    if (BackingStore.pBlendBlock && BackingStore.pBlendBlock->AssignmentCount > 0) {
        for (UINT i = 0; i < BackingStore.pBlendBlock->AssignmentCount; i++) {
            if (BackingStore.pBlendBlock->pAssignments[i].DependencyCount > 0) return HasDependencies = TRUE;
        }
    }
    if (BackingStore.pDepthStencilBlock && BackingStore.pDepthStencilBlock->AssignmentCount > 0) {
        for (UINT i = 0; i < BackingStore.pDepthStencilBlock->AssignmentCount; i++) {
            if (BackingStore.pDepthStencilBlock->pAssignments[i].DependencyCount > 0) return HasDependencies = TRUE;
        }
    }
    if (BackingStore.pRasterizerBlock && BackingStore.pRasterizerBlock->AssignmentCount > 0) {
        for (UINT i = 0; i < BackingStore.pRasterizerBlock->AssignmentCount; i++) {
            if (BackingStore.pRasterizerBlock->pAssignments[i].DependencyCount > 0) return HasDependencies = TRUE;
        }
    }
    if (BackingStore.pVertexShaderBlock && CheckShaderDependencies(BackingStore.pVertexShaderBlock)) { return HasDependencies = TRUE; }
    if (BackingStore.pGeometryShaderBlock && CheckShaderDependencies(BackingStore.pGeometryShaderBlock)) { return HasDependencies = TRUE; }
    if (BackingStore.pPixelShaderBlock && CheckShaderDependencies(BackingStore.pPixelShaderBlock)) { return HasDependencies = TRUE; }
    if (BackingStore.pHullShaderBlock && CheckShaderDependencies(BackingStore.pHullShaderBlock)) { return HasDependencies = TRUE; }
    if (BackingStore.pDomainShaderBlock && CheckShaderDependencies(BackingStore.pDomainShaderBlock)) { return HasDependencies = TRUE; }
    if (BackingStore.pComputeShaderBlock && CheckShaderDependencies(BackingStore.pComputeShaderBlock)) { return HasDependencies = TRUE; }
...
void CEffect::ApplyShaderBlock(SShaderBlock * pBlock) {
    ...
    for (; pCBDep < pLastCBDep; pCBDep++) {
        ...
        for (i = 0; i < pCBDep->Count; ++i) { CheckAndUpdateCB_FX(m_pContext, (SConstantBuffer *) pCBDep->ppFXPointers[i]); }
    ...
    if (pBlock->UAVDepCount > 0) {
        ...
        for (i = 0; i < pUAVDep->Count; i++) { pUAVDep->ppD3DObjects[i] = pUAVDep->ppFXPointers[i]->pUnorderedAccessView; }
        ...
        if (EOT_ComputeShader5 == pBlock->GetShaderType()) {
            ...
            m_pContext->OMSetRenderTargetsAndUnorderedAccessViews(D3D11_KEEP_RENDER_TARGETS_AND_DEPTH_STENCIL, NULL, NULL, pUAVDep->StartIndex, pUAVDep->Count,
                                                                  pUAVDep->ppD3DObjects, g_pNegativeOnes);
        }
    }
    ...
    for (; ppTB < ppLastTB; ppTB++) { CheckAndUpdateCB_FX(m_pContext, (SConstantBuffer *) *ppTB); }
    ...
    for (; pResourceDep < pLastResourceDep; pResourceDep++) {
        ...
        for (i = 0; i < pResourceDep->Count; i++) { pResourceDep->ppD3DObjects[i] = pResourceDep->ppFXPointers[i]->pShaderResource; }
...
BOOL CEffect::ApplyRenderStateBlock(SBaseBlock * pBlock) {
    if (pBlock->IsUserManaged) { return FALSE; }
...
BOOL CEffect::EvaluateAssignment(SAssignment * pAssignment) {
    ...
    case ERAT_ObjectVariableIndex:
        ...
        if (pVarDep0->LastModifiedTime >= pAssignment->LastRecomputedTime) {
            ...
            *((void **) pAssignment->Destination.pGeneric) = pAssignment->Source.pNumeric + pAssignment->DataSize * m_FXLIndex;
            bNeedUpdate                                    = TRUE;
        }
        break;
...
BOOL CEffect::ValidateShaderBlock(SShaderBlock * pBlock) {
    if (!pBlock->IsValid) return FALSE;
    if (pBlock->InterfaceDepCount > 0) {
        D3DXASSERT(pBlock->InterfaceDepCount == 1);
        for (UINT i = 0; i < pBlock->pInterfaceDeps[0].Count; i++) {
            SInterface * pInterfaceDep = pBlock->pInterfaceDeps[0].ppFXPointers[i];
            D3DXASSERT(pInterfaceDep != NULL);
            if (pInterfaceDep->pClassInstance == NULL) { return FALSE; }
        }
    }
    return TRUE;
}
...
BOOL CEffect::ValidatePassBlock(SPassBlock * pBlock) {
    ...
    if (NULL != pBlock->BackingStore.pBlendBlock) {
        ...
        if (!pBlock->BackingStore.pBlendBlock->IsValid) return FALSE;
    }
    ...
    if (NULL != pBlock->BackingStore.pDepthStencilBlock) {
        ...
        if (!pBlock->BackingStore.pDepthStencilBlock->IsValid) return FALSE;
    }

    if (NULL != pBlock->BackingStore.pRasterizerBlock) {
        ApplyRenderStateBlock(pBlock->BackingStore.pRasterizerBlock);
        if (!pBlock->BackingStore.pRasterizerBlock->IsValid) return FALSE;
    }

    if (NULL != pBlock->BackingStore.pVertexShaderBlock && !ValidateShaderBlock(pBlock->BackingStore.pVertexShaderBlock)) return FALSE;

    if (NULL != pBlock->BackingStore.pGeometryShaderBlock && !ValidateShaderBlock(pBlock->BackingStore.pGeometryShaderBlock)) return FALSE;
    ...
    if (NULL != pBlock->BackingStore.pHullShaderBlock && !ValidateShaderBlock(pBlock->BackingStore.pHullShaderBlock)) return FALSE;

    if (NULL != pBlock->BackingStore.pDomainShaderBlock && !ValidateShaderBlock(pBlock->BackingStore.pDomainShaderBlock)) return FALSE;

    if (NULL != pBlock->BackingStore.pComputeShaderBlock && !ValidateShaderBlock(pBlock->BackingStore.pComputeShaderBlock)) return FALSE;
...
void CEffect::ApplyPassBlock(SPassBlock * pBlock) {
    ...
    if (NULL != pBlock->BackingStore.pBlendBlock) {
        ApplyRenderStateBlock(pBlock->BackingStore.pBlendBlock);
#ifdef FXDEBUG
        if (!pBlock->BackingStore.pBlendBlock->IsValid) DPF(0, "Pass::Apply - warning: applying invalid BlendState.");
#endif
        pBlock->BackingStore.pBlendState = pBlock->BackingStore.pBlendBlock->pBlendObject;
        m_pContext->OMSetBlendState(pBlock->BackingStore.pBlendState, pBlock->BackingStore.BlendFactor, pBlock->BackingStore.SampleMask);
    }

    if (NULL != pBlock->BackingStore.pDepthStencilBlock) {
        ApplyRenderStateBlock(pBlock->BackingStore.pDepthStencilBlock);
#ifdef FXDEBUG
        if (!pBlock->BackingStore.pDepthStencilBlock->IsValid) DPF(0, "Pass::Apply - warning: applying invalid DepthStencilState.");
#endif
        pBlock->BackingStore.pDepthStencilState = pBlock->BackingStore.pDepthStencilBlock->pDSObject;
        m_pContext->OMSetDepthStencilState(pBlock->BackingStore.pDepthStencilState, pBlock->BackingStore.StencilRef);
    }

    if (NULL != pBlock->BackingStore.pRasterizerBlock) {
        ApplyRenderStateBlock(pBlock->BackingStore.pRasterizerBlock);
#ifdef FXDEBUG
        if (!pBlock->BackingStore.pRasterizerBlock->IsValid) DPF(0, "Pass::Apply - warning: applying invalid RasterizerState.");
#endif
    ...
    if (NULL != pBlock->BackingStore.pRenderTargetViews[0]) {
        ...
        for (UINT i = 0; i < pBlock->BackingStore.RenderTargetViewCount; i++) { pRTV[i] = pBlock->BackingStore.pRenderTargetViews[i]->pRenderTargetView; }

        // This call could be combined with the call to set PS UAVs if both exist in the pass
        m_pContext->OMSetRenderTargetsAndUnorderedAccessViews(pBlock->BackingStore.RenderTargetViewCount, pRTV,
                                                              pBlock->BackingStore.pDepthStencilView->pDepthStencilView, 7, D3D11_KEEP_UNORDERED_ACCESS_VIEWS,
                                                              NULL, NULL);
    }

    if (NULL != pBlock->BackingStore.pVertexShaderBlock) {
#ifdef FXDEBUG
        if (!pBlock->BackingStore.pVertexShaderBlock->IsValid) DPF(0, "Pass::Apply - warning: applying invalid vertex shader.");
#endif
    ...
    if (NULL != pBlock->BackingStore.pPixelShaderBlock) {
#ifdef FXDEBUG
        if (!pBlock->BackingStore.pPixelShaderBlock->IsValid) DPF(0, "Pass::Apply - warning: applying invalid pixel shader.");
#endif
...
#ifdef FXDEBUG
        if (!pBlock->BackingStore.pGeometryShaderBlock->IsValid) DPF(0, "Pass::Apply - warning: applying invalid geometry shader.");
#endif
...
#ifdef FXDEBUG
        if (!pBlock->BackingStore.pHullShaderBlock->IsValid) DPF(0, "Pass::Apply - warning: applying invalid hull shader.");
#endif
...
#ifdef FXDEBUG
        if (!pBlock->BackingStore.pDomainShaderBlock->IsValid) DPF(0, "Pass::Apply - warning: applying invalid domain shader.");
#endif
...
#ifdef FXDEBUG
        if (!pBlock->BackingStore.pComputeShaderBlock->IsValid) DPF(0, "Pass::Apply - warning: applying invalid compute shader.");
#endif
...
void CEffect::HandleLocalTimerRollover() {
    ...
    for (i = 0; i < m_VariableCount; ++i) { m_pVariables[i].LastModifiedTime = 0; }
    ...
    for (i = 0; i < m_DepthStencilBlockCount; ++i) {
        for (j = 0; j < m_pDepthStencilBlocks[i].AssignmentCount; ++j) { m_pDepthStencilBlocks[i].pAssignments[j].LastRecomputedTime = 0; }
    }

    for (i = 0; i < m_RasterizerBlockCount; ++i) {
        for (j = 0; j < m_pRasterizerBlocks[i].AssignmentCount; ++j) { m_pRasterizerBlocks[i].pAssignments[j].LastRecomputedTime = 0; }
    }

    for (i = 0; i < m_BlendBlockCount; ++i) {
        for (j = 0; j < m_pBlendBlocks[i].AssignmentCount; ++j) { m_pBlendBlocks[i].pAssignments[j].LastRecomputedTime = 0; }
    }

    for (i = 0; i < m_SamplerBlockCount; ++i) {
        for (j = 0; j < m_pSamplerBlocks[i].AssignmentCount; ++j) { m_pSamplerBlocks[i].pAssignments[j].LastRecomputedTime = 0; }
    }
}
