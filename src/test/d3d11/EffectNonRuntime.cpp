...
SBaseBlock::SBaseBlock(): BlockType(EBT_Invalid), IsUserManaged(FALSE), AssignmentCount(0), pAssignments(NULL) {}
...
STechnique::STechnique(): pName(NULL), PassCount(0), pPasses(NULL), AnnotationCount(0), pAnnotations(NULL), InitiallyValid(TRUE), HasDependencies(FALSE) {}

SGroup::SGroup(): pName(NULL), TechniqueCount(0), pTechniques(NULL), AnnotationCount(0), pAnnotations(NULL), InitiallyValid(TRUE), HasDependencies(FALSE) {}
...
HRESULT SShaderBlock::OnDeviceBind() {
    ...
    for (i = 0; i < CBDepCount; i++) {
        ...
        for (j = 0; j < pCBDeps[i].Count; j++) {
            ...
            if (!pCBDeps[i].ppD3DObjects[j]) VH(E_FAIL);
        }
    }
    ...
    for (i = 0; i < SampDepCount; i++) {
        ...
        for (j = 0; j < pSampDeps[i].Count; j++) {
            ...
            if (!pSampDeps[i].ppD3DObjects[j]) VH(E_FAIL);
        }
    }
...
HRESULT SShaderBlock::ComputeStateBlockMask(D3DX11_STATE_BLOCK_MASK * pStateBlockMask) {
    ...
    for (i = 0; i < SampDepCount; ++i) {
        for (j = 0; j < pSampDeps[i].Count; ++j) { _SET_BIT(pSamplerMask, (pSampDeps[i].StartIndex + j)); }
    }

    for (i = 0; i < InterfaceDepCount; ++i) {
        for (j = 0; j < pInterfaceDeps[i].Count; ++j) { _SET_BIT(pInterfaceMask, (pInterfaceDeps[i].StartIndex + j)); }
    }

    for (i = 0; i < ResourceDepCount; ++i) {
        for (j = 0; j < pResourceDeps[i].Count; ++j) { _SET_BIT(pShaderResourceMask, (pResourceDeps[i].StartIndex + j)); }
    }

    for (i = 0; i < CBDepCount; ++i) {
        for (j = 0; j < pCBDeps[i].Count; ++j) { _SET_BIT(pConstantBufferMask, (pCBDeps[i].StartIndex + j)); }
    }

    for (i = 0; i < UAVDepCount; ++i) {
        D3DXASSERT(pUnorderedAccessViewMask != NULL);
        for (j = 0; j < pUAVDeps[i].Count; ++j) {
            if (pUAVDeps[i].ppFXPointers[j] != &g_NullUnorderedAccessView) _SET_BIT(pUnorderedAccessViewMask, (pUAVDeps[i].StartIndex + j));
        }
    }
...
HRESULT SShaderBlock::GetShaderDesc(D3DX11_EFFECT_SHADER_DESC * pDesc, BOOL IsInline) {
    ...
    if (NULL != pReflectionData) {
        ...
        for (UINT iDecl = 0; iDecl < D3D11_SO_STREAM_COUNT; ++iDecl) { pDesc->SODecls[iDecl] = pReflectionData->pStreamOutDecls[iDecl]; }
...
HRESULT SShaderBlock::GetVertexShader(ID3D11VertexShader ** ppVS) {
    if (EOT_VertexShader == GetShaderType() || EOT_VertexShader5 == GetShaderType()) {
...
HRESULT SShaderBlock::GetGeometryShader(ID3D11GeometryShader ** ppGS) {
    if (EOT_GeometryShader == GetShaderType() || EOT_GeometryShaderSO == GetShaderType() || EOT_GeometryShader5 == GetShaderType()) {
...
HRESULT SShaderBlock::GetPixelShader(ID3D11PixelShader ** ppPS) {
    if (EOT_PixelShader == GetShaderType() || EOT_PixelShader5 == GetShaderType()) {
...
SString::SString() { pString = NULL; }

SRenderTargetView::SRenderTargetView() { pRenderTargetView = NULL; }

SDepthStencilView::SDepthStencilView() { pDepthStencilView = NULL; }
...
void CEffect::ReleaseShaderRefection() {
    for (UINT i = 0; i < m_ShaderBlockCount; ++i) {
        ...
        if (m_pShaderBlocks[i].pReflectionData) { SAFE_RELEASE(m_pShaderBlocks[i].pReflectionData->pReflection); }
    }
}
...
CEffect::~CEffect() {
    ...
    // Mute debug spew
    if (m_pDevice) m_pDevice->QueryInterface(__uuidof(ID3D11InfoQueue), (void **) &pInfoQueue);
    ...
    if (NULL != m_pDevice) {
        ...
        for (i = 0; i < m_RasterizerBlockCount; ++i) { SAFE_RELEASE(m_pRasterizerBlocks[i].pRasterizerObject); }

        D3DXASSERT(NULL == m_pBlendBlocks || m_Heap.IsInHeap(m_pBlendBlocks));
        for (i = 0; i < m_BlendBlockCount; ++i) { SAFE_RELEASE(m_pBlendBlocks[i].pBlendObject); }

        D3DXASSERT(NULL == m_pDepthStencilBlocks || m_Heap.IsInHeap(m_pDepthStencilBlocks));
        for (i = 0; i < m_DepthStencilBlockCount; ++i) { SAFE_RELEASE(m_pDepthStencilBlocks[i].pDSObject); }

        D3DXASSERT(NULL == m_pSamplerBlocks || m_Heap.IsInHeap(m_pSamplerBlocks));
        for (i = 0; i < m_SamplerBlockCount; ++i) { SAFE_RELEASE(m_pSamplerBlocks[i].pD3DObject); }

        D3DXASSERT(NULL == m_pShaderResources || m_Heap.IsInHeap(m_pShaderResources));
        for (i = 0; i < m_ShaderResourceCount; ++i) { SAFE_RELEASE(m_pShaderResources[i].pShaderResource); }
        ...
        for (i = 0; i < m_RenderTargetViewCount; ++i) { SAFE_RELEASE(m_pRenderTargetViews[i].pRenderTargetView); }

        D3DXASSERT(NULL == m_pDepthStencilViews || m_Heap.IsInHeap(m_pDepthStencilViews));
        for (i = 0; i < m_DepthStencilViewCount; ++i) { SAFE_RELEASE(m_pDepthStencilViews[i].pDepthStencilView); }
        ...
        for (i = 0; i < m_ShaderBlockCount; ++i) { SAFE_RELEASE(m_pShaderBlocks[i].pD3DObject); }
...
void CEffect::AddRefAllForCloning(CEffect * pEffectSource) {
    ...
    for (UINT i = 0; i < m_ShaderBlockCount; ++i) {
        ...
        if (m_pShaderBlocks[i].pReflectionData) { SAFE_ADDREF(m_pShaderBlocks[i].pReflectionData->pReflection); }
    }

    D3DXASSERT(NULL == m_pRasterizerBlocks || pEffectSource->m_Heap.IsInHeap(m_pRasterizerBlocks));
    for (i = 0; i < m_RasterizerBlockCount; ++i) { SAFE_ADDREF(m_pRasterizerBlocks[i].pRasterizerObject); }

    D3DXASSERT(NULL == m_pBlendBlocks || pEffectSource->m_Heap.IsInHeap(m_pBlendBlocks));
    for (i = 0; i < m_BlendBlockCount; ++i) { SAFE_ADDREF(m_pBlendBlocks[i].pBlendObject); }

    D3DXASSERT(NULL == m_pDepthStencilBlocks || pEffectSource->m_Heap.IsInHeap(m_pDepthStencilBlocks));
    for (i = 0; i < m_DepthStencilBlockCount; ++i) { SAFE_ADDREF(m_pDepthStencilBlocks[i].pDSObject); }

    D3DXASSERT(NULL == m_pSamplerBlocks || pEffectSource->m_Heap.IsInHeap(m_pSamplerBlocks));
    for (i = 0; i < m_SamplerBlockCount; ++i) { SAFE_ADDREF(m_pSamplerBlocks[i].pD3DObject); }

    D3DXASSERT(NULL == m_pShaderResources || pEffectSource->m_Heap.IsInHeap(m_pShaderResources));
    for (i = 0; i < m_ShaderResourceCount; ++i) { SAFE_ADDREF(m_pShaderResources[i].pShaderResource); }
    ...
    for (i = 0; i < m_RenderTargetViewCount; ++i) { SAFE_ADDREF(m_pRenderTargetViews[i].pRenderTargetView); }

    D3DXASSERT(NULL == m_pDepthStencilViews || pEffectSource->m_Heap.IsInHeap(m_pDepthStencilViews));
    for (i = 0; i < m_DepthStencilViewCount; ++i) { SAFE_ADDREF(m_pDepthStencilViews[i].pDepthStencilView); }
    ...
    for (i = 0; i < m_ShaderBlockCount; ++i) { SAFE_ADDREF(m_pShaderBlocks[i].pD3DObject); }
...
ULONG CEffect::AddRef() { return ++m_RefCount; }
...
HRESULT CEffect::BindToDevice(ID3D11Device * pDevice) {
    ...
    for (; pShader != pShaderLast; pShader++) {
        ...
        if (pShader->pReflectionData->pStreamOutDecls[0] || pShader->pReflectionData->pStreamOutDecls[1] || pShader->pReflectionData->pStreamOutDecls[2] ||
            pShader->pReflectionData->pStreamOutDecls[3]) {
            ...
            hr = m_pDevice->CreateGeometryShaderWithStreamOutput((UINT *) pShader->pReflectionData->pBytecode, pShader->pReflectionData->BytecodeLength,
                                                                 soParser.GetDeclArray(), soParser.GetDeclCount(), strides, featureLevelGE11 ? 4 : 1,
                                                                 pShader->pReflectionData->RasterizedStream, neededClassLinkage,
            ...
            if (pShader->pReflectionData->RasterizedStream == D3D11_SO_NO_RASTERIZED_STREAM)
                ...
                if (FAILED((m_pDevice->*(pShader->pVT->pCreateShader))((UINT *) pShader->pReflectionData->pBytecode, pShader->pReflectionData->BytecodeLength,
                                                                       neededClassLinkage, &pShader->pD3DObject))) {
    ...
    for (UINT i = 0; i < m_VariableCount; ++i) {
        if (m_pVariables[i].pMemberData) {
            if (m_pVariables[i].pType->IsClassInstance()) {
                for (UINT j = 0; j < std::max<UINT>(m_pVariables[i].pType->Elements, 1); ++j) {
    ...
    for (UINT iGroup = 0; iGroup < m_GroupCount; iGroup++) {
        ...
        for (UINT iTech = 0; iTech < pGroup->TechniqueCount; iTech++) {
            ...
            for (UINT iPass = 0; iPass < pTechnique->PassCount; iPass++) {
                ...
                if (pPass->BackingStore.pBlendBlock != NULL && !pPass->BackingStore.pBlendBlock->IsValid) pPass->InitiallyValid = FALSE;
                if (pPass->BackingStore.pDepthStencilBlock != NULL && !pPass->BackingStore.pDepthStencilBlock->IsValid) pPass->InitiallyValid = FALSE;
                if (pPass->BackingStore.pRasterizerBlock != NULL && !pPass->BackingStore.pRasterizerBlock->IsValid) pPass->InitiallyValid = FALSE;
                if (pPass->BackingStore.pVertexShaderBlock != NULL && !pPass->BackingStore.pVertexShaderBlock->IsValid) pPass->InitiallyValid = FALSE;
                if (pPass->BackingStore.pPixelShaderBlock != NULL && !pPass->BackingStore.pPixelShaderBlock->IsValid) pPass->InitiallyValid = FALSE;
                if (pPass->BackingStore.pGeometryShaderBlock != NULL && !pPass->BackingStore.pGeometryShaderBlock->IsValid) pPass->InitiallyValid = FALSE;
                if (pPass->BackingStore.pHullShaderBlock != NULL && !pPass->BackingStore.pHullShaderBlock->IsValid) pPass->InitiallyValid = FALSE;
                if (pPass->BackingStore.pDomainShaderBlock != NULL && !pPass->BackingStore.pDomainShaderBlock->IsValid) pPass->InitiallyValid = FALSE;
                if (pPass->BackingStore.pComputeShaderBlock != NULL && !pPass->BackingStore.pComputeShaderBlock->IsValid) pPass->InitiallyValid = FALSE;
...
SVariable * CEffect::FindVariableByNameWithParsing(LPCSTR pName) {
    ...
    while (*pSource != 0) {
        if (pDest == pEnd) {
            ...
            if (pVariable == NULL) { DPF(0, "Name %s is too long to parse", &pName); }
            return pVariable;
        }
        ...
        if (*pSource == '[') {
            ...
            *pDest     = 0;
            ...
            if (pVariable && !pVariable->IsValid()) { pVariable = NULL; }
            return pVariable;
        } else {
            // add character
            *pDest = *pSource;
...
SGlobalVariable * CEffect::FindLocalVariableByName(LPCSTR pName) {
    ...
    for (pVariable = m_pVariables; pVariable != pVariableEnd; pVariable++) {
        if (strcmp(pVariable->pName, pName) == 0) { return pVariable; }
    }
...
BOOL SType::IsEqual(SType * pOtherType) CONST {
    if (VarType != pOtherType->VarType || Elements != pOtherType->Elements || strcmp(pTypeName, pOtherType->pTypeName) != 0) { return FALSE; }

    switch (VarType) {
    case EVT_Struct: {
        if (StructType.Members != pOtherType->StructType.Members) { return FALSE; }
    ...
    case EVT_Object: {
        if (ObjectType != pOtherType->ObjectType) { return FALSE; }
    } break;

    case EVT_Numeric: {
        if (NumericType.Rows != pOtherType->NumericType.Rows || NumericType.Columns != pOtherType->NumericType.Columns ||
            NumericType.ScalarType != pOtherType->NumericType.ScalarType || NumericType.NumericLayout != pOtherType->NumericType.NumericLayout ||
            NumericType.IsColumnMajor != pOtherType->NumericType.IsColumnMajor || NumericType.IsPackedArray != pOtherType->NumericType.IsPackedArray) {
            return FALSE;
        }
    } break;
...
UINT SType::GetTotalUnpackedSize(BOOL IsSingleElement) CONST {
    if (VarType == EVT_Object) {
        ...
        D3DXASSERT((TotalSize == 0 && Stride == 0) || ((TotalSize > (Stride * (Elements - 1))) && (TotalSize <= (Stride * Elements))));
...
SConstantBuffer * CEffect::FindCB(LPCSTR pName) {
    ...
    for (i = 0; i < m_CBCount; i++) {
        if (!strcmp(m_pCBs[i].pName, pName)) { return &m_pCBs[i]; }
    }

    return NULL;
}

inline UINT PtrToDword(void * pPtr) { return (UINT) (UINT_PTR) pPtr; }
...
HRESULT CEffect::CopyStringPool(CEffect * pEffectSource, CPointerMappingTable & mappingTable) {
    ...
    for (pEffectSource->m_pStringPool->GetFirstEntry(&stringIter); !pEffectSource->m_pStringPool->PastEnd(&stringIter);
         pEffectSource->m_pStringPool->GetNextEntry(&stringIter)) {
        SPointerMapping ptrMapping;
        char *          pString;
...
HRESULT CEffect::CopyTypePool(CEffect * pEffectSource, CPointerMappingTable & mappingTableTypes, CPointerMappingTable & mappingTableStrings) {
    ...
    for (pEffectSource->m_pTypePool->GetFirstEntry(&typeIter); !pEffectSource->m_pTypePool->PastEnd(&typeIter);
         pEffectSource->m_pTypePool->GetNextEntry(&typeIter)) {
        SPointerMapping ptrMapping;
        SType *         pType;
    ...
    for (mappingTableTypes.GetFirstEntry(&mapIter); !mappingTableTypes.PastEnd(&mapIter); mappingTableTypes.GetNextEntry(&mapIter)) {
        ...
        if (pType->pTypeName) { VH(RemapString(&pType->pTypeName, &mappingTableStrings)); }
        ...
        if (EVT_Struct == pType->VarType) {
            for (UINT i = 0; i < pType->StructType.Members; ++i) {
                ...
                if (pType->StructType.pMembers[i].pName) { VH(RemapString(&pType->StructType.pMembers[i].pName, &mappingTableStrings)); }
                if (pType->StructType.pMembers[i].pSemantic) { VH(RemapString(&pType->StructType.pMembers[i].pSemantic, &mappingTableStrings)); }
            }
        }
    }
...
HRESULT CEffect::CopyOptimizedTypePool(CEffect * pEffectSource, CPointerMappingTable & mappingTableTypes) {
    ...
    for (mappingTableTypes.GetFirstEntry(&mapIter); !mappingTableTypes.PastEnd(&mapIter); mappingTableTypes.GetNextEntry(&mapIter)) {
        ...
        // if this is a struct, fix up its members' pointers
        if (EVT_Struct == pType->VarType) {
            for (UINT i = 0; i < pType->StructType.Members; ++i) { VH(RemapType((SType **) &pType->StructType.pMembers[i].pType, &mappingTableTypes)); }
        }
    }
...
HRESULT CEffect::FixupMemberInterface(SMember * pMember, CEffect * pEffectSource, CPointerMappingTable & mappingTableStrings) {
    ...
    if (pMember->pName) {
        if (pEffectSource->m_pReflection && pEffectSource->m_pReflection->m_Heap.IsInHeap(pMember->pName)) {
            pMember->pName = (char *) ((UINT_PTR) pMember->pName - (UINT_PTR) pEffectSource->m_pReflection->m_Heap.GetDataStart() +
                                       (UINT_PTR) m_pReflection->m_Heap.GetDataStart());
        } else {
    ...
    if (pMember->pSemantic) {
        if (pEffectSource->m_pReflection && pEffectSource->m_pReflection->m_Heap.IsInHeap(pMember->pSemantic)) {
            pMember->pSemantic = (char *) ((UINT_PTR) pMember->pSemantic - (UINT_PTR) pEffectSource->m_pReflection->m_Heap.GetDataStart() +
                                           (UINT_PTR) m_pReflection->m_Heap.GetDataStart());
        } else {
...
HRESULT CEffect::CloneEffect(UINT Flags, ID3DX11Effect ** ppClonedEffect) {
    ...
    if (!IsOptimized()) { VH(loader.ReallocateReflectionData(true)); }
...
lExit:
    ...
    if (FAILED(hr)) { SAFE_DELETE(pNewEffect); }
...
HRESULT CEffect::OptimizeTypes(CPointerMappingTable * pMappingTable, bool Cloning) {
    ...
    for (i = 0; i < m_VariableCount; ++i) { VH(RemapType((SType **) &m_pVariables[i].pType, pMappingTable)); }

    UINT Members = m_pMemberInterfaces.GetSize();
    for (i = 0; i < Members; i++) {
        if (m_pMemberInterfaces[i] != NULL) { VH(RemapType((SType **) &m_pMemberInterfaces[i]->pType, pMappingTable)); }
    }

    // when cloning, there may be annotations
    if (Cloning) {
        for (UINT iVar = 0; iVar < m_VariableCount; ++iVar) {
            for (i = 0; i < m_pVariables[iVar].AnnotationCount; ++i) { VH(RemapType((SType **) &m_pVariables[iVar].pAnnotations[i].pType, pMappingTable)); }
        }
        for (UINT iCB = 0; iCB < m_CBCount; ++iCB) {
            for (i = 0; i < m_pCBs[iCB].AnnotationCount; ++i) { VH(RemapType((SType **) &m_pCBs[iCB].pAnnotations[i].pType, pMappingTable)); }
        }
        for (UINT iGroup = 0; iGroup < m_GroupCount; ++iGroup) {
            for (i = 0; i < m_pGroups[iGroup].AnnotationCount; ++i) { VH(RemapType((SType **) &m_pGroups[iGroup].pAnnotations[i].pType, pMappingTable)); }
...
HRESULT CEffect::Optimize() {
    ...
    for (m_pTypePool->GetFirstEntry(&typeIter); !m_pTypePool->PastEnd(&typeIter); m_pTypePool->GetNextEntry(&typeIter)) {
        ...
        if (EVT_Struct == pType->VarType) { chkSpaceNeeded += AlignToPowerOf2(pType->StructType.Members * sizeof(SVariable), c_DataAlignment); }
    }
