...
public:
    ...
    bool init() {
        ...
        c0.attach(gpu.create2DTexture(PixelFormat::RGBA8(), (uint32) RT_WIDTH, (uint32) RT_HEIGHT, 1, TextureUsage::COLOR_RENDER_TARGET));
        if (!c0) {
            GN_ERROR(sLogger)("Current graphics hardware does not support render-to-texture at all.");
            return false;
        }

        ds.attach(gpu.create2DTexture(PixelFormat::RG_24_UNORM_8_UINT(), (uint32) RT_WIDTH, (uint32) RT_HEIGHT, 1, TextureUsage::DEPTH_RENDER_TARGET));
