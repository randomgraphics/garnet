...
//
// ----------------------------------------------------------------------------
bool level_c::collide_with_radial(collideinfo_s & cinfo, const Vector3f & start, const Vector3f & end) const {
    GN_GUARD_SLOW;
    ...
    if (cinfo.sector_id < 0) {
        ...
        return m_sectors[cinfo.sector_id].collide_with_radial(cinfo, start, end);
    }
...
//
// ----------------------------------------------------------------------------
bool level_c::collide_with_segment(collideinfo_s & cinfo, const Vector3f & start, const Vector3f & end) const {
    GN_GUARD_SLOW;
    ...
    if (cinfo.sector_id < 0) {
        ...
        GN_ASSERT(uint(cinfo.sector_id) < m_sectors.size());
        return m_sectors[cinfo.sector_id].collide_with_segment(cinfo, start, end);
    }
