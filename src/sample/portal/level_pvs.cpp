...
//
// ----------------------------------------------------------------------------
void level_c::update_visinfo(const Matrix44f & proj, const Matrix44f & view) {
    GN_GUARD;
    ...
    for (uint i = 0; i < m_sectors.size(); ++i) { m_sectors[i].vischeck = false; }
    ...
    if (last_sector != camera_sector) {
        GN_INFO(GN_formatstr(GN_T("======= Go from sector %d to %d ========="), last_sector, camera_sector));
    ...
    if (-1 == camera_sector) {
        ...
        for (uint i = 0; i < m_sectors.size(); ++i) m_visible_sectors[i] = i;
        return;
    }
    ...
    for (uint i = 0; i < s.portals.size(); ++i) {
        ...
        if (0 == v) {
            ...
            if (-1 == v) {
                ...
                Vector3f ipoint;
                if (!intersection_polygon_line(ipoint, p.plane, &p.verts[0], p.verts.size(), cf.eye_point, cf.eye_direction)) continue;
        ...
        if (!bsptree_check_poly(sector_id, cf, &p.verts[0], p.verts.size())) continue;
        ...
        if (0 == v) {
            ...
            clip_polygon_with_plane_list(clipresult, &p.verts[0], p.verts.size(), cf.planes, cf.num_planes);
...
//
// ----------------------------------------------------------------------------
bool level_c::bsptree_check_poly(uint /*section_id*/, const clipfrustum_c & /*cf*/, const Vector3f * /*vlist*/, size_t /*numvert*/) {
    GN_GUARD_SLOW;
