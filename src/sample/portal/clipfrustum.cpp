...
//
// ----------------------------------------------------------------------------
void clipfrustum_c::from_projview(const Matrix44f & proj, const Matrix44f & view) {
    GN_GUARD_SLOW;
...
//
// ----------------------------------------------------------------------------
void clipfrustum_c::clip_by_polygon(clipfrustum_c & result, const Vector3f * verts, size_t numvert, const Plane3f & plane) const {
    GN_GUARD_SLOW;
    ...
    if (-PLANE_THICKNESS * 10 < d && d < PLANE_THICKNESS * 10) {
        ...
        if (Vector3f::dot(eye_direction, plane.n) >= 0.0f) {
            clip_polygon_with_plane_list(varray, far_verts, num_planes - 2, &plane, 1);
        } else {
            Plane3f reverse_plane = -plane;
            clip_polygon_with_plane_list(varray, far_verts, num_planes - 2, &reverse_plane, 1);
        }
        GN_ASSERT(varray.size() < MAX_PLANES - 2);
        numvert = varray.size();
        if (numvert > 0) { memcpy(result.far_verts, &varray[0], sizeof(Vector3f) * numvert); }
        ...
        result.planes[FAR_PLANE].from_3points(result.far_verts[0], result.far_verts[1], result.far_verts[2]);
        result.planes[FAR_PLANE].normalize();
    }

    // build side planes ( except the last one )
    for (i = 2; i <= numvert; ++i) {
        result.planes[i].from_3points(eye_point, result.far_verts[i - 1], result.far_verts[i - 2]);
        result.planes[i].normalize();
    }
    // build the last side plane
    result.planes[i].from_3points(eye_point, result.far_verts[0], result.far_verts[numvert - 1]);
    result.planes[i].normalize();
...
//
// ----------------------------------------------------------------------------
int clipfrustum_c::check_poly_visibility(const Vector3f * verts, size_t numvert, const Plane3f & plane) const {
    GN_GUARD_SLOW;
