...
bool portaldemo_c::on_initialize() {
    ...
    get_camctrl().lookat(Vector3f(1000.0f, 1000.0f, 1000.0f), Vector3f(0.0f, 0.0f, 0.0f), Vector3f(0.0f, 1.0f, 0.0f));
    ...
    input_i::keycode_t keys[] = {
        input_i::KEY_I,      input_i::KEY_K,        input_i::KEY_J,          input_i::KEY_L,          input_i::KEY_INSERT,
        input_i::KEY_DELETE, input_i::KEY_NUMPAD_5, input_i::KEY_NUMPAD_8,   input_i::KEY_NUMPAD_4,   input_i::KEY_NUMPAD_6,
        input_i::KEY_NONE,   input_i::KEY_NONE,     input_i::KEY_MOUSEBTN_1, input_i::KEY_MOUSEBTN_2,
    };
    ...
    eff.desc.rsblock.set_rs(GNgfx::RS_DEPTH_TEST, GNgfx::RSVAL_FALSE);
    eff.desc.rsblock.set_tss(0, GNgfx::TSS_COLOROP, GNgfx::TSSVAL_ARG0);
    eff.desc.rsblock.set_tss(0, GNgfx::TSS_COLORARG0, GNgfx::TSSVAL_CONSTANT_COLOR);
    eff.desc.rsblock.set_tss(0, GNgfx::TSS_ALPHAOP, GNgfx::TSSVAL_ARG0);
    eff.desc.rsblock.set_tss(0, GNgfx::TSS_ALPHAARG0, GNgfx::TSSVAL_CONSTANT_ALPHA);
    if (!g_efflib.add(GN_T("portaldemo_nolight.gfx"), eff, 0)) return false;

    // light effect
    eff.desc.rsblock.set_rs(GNgfx::RS_LIGHTING_ENABLE, GNgfx::RSVAL_TRUE);
    eff.desc.rsblock.set_rs(GNgfx::RS_BLEND_ENABLE, GNgfx::RSVAL_TRUE);
    eff.desc.rsblock.set_tss(0, GNgfx::TSS_COLORARG0, GNgfx::TSSVAL_CURRENT_COLOR);
    eff.desc.rsblock.set_tss(0, GNgfx::TSS_ALPHAARG0, GNgfx::TSSVAL_CURRENT_ALPHA);
    if (!g_efflib.add(GN_T("portaldemo_light.gfx"), eff, 0)) return false;

    // initialize help screen
    const wchar_t help[] = L" F1              切换帮助屏幕\n"
...
void portaldemo_c::on_frame_update(float elapsed_time) {
    ...
    m_camframe2.set_l2p(Matrix44f(m_camctrl2.get_orientation().to_matrix33(), m_camctrl2.get_position()));
...
void portaldemo_c::on_frame_render() {
    ...
    string_wc        str   = GN_formatwcs(L"Camera 2 : (%.2f,%.2f,%.2f)", c_pos.x, c_pos.y, c_pos.z);
