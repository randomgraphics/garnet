...
bool level_c::init(const char * filename) {
    ...
    for (uint i = 0; i < m_sectors.size(); ++i) {
        ...
        m_sectors[i].bsptree.initialize(s.bspNodeList.data, s.bspNodeList.length);

        // setup sector portal list
        uint j, k;
        m_sectors[i].portals.resize(s.frontPortalIDList.length + s.backPortalIDList.length);
        k = 0;
        for (j = 0; j < s.frontPortalIDList.length; ++j, ++k) { m_sectors[i].portals[k].initialize(m_lvlfile->portals.data[s.frontPortalIDList.data[j]]); }
        for (j = 0; j < s.backPortalIDList.length; ++j, ++k) { m_sectors[i].portals[k].initialize(m_lvlfile->portals.data[s.backPortalIDList.data[j]], true); }
    }
