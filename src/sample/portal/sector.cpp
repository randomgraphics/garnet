...
//
// ----------------------------------------------------------------------------
bool sector_c::collide_with_radial(collideinfo_s & cinfo, const Vector3f & start, const Vector3f & dir) const {
    // make sure start point is in myself
    GN_ASSERT(-1 != in_solid(start));

    // find the portal which the ray going through
    Vector3f                      ipoint;
    portal_list_c::const_iterator pi = portals.begin(), pe = portals.end();
    for (; pi != pe; ++pi) {
        ...
        if (1 == side) {
            if (intersection_polygon_ray(ipoint, pi->plane, GN_vec2ptr(pi->verts), pi->verts.size(), start, dir)) break;
        } else {
...
//
// ----------------------------------------------------------------------------
bool sector_c::collide_with_segment(collideinfo_s & cinfo, const Vector3f & start, const Vector3f & end) const {
    // make sure start point is in myself
    // GN_ASSERT( -1 != in_solid(start) );
    if (-1 == in_solid(start)) {
        halt = true;
    ...
    for (; pi != pe; ++pi) {
        ...
        // check collision of the portal with start-end
        if (!intersection_polygon_segment(ipoint, pi->plane, GN_vec2ptr(pi->verts), pi->verts.size(), start, end)) continue;
