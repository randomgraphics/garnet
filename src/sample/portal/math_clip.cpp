...
//
// ----------------------------------------------------------------------------
static GN_INLINE void clip_edge(Vector3f *& result, const Vector3f & start, const Vector3f & end, const Plane3f & p) {
...
void clip_polygon_with_plane_list(std::vector<Vector3f> & result, const Vector3f * verts, size_t numvert, const Plane3f * planes, size_t numplane) {
    ...
    for (i = 0; i < numplane; ++i, ++planes) {
        ...
        Vector3f *       v2 = output;
        for (j = 0; j < numvert - 1; ++j, ++v1) { clip_edge(v2, *v1, v1[1], *planes); }
    ...
    if (numvert > 0) { memcpy(&result[0], output, sizeof(Vector3f) * numvert); }
