...
FirstPersonCamera::FirstPersonCamera(Handness h)
    : mHandNess(h), mPosition(0, 0, 0), mAngle(0, 0, 0), mMoveSpeed(1.0), mRotateSpeed(GN_PI / 4.0f), mAxisSensitivity(0.01f),
      mRotation3x3(Matrix33f::sIdentity()), mRotation4x4(Matrix44f::sIdentity()), mView(Matrix44f::sIdentity()),
      mTargetPosition(mPosition, mPosition, INTERP_TIME), mTargetAngle(mAngle, mAngle, INTERP_TIME) {
...
//
// -----------------------------------------------------------------------------
void FirstPersonCamera::setTargetPosition(const Vector3f & p) { mTargetPosition.reset(mPosition, p, INTERP_TIME); }

//
//
// -----------------------------------------------------------------------------
void FirstPersonCamera::setTargetAngle(const Vector3f & r) {
    Vector3f v(fmod(r.x, GN_TWO_PI), fmod(r.y, GN_TWO_PI), fmod(r.z, GN_TWO_PI));
...
void FirstPersonCamera::update(float timeslice) {
    ...
    // update rotation
    if (ZERO3 != delta_a) { setTargetAngle(mTargetAngle.getTarget() + delta_a); }
    ...
    // update position
    if (ZERO3 != delta_p) { setTargetPosition(mTargetPosition.getTarget() + delta_p * mRotation3x3); }
...
void FirstPersonCamera::connectToInput() {
    ...
    if (gInputPtr) {
        manageTether(gInput.sigAxisMove.connect<FirstPersonCamera, &FirstPersonCamera::onAxisMove>(this));
    } else {
