...
GN::util::ArcBall::ArcBall(Handness h)
    : mTranslation(0, 0, 0), mQuat(Quaternionf::sIdentity()), mRotation3x3(Matrix33f::sIdentity()), mRotation4x4(Matrix44f::sIdentity()),
      mTransView(Matrix44f::sIdentity()), mWindowCenter(0, 0), mWindowHalfSize(1, 1), mHandness(LEFT_HAND == h ? -1.0f : 1.0f), mRolling(false),
      mMoveSpeed(1.0f), mMoving(false) {}
...
void GN::util::ArcBall::connectToInput() {
    ...
    if (gInputPtr) {
        manageTether(gInput.sigKeyPress.connect<ArcBall, &ArcBall::onKeyPress>(this));
        manageTether(gInput.sigAxisMove.connect<ArcBall, &ArcBall::onAxisMove>(this));
    } else {
