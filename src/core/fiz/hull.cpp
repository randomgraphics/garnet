...
public:
    ...
    Box             localAabb() const override { return Box(-mHalfExtents, mHalfExtents); }
    Scalar          calculateVolume() const override { return 8.0f * mHalfExtents.x * mHalfExtents.y * mHalfExtents.z; }
...
public:
    ...
    Scalar calculateVolume() const override { return (4.0f / 3.0f) * PI * mRadius * mRadius * mRadius; }
...
public:
    ...
    Scalar calculateVolume() const override { return (4.0f / 3.0f) * PI * mRadius * mRadius * mRadius + PI * mRadius * mRadius * (2.0f * mHalfHeight); }
    Scalar radius() const override { return mRadius; }
...
public:
    ...
    Scalar calculateVolume() const override { return PI * mRadius * mRadius * (2.0f * mHalfHeight); }
    Scalar radius() const override { return mRadius; }
    Scalar halfHeight() const override { return mHalfHeight; }
...
public:
    ...
    Scalar                       calculateVolume() const override { return mAabb.w * mAabb.h * mAabb.d * 0.5f; }
...
public:
    MeshHullImpl(const Vector3 * vertices, size_t vertexCount, const uint32_t * indices, size_t indexCount): MeshHull(TYPE_INFO(), "MeshHull") {
        ...
        if (indices && indexCount > 0) { mIndices.assign(indices, indices + indexCount); }
    }

    HullType                      type() const override { return HullType::MESH; }
    Box                           localAabb() const override { return mAabb; }
    Scalar                        calculateVolume() const override { return mAabb.w * mAabb.h * mAabb.d; }
    const std::vector<Vector3> &  vertices() const override { return mVertices; }
...
AutoRef<Hull> Hull::createBox(const Vector3 & halfExtents) { return AutoRef<Hull>(new BoxHullImpl(halfExtents)); }

AutoRef<Hull> Hull::createSphere(Scalar radius) { return AutoRef<Hull>(new SphereHullImpl(radius)); }

AutoRef<Hull> Hull::createCapsule(Scalar radius, Scalar halfHeight) { return AutoRef<Hull>(new CapsuleHullImpl(radius, halfHeight)); }

AutoRef<Hull> Hull::createCylinder(Scalar radius, Scalar halfHeight) { return AutoRef<Hull>(new CylinderHullImpl(radius, halfHeight)); }

AutoRef<Hull> Hull::createConvex(const Vector3 * points, size_t count) { return AutoRef<Hull>(new ConvexHullImpl(points, count)); }
