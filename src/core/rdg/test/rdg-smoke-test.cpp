...
TEST_CASE("RDG smoke: prepare then present immediately (no draws)", "[rdg][smoke][gpu]") {
    ...
    auto           backbuffer =
        Backbuffer::create("backbuffer", Backbuffer::CreateParameters {.context = gpuContext, .descriptor = Backbuffer::Descriptor {}.setDimensions(w, h)});
    REQUIRE(backbuffer);

    auto prepareAction = PrepareBackbuffer::create("prepare", PrepareBackbuffer::CreateParameters {.gpu = gpuContext});
    REQUIRE(prepareAction);
    auto presentAction = PresentBackbuffer::create("present", PresentBackbuffer::CreateParameters {.gpu = gpuContext});
    REQUIRE(presentAction);

    auto wf = renderGraph->createWorkflow("Render");
    REQUIRE(wf);
    REQUIRE(wf.valid());
    REQUIRE(wf.tasks().empty());
    REQUIRE(wf.name() == "Render");
...
TEST_CASE("RDG smoke: clear-to-red headless render", "[rdg][smoke][gpu]") {
    ...
    auto           backbuffer = Backbuffer::create(
        "backbuffer", Backbuffer::CreateParameters {.context = gpuContext, .descriptor = Backbuffer::Descriptor {}.setDimensions(displayWidth, displayHeight)});
    REQUIRE(backbuffer);
    ...
    REQUIRE(renderWorkflow.name() == "Render");
