...
bool GN::input::InputDInput::attachToWindow(intptr_t disp, intptr_t window) {
    ...
    GN_DX_CHECK_RETURN(mKeyboard->SetCooperativeLevel((HWND) window, DISCL_NONEXCLUSIVE | DISCL_FOREGROUND), false);

    // set mouse cooperative level
    GN_DX_CHECK_RETURN(mMouse->SetCooperativeLevel((HWND) window, DISCL_NONEXCLUSIVE | DISCL_FOREGROUND), false);
...
void GN::input::InputDInput::pollKeyboard() {
    ...
    GN_DX_CHECK_DO(mKeyboard->GetDeviceData(sizeof(DIDEVICEOBJECTDATA), od, (LPDWORD) &elementCount, 0),
                   mLost = (DIERR_INPUTLOST == hr || DIERR_NOTACQUIRED == hr);
                   return;);
...
void GN::input::InputDInput::pollMouse() {
    ...
    uint32             elementCount = IDX_INPUT_BUFFER_SIZE;
    GN_DX_CHECK_DO(
        mMouse->GetDeviceData(sizeof(DIDEVICEOBJECTDATA), od, (LPDWORD) &elementCount, 0), mLost = (DIERR_INPUTLOST == hr || DIERR_NOTACQUIRED == hr); return;);
...
bool GN::input::InputDInput::diInit() {
    ...
    // create dinput
    GN_DX_CHECK_RETURN(dc((HINSTANCE) GetModuleHandle(0), DIRECTINPUT_VERSION, IID_IDirectInput8, (void **) &mDInput, 0), false);
