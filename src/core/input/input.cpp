...
public:
    ...
    virtual KeyEvent popLastKeyEvent() {
        KeyEvent k;
        k.u16 = 0;
        return k;
    }
    virtual const KeyStatus * getKeyboardStatus() const {
        static KeyStatus ks[256];
        return ks;
    }
...
GN_API bool GN::input::initializeInputSystem(InputAPI api) {
    ...
    // then create new one
    switch (api) {
    case InputAPI::NATIVE:
        return NULL != sCreateNativeInputSystem();
    ...
    default:
        GN_ERROR(sLogger)("unknow or unsupport API : %d", (int) api.toRawEnum());
        return 0;
    }
