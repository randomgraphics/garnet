...
void GN::input::BasicInput::triggerKeyPress(KeyCode code, bool keydown) {
    ...
    if (KeyCode::LCTRL == code)
        mKeyFlags.lctrl = keydown;
    else if (KeyCode::RCTRL == code)
        mKeyFlags.rctrl = keydown;
    else if (KeyCode::LALT == code)
        mKeyFlags.lalt = keydown;
    else if (KeyCode::RALT == code)
        mKeyFlags.ralt = keydown;
    else if (KeyCode::LSHIFT == code)
        mKeyFlags.lshift = keydown;
    else if (KeyCode::RSHIFT == code)
        mKeyFlags.rshift = keydown;
...
void GN::input::BasicInput::triggerCharPress(char ch) {
    ...
    if ((unsigned char) ch < 128) {
        // ASCII character
        // GN_TRACE( "Char press: %s", std::string(&ch,1).data() );
        sigCharPress(ch);
        ...
        // GN_TRACE( "Char press: %s", std::string(mHalfBytes,2).data() );
        sigCharPress(wch[0]);
