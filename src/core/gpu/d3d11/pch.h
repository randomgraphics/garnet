...
struct D3DUdaHelper {
    ...
    D3DUdaHelper(ID3D11DeviceContext * cxt = nullptr): uda(nullptr) { SetContext(cxt); }

    D3DUdaHelper(ID3D11DeviceContext & cxt): uda(nullptr) { SetContext(&cxt); }

    ~D3DUdaHelper() { SetContext(nullptr); }

    void SetContext(ID3D11DeviceContext * cxt) {
        if (uda) uda->Release(), uda = nullptr;
        if (cxt) {
            if (FAILED(cxt->QueryInterface<ID3DUserDefinedAnnotation>(&uda))) { uda = nullptr; }
        }
    }

    INT BeginEvent(LPCWSTR name) { return uda ? uda->BeginEvent(name) : -1; }

    INT EndEvent() { return uda ? uda->EndEvent() : -1; }
...
struct PixPerfScopeEvent {
    ...
    ~PixPerfScopeEvent() {
        if (gD3D11EnablePixPerf) { uda.EndEvent(); }
    }
};
