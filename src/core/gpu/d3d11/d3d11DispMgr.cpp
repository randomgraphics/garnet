...
bool GN::gfx::D3D11Gpu::dispInit() {
    ...
    GN_RETURN_FALSE_ON_HR_FAILED(D3D11CreateDevice(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, 0, NULL, 0, D3D11_SDK_VERSION, NULL, &featureLevel, NULL));

    // create device
    UINT flags = D3D11_CREATE_DEVICE_SINGLETHREADED;
    if (ro.debug) flags |= D3D11_CREATE_DEVICE_DEBUG;
    HRESULT firstTry = D3D11CreateDeviceAndSwapChain(mAdapter, ro.reference ? D3D_DRIVER_TYPE_REFERENCE : D3D_DRIVER_TYPE_HARDWARE,
    ...
    if (FAILED(firstTry) && ro.debug) {
        ...
        GN_RETURN_FALSE_ON_HR_FAILED(D3D11CreateDeviceAndSwapChain(mAdapter, ro.reference ? D3D_DRIVER_TYPE_REFERENCE : D3D_DRIVER_TYPE_HARDWARE,
                                                                   NULL, // software module handle
                                                                   flags, &featureLevel, 1, D3D11_SDK_VERSION, &sd, &mSwapChain, &mDevice,
                                                                   NULL, // feature level
                                                                   &mDeviceContext));
...
void GN::gfx::D3D11Gpu::dispQuit() {
    ...
    if (mSwapChain && getOptions().displayMode.mode == DisplayMode::FULL_SCREEN) { mSwapChain->SetFullscreenState(FALSE, NULL); }
