...
//
// -----------------------------------------------------------------------------
bool GN::gfx::D3D11RTMgr::bind(const RenderTargetDesc & oldrt, const RenderTargetDesc & newrt, bool skipDirtyCheck, bool & renderTargetSizeChanged) {
    ...
    if (0 == newrt.colortargets.size() && 0 == newrt.depthstencil.texture) {
        ...
        for (uint32_t i = 0; i < newrt.colortargets.size(); ++i) {
            ...
            auto tex = rtt.texture.get<D3D11Texture>();
        ...
        auto dstex = newrt.depthstencil.texture.get<D3D11Texture>();
        if (dstex) {
