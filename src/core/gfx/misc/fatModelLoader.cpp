...
struct Skinning {
    ...
    bool operator==(const Skinning & rhs) const {
        for (int i = 0; i < 4; ++i) {
            if (joints[i] != rhs.joints[i] || weights[i] != rhs.weights[i]) { return false; }
        }
...
Matrix44f & sFbxMatrix2GarnetMatrix(Matrix44f & gnmat, const FbxMatrix & fbxmat) {
    ...
    gnmat.set((float) d44[0], (float) d44[4], (float) d44[8], (float) d44[12], (float) d44[1], (float) d44[5], (float) d44[9], (float) d44[13], (float) d44[2],
              (float) d44[6], (float) d44[10], (float) d44[14], (float) d44[3], (float) d44[7], (float) d44[11], (float) d44[15]);
...
static FbxSkeleton * sFbxNode2LimbNode(FbxNode * fbxnode) {
    if (NULL == fbxnode) { return NULL; }

    FbxSkeleton * fbxlimb = fbxnode->GetSkeleton();

    if (NULL == fbxlimb) { return NULL; }

    if (fbxlimb->GetSkeletonType() != FbxSkeleton::eLimbNode) { return NULL; }
...
static void sLoadFbxLimbNodes(FatSkeleton & fatsk, uint32_t parent, uint32_t previousSibling, FbxNode * fbxnode) {
    ...
    if (FatJoint::NO_JOINT != parent && FatJoint::NO_JOINT == fatsk.joints[parent].child) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        // subtree.
        if (newJointCount > oldJointCount) { newPrevSibling = oldJointCount; }
    }
}
...
//
// -----------------------------------------------------------------------------
static void sLoadFbxSkeletons(FatModel & fatmodel, FbxNode * fbxnode) {
    if (sFbxNode2LimbNode(fbxnode)) {
        ...
        int count = fbxnode->GetChildCount();
        for (int i = 0; i < count; ++i) { sLoadFbxSkeletons(fatmodel, fbxnode->GetChild(i)); }
    }
...
// -----------------------------------------------------------------------------
static void sBuildSkinningMap(OUT SkinningMap & sm, IN FbxSdkWrapper & sdk, IN const FbxMesh * fbxmesh) {
    ...
    for (int iDef = 0; iDef < numdef; ++iDef) {
        ...
        for (int iCluster = 0; iCluster < numclusters; ++iCluster) {
            ...
            SkinningWeight sw;
            if (!sSearchForNamedJoint(sw.skeleton, sw.joint, sdk.jointMap, link->GetName())) {
...
static void sLoadFbxVertexSkinning(INOUT uint32_t & skeleton, // Index into FatModel::skeleton arrayreturns the skeleton that of the joint
    ...
    if (FatMesh::NO_SKELETON != skeleton) {
        ...
        float sum = 0.0f;
        for (size_t i = 0; i < GN_ARRAY_COUNT(sk.joints); ++i) {
            if (FatJoint::NO_JOINT != sk.joints[i]) { sum += sk.weights[i]; }
        }
        ...
        if (FatJoint::NO_JOINT == sk.joints[0] || 0.0f == sum) {
            ...
            for (size_t i = 0; i < GN_ARRAY_COUNT(sk.joints); ++i) {
                if (FatJoint::NO_JOINT != sk.joints[i]) { sk.weights[i] *= invsum; }
            }
        }
    }
}
...
//
// -----------------------------------------------------------------------------
static bool sBuildFatMeshSubsetJointList(FatMesh & mesh) {
...
static void sLoadFbxMesh(FatModel & fatmodel, const StrA & filename, FbxSdkWrapper & sdk, FbxMesh * fbxmesh) {
    ...
    for (uint32_t sortedPolygonIndex = 0; sortedPolygonIndex < sortedPolygons.size(); ++sortedPolygonIndex) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            if (fatmesh.skeleton != FatMesh::NO_SKELETON) {
                // Sanity check
                for (size_t i = 0; i < GN_ARRAY_COUNT(sk.joints); ++i) {
                    GN_ASSERT(sk.joints[i] == FatJoint::NO_JOINT || sk.joints[i] < fatmodel.skeletons[fatmesh.skeleton].joints.size());
                }
    ...
    calculateBoundingBox(fatmesh.bbox, &vertices->x, sizeof(vertices[0]), &vertices->y, sizeof(vertices[0]), &vertices->z, sizeof(vertices[0]),
...
//
// -----------------------------------------------------------------------------
static void sLoadFbxAnimStack(FatModel & fatmodel, FbxAnimStack & fbxanim) {
    #if 1
...
static void sLoadFbxAnimations(FatModel & fatmodel, FbxScene & fbxscene) {
    ...
    int animCount = fbxscene.GetSrcObjectCount<FbxAnimStack>();
    for (int i = 0; i < animCount; i++) {
        FbxAnimStack * fbxanim = fbxscene.GetSrcObject<FbxAnimStack>(i);
...
static bool sLoadFromFBX(FatModel & fatmodel, File & file, const StrA & filename) {
    ...
    if (SceneAxisSystem != OurAxisSystem) { OurAxisSystem.ConvertScene(gScene); }
    ...
    int nummat = gScene->GetSrcObjectCount<FbxSurfaceMaterial>();
        ...
        static void sPrintNodeRecursivly(StrA & hierarchy, FbxNode * node, int depth) {
            ...
            if (a) {
                static const char * sAttributeTypeNames[] = {
                    "eUNIDENTIFIED",    "eNULL",          "eMARKER",
                    "eSKELETON",        "eMESH",          "eNURB",
                    "ePATCH",           "eCAMERA",        "eCAMERA_STEREO",
                    "eCAMERA_SWITCHER", "eLIGHT",         "eOPTICAL_REFERENCE",
                    "eOPTICAL_MARKER",  "eNURBS_CURVE",   "eTRIM_NURBS_SURFACE",
                    "eBOUNDARY",        "eNURBS_SURFACE", "eSHAPE",
                    "eLODGROUP",        "eSUBDIV",        "eCACHED_EFFECT",
                };
                ...
                if (FbxNodeAttribute::eSkeleton == atype) {
                    ...
                    case FbxSkeleton::eRoot:
                        hierarchy += "eSKELETON : eRoot";
                        break;
                    case FbxSkeleton::eLimb:
                        hierarchy += "eSKELETON : eLimb";
                        break;
                    case FbxSkeleton::eLimbNode:
                        hierarchy += "eSKELETON : eLimbNode";
                        break;
                    case FbxSkeleton::eEffector:
                        hierarchy += "eSKELETON : eEffector";
                        break;
                    default:
                        hierarchy += "eSKELETON : [INVALID TYPE]";
                        break;
                    }
            ...
            for (int i = 0; i < count; ++i) { sPrintNodeRecursivly(hierarchy, node->GetChild(i), depth + 1); }
        }
    };
...
//
// -----------------------------------------------------------------------------
void sLoadAiMeshSkeleton(FatModel & fatmodel, FatMesh & fatmesh, const aiScene & aiscene, const aiMesh & aimesh, const aiMatrix4x4 & meshTransform) {
...
//
// -----------------------------------------------------------------------------
static bool sLoadAiVertices(FatMesh & fatmesh,
...
//
// -----------------------------------------------------------------------------
static void sLoadAiNodeRecursivly(FatModel & fatmodel, const aiScene * aiscene, const aiNode * ainode, const aiMatrix4x4 & parentTransform) {
    if (NULL == ainode) return;
...
static bool sLoadFromAssimp(FatModel & fatmodel, const StrA & filename) {
    ...
    for (uint32_t i = 0; i < fatmodel.materials.size(); ++i) {
        ...
        if (aimat->GetTextureCount(aiTextureType_DIFFUSE) > 0 && aiReturn_SUCCESS == aimat->GetTexture(aiTextureType_DIFFUSE, 0, &path)) {
