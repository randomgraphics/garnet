...
//
// -----------------------------------------------------------------------------
static bool sLoadModelHierarchyFromXPR(ModelHierarchyDesc & desc, File & file) {
    // load XPR file
    XPRScene xpr;
...
//
// -----------------------------------------------------------------------------
static bool sHasTex0(const MeshVertexFormat & vf) { return vf.hasSemantic("texcoord"); }
...
//
// -----------------------------------------------------------------------------
static bool sLoadModelHierarchyFromASE(ModelHierarchyDesc & desc, File & file) {
    // load ASE scene
    AseScene ase;
...
#define FULL_MESH_NAME(n) fmt::format("%s.%s", filename.data(), n.data())
    ...
    for (size_t i = 0; i < ase.meshes.size(); ++i) {
        ...
        const std::string & meshname = FULL_MESH_NAME(src.name);
    ...
    for (size_t i = 0; i < ase.subsets.size(); ++i) {
        ...
        if (model.hasTexture("ALBEDO_TEXTURE") && !am.mapdiff.bitmap.empty()) { model.textures["ALBEDO_TEXTURE"].resourceName = am.mapdiff.bitmap; }
...
// -----------------------------------------------------------------------------
template<typename T>
static inline int sGetLayerElementIndex(const FbxLayerElementTemplate<T> * elements, int vertexIndex, int polygonIndex, int cornerIndex) {
    if (NULL == elements) return -1;
...
struct SortPolygonByMaterial {
    ...
    SortPolygonByMaterial(FbxLayerElementMaterial * m): materials(m) {}
...
struct MeshVertex {
    ...
    static MeshVertexFormat sGetVertexFormat() { return MeshVertexFormat::XYZ_NORM_UV(); }
};
...
//
// -----------------------------------------------------------------------------
static void sLoadFbxMesh(ModelHierarchyDesc & desc, const std::string & filename, ModelHierarchyDesc::NodeDesc & gnnode, FbxSdkWrapper & sdk, FbxNode * fbxnode,
    ...
    int nummat;
    if (fbxMaterials && FbxLayerElement::eByPolygon == fbxMaterials->GetMappingMode() && FbxLayerElement::eIndexToDirect == fbxMaterials->GetReferenceMode()) {
        // multiple materials
    ...
    if (nummat > 1) { std::sort(sortedPolygons.begin(), sortedPolygons.end(), SortPolygonByMaterial(fbxMaterials)); }
    ...
    for (uint32 sortedPolygonIndex = 0; sortedPolygonIndex < sortedPolygons.size(); ++sortedPolygonIndex) {
        ...
        if (matid != lastMatID) {
            ...
            if (mat) {
                ...
                if (model.hasTexture("ALBEDO_TEXTURE") && diffuse) { model.textures["ALBEDO_TEXTURE"].resourceName = fs::resolvePath(dirname, diffuse); }
            }
        }
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            if (-1 != uvIndex) { uvIndex = sGetLayerElementIndex(fbxUVs, posIndex, polygonIndex, i); }

            // get index into normal elements.
            if (-1 != normalIndex) { normalIndex = sGetLayerElementIndex(fbxNormals, posIndex, polygonIndex, i); }
...
//
// -----------------------------------------------------------------------------
static bool sLoadFbxNodeRecursivly(ModelHierarchyDesc & desc, const std::string & filename, FbxSdkWrapper & sdk, FbxNode * node, FbxNode * parent) {
    ...
    if (FbxNodeAttribute::eMesh == type) {
        // load mesh node
        std::string fullMeshName = filename + "." + name;
    ...
    for (int i = 0; i < node->GetChildCount(); ++i) {
        if (!sLoadFbxNodeRecursivly(desc, filename, sdk, node->GetChild(i), node)) { return false; }
    }
...
//
// -----------------------------------------------------------------------------
static bool sLoadModelHierarchyFromFBX(ModelHierarchyDesc & desc, File & file) {
#ifdef HAS_FBX
    ...
    // detect file format
    std::string filename    = fs::toNativeDiskFilePath(file.name());
    int  lFileFormat = -1;
...
// post error message
// -----------------------------------------------------------------------------
static void sPostXMLError(const XmlNode & node, const std::string & msg) {
...
//
// -----------------------------------------------------------------------------
static bool sParseModel(ModelHierarchyDesc & desc, XmlElement & root, const std::string & basedir) {
...
//
// -----------------------------------------------------------------------------
static bool sParseNode(ModelHierarchyDesc & desc, XmlElement & root) {
...
//
// -----------------------------------------------------------------------------
static bool sLoadModelHierarchyFromXML(ModelHierarchyDesc & desc, File & file) {
    XmlDocument    doc;
    XmlParseResult xpr;
    ...
    std::string basedir = fs::dirName(file.name());
...
static bool sSaveModelHierarchyToXML(const ModelHierarchyDesc & desc, const char * filename) {
    ...
    std::string fullpath = fs::resolvePath(fs::getCurrentDir(), filename);
    filename      = fullpath;
    std::string dirname  = fs::dirName(fullpath);
    std::string basename = fs::baseName(fullpath);
    ...
    // write meshes
    int                   meshindex = 0;
    StringMap<char, std::string> meshNameMapping;
    for (const StringMap<char, MeshResourceDesc>::KeyValuePair * i = desc.meshes.first(); i != NULL; i = desc.meshes.next(i)) {
    ...
    models->name        = "models";
    for (const StringMap<char, gfx::ModelResourceDesc>::KeyValuePair * i = desc.models.first(); i != NULL; i = desc.models.next(i)) {
        const std::string &      modelName = i->key;
        ModelResourceDesc model     = i->value;

        std::string * pNewMeshName = meshNameMapping.find(model.mesh);
        if (NULL != pNewMeshName) { model.mesh = *pNewMeshName; }
    ...
    // rename nodes
    int                   entityIndex = 0;
    StringMap<char, std::string> entityNameMap;
    ...
    nodes->name        = "nodes";
    for (const StringMap<char, ModelHierarchyDesc::NodeDesc>::KeyValuePair * i = desc.nodes.first(); i != NULL; i = desc.nodes.next(i)) {
        const std::string &                         nodeName = *entityNameMap.find(i->key);
        ...
        a->name                  = "parent";
        std::string * pParentEntityName = entityNameMap.find(nodeDesc.parent);
...
GN_API bool GN::gfx::ModelHierarchyDesc::loadFromFile(const char * filename) {
    ...
    // get file extension
    std::string ext = fs::extName(filename);
    ...
    } else if (sStrEndWithI(filename, ".xpr") || sStrEndWithI(filename, ".tpr")) {
