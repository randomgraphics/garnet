...
//
// -----------------------------------------------------------------------------
static bool sCheckGpuCaps(Gpu & r, const EffectGpuProgramDesc & desc) {
    const GpuCaps & caps = r.caps();

    // check shader model
    if (0 == (desc.gpd.shaderModels & caps.shaderModels)) { return false; }
...
// -----------------------------------------------------------------------------
static void sMergeRenderStates(EffectRenderStateDesc & out, const EffectRenderStateDesc & a, const EffectRenderStateDesc & b) {
...
// -----------------------------------------------------------------------------
static bool sCheckShaderTextures(const EffectResourceDesc & effectDesc, const EffectGpuProgramDesc & programDesc, const char * programName,
...
// -----------------------------------------------------------------------------
static bool sCheckShaderUniforms(const EffectResourceDesc & effectDesc, const EffectGpuProgramDesc & programDesc, const char * programName,
                                 const GpuProgram & program) {
    const GpuProgramParameterDesc & param = program.getParameterDesc();
...
static bool sAttributeNameEqual(const char * name1, const char * name2) {
    ...
    if (len1 >= 2 && '0' == name1[len1 - 1] && (name1[len1 - 2] < '0' || name1[len1 - 2] > '9')) { len1--; }

    size_t len2 = str::length(name2);
    if (len2 >= 2 && '0' == name2[len2 - 1] && (name2[len2 - 2] < '0' || name2[len2 - 2] > '9')) { len2--; }
...
//
// -----------------------------------------------------------------------------
bool GN::gfx::EffectResource::Impl::initGpuProgram(const EffectResourceDesc & effectDesc, const std::string & programName, const EffectGpuProgramDesc & programDesc) {
...
//
// -----------------------------------------------------------------------------
bool GN::gfx::EffectResource::Impl::initTechniques(const EffectResourceDesc & effectDesc) {
    int currentQuality = (int) 0x80000000; // minimal signed integer

    for (const auto & techDesc : effectDesc.techniques) {
        // ignore the technique with lower quality
        if (techDesc.quality <= currentQuality) { continue; }
...
//
// -----------------------------------------------------------------------------
uint32 GN::gfx::EffectResource::Impl::findGpuProgram(const EffectResourceDesc & effectDesc, const std::string & programName) {
...
class EffectResourceInternal : public EffectResource {
    EffectResourceInternal(GpuResourceDatabase & db): EffectResource(db) {}
...
//
// -----------------------------------------------------------------------------
GN::gfx::EffectResource::EffectResource(GpuResourceDatabase & db): GpuResource(db), mImpl(NULL) { mImpl = new Impl(*this); }

//
//
// -----------------------------------------------------------------------------
GN::gfx::EffectResource::~EffectResource() { delete mImpl; }
...
//
// -----------------------------------------------------------------------------
AutoRef<EffectResource> GN::gfx::EffectResource::loadFromFile(GpuResourceDatabase & db, const char * filename) {
    if (NULL == filename) {
