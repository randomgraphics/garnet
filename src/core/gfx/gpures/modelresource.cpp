...
//
// -----------------------------------------------------------------------------
static void sApplyRenderStates(GN::gfx::GpuContext::RenderStates & dst, const GN::gfx::GpuContext::RenderStates & src,
...
//
// -----------------------------------------------------------------------------
GN::gfx::ModelResource::Impl::TextureItem::TextureItem(): mOwner(0) {}
...
//
// -----------------------------------------------------------------------------
GN::gfx::ModelResource::Impl::UniformItem::UniformItem(): mOwner(0) {}
...
void GN::gfx::ModelResource::Impl::UniformItem::updateContext(Uniform * uniform) {
    ...
    for (size_t i = 0; i < prop.bindings.size(); ++i) {
        ...
        if (location.gpuProgramParameterIndex >= gc.uniforms.size()) { gc.uniforms.resize(location.gpuProgramParameterIndex + 1); }
...
//
// -----------------------------------------------------------------------------
AutoRef<ModelResource> GN::gfx::ModelResource::Impl::makeClone(const char * nameOfTheClone) const {
...
//
// -----------------------------------------------------------------------------
AutoRef<UniformResource> GN::gfx::ModelResource::Impl::uniformResource(const char * effectParameterName) const {
    EffectResource * effect = mEffectResource;
    if (NULL == effect) {
...
bool GN::gfx::ModelResource::Impl::setMeshResource(GpuResource * resource, const MeshResourceSubset * subset) {
    ...
    for (size_t i = 0; i < mPasses.size(); ++i) {
        ...
        if (mMeshResource) { mMeshResource->applyToContext(pass.gc); }
    }
...
//
// -----------------------------------------------------------------------------
AutoRef<MeshResource> GN::gfx::ModelResource::Impl::meshResource(MeshResourceSubset * subset) const {
...
bool GN::gfx::ModelResource::Impl::setEffectResource(GpuResource * resource) {
    ...
    for (uint32_t i = 0; i < numtextures; ++i) {
        ...
        if (!texres) {
            ...
            StrA                                      texname = str::format("%s.texture.%s", getModelName(), tp.parameterName.data());
    ...
    for (uint32_t i = 0; i < numuniforms; ++i) {
        ...
        if (!unires) {
            ...
            StrA                                      uniname = str::format("%s.uniform.%s", getModelName(), up.parameterName.data());
...
void GN::gfx::ModelResource::Impl::draw() const {
    ...
    if (0 == subset.basevtx && 0 == subset.numvtx) { subset.numvtx = meshdesc.numvtx; }
    if (0 == subset.startidx && 0 == subset.numidx) { subset.numidx = meshdesc.numidx; }

    // draw
    GN_GPU_DEBUG_MARK_BEGIN(&g, str::format("ModelResource::draw : %s (%s)", mOwner.name().data(),
    ...
    for (size_t i = 0; i < mPasses.size(); ++i) {
        ...
        // do rendering
        if (gc.idxbuf) {
            g.drawIndexed(meshdesc.prim, subset.numidx, subset.basevtx,
                          0, // startvtx,
                          subset.numvtx, subset.startidx);
        } else {
            g.draw(meshdesc.prim, subset.numvtx, subset.basevtx);
        }
    }
    GN_GPU_DEBUG_MARK_END(&g);
}
...
void GN::gfx::ModelResource::Impl::updateVertexFormat() {
    for (size_t pi = 0; pi < mPasses.size(); ++pi) {
        ...
        if (NULL == mMeshResource || NULL == mEffectResource || NULL == mPasses[pi].gc.gpuProgram) { continue; }
...
class ModelResourceInternal : public ModelResource {
    ModelResourceInternal(GpuResourceDatabase & db): ModelResource(db) {}
...
//
// -----------------------------------------------------------------------------
GN::gfx::ModelResource::ModelResource(GpuResourceDatabase & db): GpuResource(db), mImpl(NULL) { mImpl = new Impl(*this); }

//
//
// -----------------------------------------------------------------------------
GN::gfx::ModelResource::~ModelResource() { delete mImpl; }
