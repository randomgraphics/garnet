...
static bool sGetRequiredIntAttrib(T & result, const XmlElement & node, const char * attribName) {
    ...
    if (!a || 0 == str::toInteger<T>(result, a->value.data())) {
        GN_ERROR(sLogger, "Element <%s>: attribute \"%s\" is missing or is not a valid integer.", node.name.data(), attribName ? attribName : "!!!NULLPTR!!!");
        return false;
    } else {
        return true;
    }
...
AutoRef<Blob> sLoadFromMeshXMLFile(File & fp, MeshResourceDesc & desc) {
    ...
    ArrayView<uint8_t> meshData((uint8_t *) blob->data(), blob->size());
    uint32_t           offset = 0;
    for (const XmlNode * n = root->firstc; n != NULL; n = n->nexts) {
        const XmlElement * e = n->toElement();
        if (!e) continue;
