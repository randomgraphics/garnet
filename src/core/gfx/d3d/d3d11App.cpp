...
public:
    ...
    bool AdjustWindow(const Option & o) { return sAdjustWindow(_hwnd, o.width, o.height, o.fullscreen); }

private:
    HWND _hwnd;

    RenderWindow(): _hwnd(0) {}
    ...
    static HWND sCreateWindow(UINT width, UINT height, bool fullscreen) {
        ...
        // create window
        HWND hwnd = ::CreateWindowExW(exStyle, classname, L"D3DApp", style, CW_USEDEFAULT, CW_USEDEFAULT, rc.right - rc.left, rc.bottom - rc.top, parent,
                                      0, // no menu
                                      module, 0);
        if (0 == hwnd) {
    ...
    static LRESULT CALLBACK sStaticWindowProc(HWND wnd, UINT msg, WPARAM wp, LPARAM lp) {
        ...
        case WM_KEYUP:
            if (VK_ESCAPE == wp) { ::PostQuitMessage(0); }
            break;
...
// -----------------------------------------------------------------------------
static DXGI_SAMPLE_DESC sConstructDXGISampleDesc(ID3D11Device & device,
...
bool GN::d3d11::D3D11Application::createDevice() {
    ...
    GN_RETURN_FALSE_ON_HR_FAILED(D3D11CreateDevice(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, flags, NULL, 0, D3D11_SDK_VERSION, NULL, &featureLevel, NULL));

    // create device
    GN_RETURN_FALSE_ON_HR_FAILED(D3D11CreateDevice(mAdapter, D3D_DRIVER_TYPE_HARDWARE,
                                                   nullptr, // software module
                                                   flags, &featureLevel, 1, D3D11_SDK_VERSION, &mDevice, nullptr, &mContext));
