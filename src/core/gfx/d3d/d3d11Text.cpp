...
class GN::d3d11::TextRenderer::Impl : public IDWriteTextRenderer {
    ...
    ID2D1Brush * GetD2DBrush(DWORD color) {
        if (_textColor == color && nullptr != _brush) { return _brush; }
        ...
        GN_RETURN_NULL_ON_HR_FAILED(_rt->CreateSolidColorBrush(c, &brush));
...
public:
    ...
    bool Initialize(ID3D11Device &, const FontDesc & fontdesc) {
        // dwrite stuff
        GN_RETURN_FALSE_ON_HR_FAILED(DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, __uuidof(IDWriteFactory), reinterpret_cast<IUnknown **>(&_dWriteFactory)));
        GN_RETURN_FALSE_ON_HR_FAILED(_dWriteFactory->CreateTextFormat(fontdesc.face, // Font family name.
                                                                      NULL,          // Font collection (NULL sets it to use the system font collection).
                                                                      DWRITE_FONT_WEIGHT_REGULAR, DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL,
                                                                      fontdesc.height, L"en-us", &_textFormat));
        ...
        GN_RETURN_FALSE_ON_HR_FAILED(D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, d2dfo, &_d2dFactory));
    ...
    void Draw2DText(ID3D11DeviceContext & cxt, const TextDesc & td) {
        ...
        rt->BeginDraw();
        rt->DrawText(td.text, (0 == td.len) ? (UINT) wcslen(td.text) : (UINT) td.len, _textFormat, &rc, brush);
        rt->EndDraw();
    }
    ...
    virtual HRESULT STDMETHODCALLTYPE QueryInterface(REFIID riid, void ** ppvObject) {
        if (nullptr == ppvObject) {
            return E_POINTER;
        } else if (riid == __uuidof(IUnknown) || riid == __uuidof(IDWriteTextRenderer) || riid == __uuidof(IDWritePixelSnapping)) {
            *ppvObject = this;
            return S_OK;
        } else {
            *ppvObject = nullptr;
    ...
    virtual HRESULT STDMETHODCALLTYPE DrawGlyphRun(void * clientDrawingContext, FLOAT baselineOriginX, FLOAT baselineOriginY,
    ...
    virtual HRESULT STDMETHODCALLTYPE DrawInlineObject(void * clientDrawingContext, FLOAT originX, FLOAT originY, IDWriteInlineObject * inlineObject,
                                                       BOOL isSideways, BOOL isRightToLeft, IUnknown * clientDrawingEffect) {
        // not implemented yet.
        return S_OK;
    }

    virtual HRESULT STDMETHODCALLTYPE DrawStrikethrough(void * clientDrawingContext, FLOAT baselineOriginX, FLOAT baselineOriginY,
                                                        const DWRITE_STRIKETHROUGH * strikethrough, IUnknown * clientDrawingEffect) {
        // not implemented yet.
        return S_OK;
    }

    virtual HRESULT STDMETHODCALLTYPE DrawUnderline(void * clientDrawingContext, FLOAT baselineOriginX, FLOAT baselineOriginY,
...
// -----------------------------------------------------------------------------
GN::d3d11::TextRenderer::~TextRenderer() { shutdown(); }
...
void GN::d3d11::TextRenderer::draw2DText(ID3D11DeviceContext & cxt, const TextDesc & text) const {
    if (nullptr != _impl) { _impl->Draw2DText(cxt, text); }
}
