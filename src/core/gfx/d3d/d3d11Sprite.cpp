...
static const char * hlslpscode = "Texture2D t0; \n"
                                 "sampler s0; \n"
                                 "struct VSOUT { \n"
                                 "   float4 position  : SV_POSITION; \n"
                                 "   float4 color     : COLOR; \n"
...
public:
    ...
    bool Initialize(ID3D11Device & dev) {
        ...
        cxt->UpdateSubresource(mPureWhiteTexture.resource, 0, nullptr, PURE_WHITE, sizeof(UINT) * 2, sizeof(UINT) * 4);
        ...
        GN_RETURN_FALSE_ON_HR_FAILED(
            dev.CreateInputLayout(elements, (uint32_t) std::size(elements), signature->GetBufferPointer(), signature->GetBufferSize(), &mVertexBinding));
    ...
    void DrawEnd() {
        ...
        if (numPendingSprites > 0) {
            ...
            D3D11_BOX box = {(UINT) (firstPendingSpriteOffset * sizeof(Sprite)), 0, 0, (UINT) (numPendingSprites * sizeof(Sprite)), 1, 1};
            mCxt->UpdateSubresource(mVertexBuffer, 0, &box, mNextPendingSprite, 0, 0);

            mCxt->DrawIndexed((UINT) (numPendingSprites * 6),       // index count
