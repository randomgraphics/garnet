...
class RenderGraphImpl : public RenderGraph {
    ...
    bool validateAndBuildDependencyGraph() {
        ...
        for (size_t taskIdx = 0; taskIdx < mPendingTasks.size(); ++taskIdx) {
            ...
            for (size_t shardIdx = 0; shardIdx < task->shards.size(); ++shardIdx) {
                ...
                if (!validateShard(shard, task->name, shardIdx)) { return false; }
            }
    ...
    Action::ExecutionResult executeTask(size_t taskIndex) {
        ...
        for (size_t shardIdx = 0; shardIdx < task->shards.size(); ++shardIdx) {
            ...
            if (result == Action::ExecutionResult::WARNING) { GN_WARN(sLogger)("Task '{}' shard {} execution completed with warnings", task->name, shardIdx); }
        }
...
public:
    ...
    RenderGraph::ExecutionResult submit(const ExecutionParams & params) override {
        ...
        if (!compileTaskGraph()) {
            ...
            if (params.debug) { execResult.debugStats = "Task graph compilation failed"; }
            return execResult;
        }
        ...
        if (executionOrder.empty()) {
            ...
            execResult.result = Action::ExecutionResult::FAILED;
            if (params.debug) { execResult.debugStats = "Topological sort failed - circular dependency detected"; }
            return execResult;
        }
        ...
        for (size_t taskIdx : executionOrder) {
            ...
            if (result == Action::ExecutionResult::FAILED) {
                ...
                execResult.result = Action::ExecutionResult::FAILED;
                if (params.debug) { execResult.debugStats = StrA::format("Execution failed at task index {}", taskIdx); }
                return execResult;
            }
        ...
        if (params.debug) {
            ...
            if (hasWarning) { stats += " (with warnings)"; }
...
GN_API RenderGraph * createRenderGraph(const CreateRenderGraphParams &) { return new RenderGraphImpl(); }
