...
int FbxBin2Ascii(int argc, const char * argv[]) {
    struct Local {
        ...
        Local(): sdk(NULL), importer(NULL), exporter(NULL), scene(NULL) {}
    ...
    if (NULL == (l.importer = KFbxImporter::Create(l.sdk, "")) || NULL == (l.exporter = KFbxExporter::Create(l.sdk, "")) ||
...
int main(int argc, const char * argv[]) {
#ifdef HAS_FBX
    if (argc > 2 && 0 == str::compareI(argv[1], "-fbx")) { return FbxBin2Ascii(argc, argv); }
#endif
    ...
    if (argc > 2) {
        ...
        outputFile = StrA::format("startup::{}.scene.xml", fs::baseName(inputFile).data());
    }
