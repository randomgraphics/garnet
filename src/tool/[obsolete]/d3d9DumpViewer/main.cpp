...
struct D3D9OperationDump {
    ...
    void draw(IDirect3DDevice9 & dev) {
        if (indexed) {
            dev.DrawIndexedPrimitive((D3DPRIMITIVETYPE) prim, startvtx, minvtxidx, numvtx, startidx, numprim);
        } else {
            dev.DrawPrimitive((D3DPRIMITIVETYPE) prim, startvtx, numprim);
        }
    }
};
...
struct D3D9StateDump {
    ...
    bool onRestore(IDirect3DDevice9 & dev) {
        ...
        for (size_t i = 0; i < GN_ARRAY_COUNT(rendertargets); ++i) {
            ...
            if (0 == i) {
                GN_DX_CHECK_RETURN(dev.GetRenderTarget(0, &rtd.surf), false);
            }
#endif
            else {
                GN_DX_CHECK_RETURN(dev.CreateRenderTarget(rtd.width, rtd.height, (D3DFORMAT) rtd.format, (D3DMULTISAMPLE_TYPE) rtd.msaa, rtd.quality,
                                                          0, // non-lockable
                                                          &rtd.surf, 0),
                                   false);
            }
        ...
        if (depthstencil.inuse) {
            GN_DX_CHECK_RETURN(dev.CreateDepthStencilSurface(depthstencil.width, depthstencil.height, (D3DFORMAT) depthstencil.format,
                                                             (D3DMULTISAMPLE_TYPE) depthstencil.msaa, depthstencil.quality,
                                                             TRUE, // discard
                                                             &depthstencil.surf, 0),
                               false);
        }
    ...
    bool loadFromXml(const XmlNode & root, const StrA & basedir) {
        ...
        for (XmlNode * n = root.firstc; n; n = n->nexts) {
            ...
            if ("vs" == e->name) {
                ...
                if (!sLoadConst(vsconstf[0], GN_ARRAY_COUNT(vsconstf), vsconsti[0], GN_ARRAY_COUNT(vsconsti), vsconstb, GN_ARRAY_COUNT(vsconstb), *e))
                    return false;
            } else if ("psc" == e->name) {
                if (!sLoadConst(psconstf[0], GN_ARRAY_COUNT(psconstf), psconsti[0], GN_ARRAY_COUNT(psconsti), psconstb, GN_ARRAY_COUNT(psconstb), *e))
                    return false;
