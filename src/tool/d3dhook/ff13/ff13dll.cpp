...
HRESULT STDMETHODCALLTYPE IDirect3D9_CreateDevice_Hook(IDirect3D9 * this_, UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags,
                                                       D3DPRESENT_PARAMETERS * pp, IDirect3DDevice9 ** ppReturnedDeviceInterface) {
    AdjustPP(hFocusWindow, pp);
    return g_IDirect3D9_Original.CreateDevice(this_, Adapter, DeviceType, hFocusWindow, BehaviorFlags, pp, ppReturnedDeviceInterface);
}

// -----------------------------------------------------------------------------
static IDirect3D9ExVtbl   g_IDirect3D9Ex_Original;
HRESULT STDMETHODCALLTYPE IDirect3D9Ex_CreateDevice_Hook(IDirect3D9Ex * this_, UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags,
                                                         D3DPRESENT_PARAMETERS * pp, IDirect3DDevice9 ** ppReturnedDeviceInterface) {
    AdjustPP(hFocusWindow, pp);
    return g_IDirect3D9Ex_Original.CreateDevice(this_, Adapter, DeviceType, hFocusWindow, BehaviorFlags, pp, ppReturnedDeviceInterface);
}

// -----------------------------------------------------------------------------
HRESULT STDMETHODCALLTYPE IDirect3D9Ex_CreateDeviceEx_Hook(IDirect3D9Ex * this_, UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags,
                                                           D3DPRESENT_PARAMETERS * pp, D3DDISPLAYMODEEX * pFullscreenDisplayMode,
                                                           IDirect3DDevice9Ex ** ppReturnedDeviceInterface) {
    AdjustPP(hFocusWindow, pp);
    return g_IDirect3D9Ex_Original.CreateDeviceEx(this_, Adapter, DeviceType, hFocusWindow, BehaviorFlags, pp, pFullscreenDisplayMode,
...
BOOL APIENTRY DllMain(HINSTANCE hModule, DWORD fdwReason, LPVOID) {
    if (fdwReason == DLL_PROCESS_ATTACH) // When initializing....
    {
        ...
        if (0 == _stricmp(Work, "gntestd3d9.exe") || 0 == _stricmp(Work, "ffxiiiimg.exe") || 0 == _stricmp(Work, "ffxiii2img.exe")) { HookAPICalls(&D3DHook); }
    }
    return TRUE;
}
