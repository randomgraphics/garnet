    ...
    while (pIteratingIAT->u1.Function) {
        ...
        if (HookFn) {
            ...
            if (IsBadWritePtr((PVOID) pIteratingIAT->u1.Function, 1)) {
                pIteratingIAT->u1.Function = (UINT_PTR) HookFn;
                ...
                if (pIteratingIAT->u1.Function > (UINT_PTR) 0x80000000) pIteratingIAT->u1.Function = (UINT_PTR) HookFn;
            }
        }

        if (DLLHook->UseDefault) pStubs++; // Advance to next stub

        pIteratingIAT++; // Advance to next IAT entry
        pINT++;          // Advance to next INT entry
    }

    if (DLLHook->UseDefault) pStubs->pszNameOrOrdinal = 0; // Final stub is a sentinel
...
bool HookAPICalls(SDLLHook * Hook) {
    ...
    while (pImportDesc->FirstThunk) {
        ...
        if (lstrcmpiA(pszImportModuleName, Hook->Name) == 0) {
            OutputDebugStringA("Found ");
            OutputDebugStringA(Hook->Name);
            OutputDebugStringA("...\n");
