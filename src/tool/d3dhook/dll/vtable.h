...
struct VTable {
    ...
    VTable(): count(0) { InitializeCriticalSection(&cs); }

    ~VTable() { DeleteCriticalSection(&cs); }
};
...
public:
    UpdatePageProtection(CRITICAL_SECTION * cs, void * addr_, const char * interfaceName_)
        : _cs(cs), _address(addr_), _interfaceName(interfaceName_), _succeeded(false) {
...
inline void RealToHooked_General(VTABLE_STRUCT & vtable, VTable<VTABLE_STRUCT> & origin, VTable<VTABLE_STRUCT> & hooked, const char * interfaceName) {
