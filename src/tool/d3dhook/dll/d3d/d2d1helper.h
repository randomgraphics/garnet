...
//
template<typename Type>
D2D1FORCEINLINE typename TypeTraits<Type>::Point Point2(Type x, Type y) {
...
D2D1FORCEINLINE
D2D1_POINT_2F
Point2F(FLOAT x = 0.f, FLOAT y = 0.f) { return Point2<FLOAT>(x, y); }

D2D1FORCEINLINE
D2D1_POINT_2U
Point2U(UINT32 x = 0, UINT32 y = 0) { return Point2<UINT32>(x, y); }

template<typename Type>
D2D1FORCEINLINE typename TypeTraits<Type>::Size Size(Type width, Type height) {
...
D2D1FORCEINLINE
D2D1_SIZE_F
SizeF(FLOAT width = 0.f, FLOAT height = 0.f) { return Size<FLOAT>(width, height); }

D2D1FORCEINLINE
D2D1_SIZE_U
SizeU(UINT32 width = 0, UINT32 height = 0) { return Size<UINT32>(width, height); }

template<typename Type>
D2D1FORCEINLINE typename TypeTraits<Type>::Rect Rect(Type left, Type top, Type right, Type bottom) {
...
D2D1FORCEINLINE
D2D1_RECT_F
RectF(FLOAT left = 0.f, FLOAT top = 0.f, FLOAT right = 0.f, FLOAT bottom = 0.f) { return Rect<FLOAT>(left, top, right, bottom); }

D2D1FORCEINLINE
D2D1_RECT_U
RectU(UINT32 left = 0, UINT32 top = 0, UINT32 right = 0, UINT32 bottom = 0) { return Rect<UINT32>(left, top, right, bottom); }
...
D2D1FORCEINLINE
D2D1_ARC_SEGMENT
ArcSegment(_In_ CONST D2D1_POINT_2F & point, _In_ CONST D2D1_SIZE_F & size, _In_ FLOAT rotationAngle, _In_ D2D1_SWEEP_DIRECTION sweepDirection,
...
D2D1FORCEINLINE
D2D1_BEZIER_SEGMENT
BezierSegment(_In_ CONST D2D1_POINT_2F & point1, _In_ CONST D2D1_POINT_2F & point2, _In_ CONST D2D1_POINT_2F & point3) {
...
D2D1FORCEINLINE
D2D1_ELLIPSE
Ellipse(_In_ CONST D2D1_POINT_2F & center, FLOAT radiusX, FLOAT radiusY) {
...
BrushProperties(_In_ FLOAT opacity = 1.0, _In_ CONST D2D1_MATRIX_3X2_F & transform = D2D1::IdentityMatrix()) {
...
D2D1FORCEINLINE
D2D1_GRADIENT_STOP
GradientStop(FLOAT position, _In_ CONST D2D1_COLOR_F & color) {
...
QuadraticBezierSegment(_In_ CONST D2D1_POINT_2F & point1, _In_ CONST D2D1_POINT_2F & point2) {
...
StrokeStyleProperties(D2D1_CAP_STYLE startCap = D2D1_CAP_STYLE_FLAT, D2D1_CAP_STYLE endCap = D2D1_CAP_STYLE_FLAT, D2D1_CAP_STYLE dashCap = D2D1_CAP_STYLE_FLAT,
...
BitmapBrushProperties(D2D1_EXTEND_MODE extendModeX = D2D1_EXTEND_MODE_CLAMP, D2D1_EXTEND_MODE extendModeY = D2D1_EXTEND_MODE_CLAMP,
...
LinearGradientBrushProperties(_In_ CONST D2D1_POINT_2F & startPoint, _In_ CONST D2D1_POINT_2F & endPoint) {
...
RadialGradientBrushProperties(_In_ CONST D2D1_POINT_2F & center, _In_ CONST D2D1_POINT_2F & gradientOriginOffset, FLOAT radiusX, FLOAT radiusY) {
...
D2D1FORCEINLINE
D2D1_PIXEL_FORMAT
PixelFormat(_In_ DXGI_FORMAT dxgiFormat = DXGI_FORMAT_UNKNOWN, _In_ D2D1_ALPHA_MODE alphaMode = D2D1_ALPHA_MODE_UNKNOWN) {
...
BitmapProperties(CONST D2D1_PIXEL_FORMAT & pixelFormat = D2D1::PixelFormat(), FLOAT dpiX = 96.0f, FLOAT dpiY = 96.0f) {
...
RenderTargetProperties(D2D1_RENDER_TARGET_TYPE type = D2D1_RENDER_TARGET_TYPE_DEFAULT, _In_ CONST D2D1_PIXEL_FORMAT & pixelFormat = D2D1::PixelFormat(),
                       FLOAT dpiX = 0.0, FLOAT dpiY = 0.0, D2D1_RENDER_TARGET_USAGE usage = D2D1_RENDER_TARGET_USAGE_NONE,
...
HwndRenderTargetProperties(_In_ HWND hwnd, _In_ D2D1_SIZE_U pixelSize = D2D1::Size(static_cast<UINT32>(0), static_cast<UINT32>(0)),
...
LayerParameters(_In_ CONST D2D1_RECT_F & contentBounds = D2D1::InfiniteRect(), _In_opt_ ID2D1Geometry * geometricMask = NULL,
                ...
                FLOAT opacity = 1.0, _In_opt_ ID2D1Brush * opacityBrush = NULL, D2D1_LAYER_OPTIONS layerOptions = D2D1_LAYER_OPTIONS_NONE) {
...
DrawingStateDescription(D2D1_ANTIALIAS_MODE      antialiasMode     = D2D1_ANTIALIAS_MODE_PER_PRIMITIVE,
...
public:
    ...
    //
    D2D1FORCEINLINE
    ColorF(UINT32 rgb, FLOAT a = 1.0) { Init(rgb, a); }

    D2D1FORCEINLINE
    ColorF(Enum knownColor, FLOAT a = 1.0) { Init(knownColor, a); }

    D2D1FORCEINLINE
    ColorF(FLOAT r_, FLOAT g_, FLOAT b_, FLOAT a_ = 1.0) {
        this->r = r_;
        this->g = g_;
        this->b = b_;
        this->a = a_;
...
public:
    ...
    D2D1FORCEINLINE
    D2D1_POINT_2F
    TransformPoint(D2D1_POINT_2F point) const {
        D2D1_POINT_2F result = {point.x * _11 + point.y * _21 + _31, point.x * _12 + point.y * _22 + _32};
...
D2D1FORCEINLINE
D2D1_POINT_2F
operator*(const D2D1_POINT_2F & point, const D2D1_MATRIX_3X2_F & matrix) { return Matrix3x2F::ReinterpretBaseType(&matrix)->TransformPoint(point); }

D2D1_MATRIX_3X2_F
IdentityMatrix() { return Matrix3x2F::Identity(); }

} // namespace D2D1

D2D1FORCEINLINE
D2D1_MATRIX_3X2_F
operator*(const D2D1_MATRIX_3X2_F & matrix1, const D2D1_MATRIX_3X2_F & matrix2) {
    return (*D2D1::Matrix3x2F::ReinterpretBaseType(&matrix1)) * (*D2D1::Matrix3x2F::ReinterpretBaseType(&matrix2));
}

inline bool operator==(const D2D1_SIZE_U & size1, const D2D1_SIZE_U & size2) { return (size1.width == size2.width) && (size1.height == size2.height); }
